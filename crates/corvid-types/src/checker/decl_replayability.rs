    ...
    fn walk_deterministic_expr(&mut self, agent: &str, expr: &Expr) {
        match expr {
            ...
            Expr::Match {
                ...
                self.walk_deterministic_expr(agent, scrutinee);
                for arm in arms {
                    if let Some(g) = &arm.guard {
                        self.walk_deterministic_expr(agent, g);
                    }
...
fn collect_replayability_violations_in_expr(expr: &Expr, out: &mut Vec<ReplayabilityViolation>) {
    match expr {
        ...
        Expr::Match {
            ...
            collect_replayability_violations_in_expr(scrutinee, out);
            for arm in arms {
                if let Some(g) = &arm.guard {
                    collect_replayability_violations_in_expr(g, out);
                }
