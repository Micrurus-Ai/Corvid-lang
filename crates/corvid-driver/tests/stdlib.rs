...
#[test]
fn std_json_compiles_as_corvid_source() {
    let repo = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(std::path::Path::parent)
        .expect("repo root");
    let source_path = repo.join("std").join("json.cor");
    let source = fs::read_to_string(&source_path).expect("std/json.cor");

    compile_to_ir_with_config_at_path(&source, &source_path, None)
        .expect("std.json should compile as a standalone Corvid module");
}
