-- Approvals + durable jobs + trace lineage for the Finance Operations Agent.
-- Slice 35V2-P42-D-LR-app-maturity-Finance-1.
--
-- Approvals gate the dangerous financial operations. The payment
-- intent already ships (SubmitPaymentIntent); D-Fin-2 adds four more
-- operational surfaces (cancel subscription, dispute transaction,
-- export report, schedule recurring payment), each developer-authored
-- with its own role, ceiling, and irreversibility. Durable jobs +
-- checkpoints support the 3 cron jobs (nightly_balance_sync,
-- weekly_anomaly_scan, daily_subscription_renewal_check). Trace
-- lineage is the storage backend for the per-job JSONL trace files.
