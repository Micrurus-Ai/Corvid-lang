-- Auth surface for the Customer Support Agent.
-- Slice 35V2-P42-D-LR-app-maturity-CustomerSupport-1.
--
-- Mirrors the Phase 39 auth model: per-tenant identity, sessions
-- bound to a CSRF cookie, API keys with Argon2id-hashed material, and
-- a roles/permissions table that gates dangerous support operations
-- (reply send, refund, escalation, ticket close, account credit). The
-- agent never sends a customer-facing reply or moves money without a
-- human with the right role approving it.
