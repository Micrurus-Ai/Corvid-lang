-- Approvals + durable jobs + trace lineage for the Code Maintenance Agent.
-- Slice 35V2-P42-D-LR-app-maturity-CodeMaintenance-1.
--
-- Approvals gate the dangerous code-maintenance operations.
-- PostReviewComment + CreatePatchProposal already ship; D-CM-2 adds
-- three more (OpenPullRequest, MergePullRequest, TagRelease), each
-- developer-authored with its own role and reversibility. Durable jobs
-- + checkpoints support the 3 cron jobs (hourly_ci_signal_scan,
-- nightly_repo_reindex, weekly_stale_issue_sweep). Trace lineage is the
-- storage backend for the per-job JSONL trace files.
