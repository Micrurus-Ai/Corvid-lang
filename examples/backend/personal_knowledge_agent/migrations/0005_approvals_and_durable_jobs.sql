-- Approvals + durable jobs + trace lineage for the Personal Knowledge Agent.
-- Slice 35V2-P42-D-LR-app-maturity-PKA-1.
--
-- Approvals gate the 5 external-write surfaces D-PKA-2 introduces
-- (share-to-chat, share-via-email, publish-authoritative-answer,
-- export-tenant-corpus, cross-tenant-index-share). Durable jobs +
-- checkpoints support the 3 cron jobs (nightly_reindex,
-- weekly_feedback_batch, daily_provenance_audit). Trace lineage is
-- the storage backend for the per-job JSONL trace files.
