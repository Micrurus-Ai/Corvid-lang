...
- [ ] 44c-grammar-drift-gate-strengthening   `docs/reference/grammar.md:3-10` claims a drift gate cross-checks every production against the parser; the actual gate (`crates/corvid-syntax/tests/grammar_drift.rs:1-65`) checks only internal EBNF consistency. Two-part fix: (1) annotate unimplemented productions (`match_expr`, `map_literal`, `struct_literal`, `pattern`, sum-type variants, `type_alias_body`, `let`-form, unary `+`) with a machine-readable `# PLANNED` marker; (2) extend the gate so every non-planned production must be exercised by at least one parser test, and the gate's doc header honestly states what it checks. Acceptance: gate fails if a new production lands unmarked + untested.
