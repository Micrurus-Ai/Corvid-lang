...
- [ ] 47e-hardening-tests   **[+1 item filed 2026-07-14]** `host_bindings_integration`'s python smoke fails on this machine with EMPTY stdout/stderr from the python subprocess (the rust smoke against the SAME cdylib passes; fails identically on clean HEAD, so it predates Phase 47's slices) — diagnose the environment interaction (ctypes dlopen under the OneDrive path? python version drift?) and make the test either pass or skip LOUDLY with the reason instead of failing opaquely. Plus the original scope: recursive struct types (representation supports them — `Type::Struct(DefId)` nominal indirection — zero test coverage per audit; risk zone is codegen struct layout) + Int-overflow doc/impl alignment (book claims release-mode saturation + `--overflow` flag; impl always checks — `interp/expr.rs:144-149`; 44a fixes the doc, this slice adds the pinning tests) + numeric-conversion edge tests once 45e lands.
