...
- [ ] 33Q-trial-round-2-code-findings  **[33M-blocker — five code-class findings from anonymous-2026-06-04 round-2 trial report (`8e76563`)]** The same friends-and-family reviewer retested after the round-1 wrappers-and-onboarding fixes shipped, and surfaced five language-and-runtime-class findings that block reviewers #2-#10 from shipping the six-surface app for real. Triage at [`docs/external-trials/33m-trial-anonymous-2026-06-04.md`](docs/external-trials/33m-trial-anonymous-2026-06-04.md). Four DOCS dispositions (P3.c + P4 + P5 + Minor versioning note) closed in commit `9ac154a` against the build prompt; the five code-class slices below own the deeper fixes. Acceptance per the 33M closing criterion at L51: "feedback closes as code / docs / tests / explicit non-scope before the public cut."
  - [ ] 33Q1-serve-with-tools-lib   `corvid serve` has no tool-handler registration story today — Surface 3 (approval-gated dangerous tool over HTTP) is undemonstrable because approving the queued action returns 500 "no handler registered for tool `<name>`". Ship parity with the `build --target=cdylib` linkage for the interpreter `serve` path. Design pass needed: subprocess `tools.py` loader (Python-runtime-shaped) vs explicit `--with-tools-lib <path>` flag vs both. Acceptance: a `corvid new` app with a `tools.py` handler can be served via `corvid serve`, hit with an approval-gated POST, approved via `/__approvals/<id>/approve`, and observe the side effect run. Adversarial: missing handler still returns 500, but the approval is preserved (see 33Q2). Filed by anonymous-2026-06-04 P1.1.
  - [ ] 33Q2-approval-not-burned-on-failure   A 500 from the approval-gated handler currently consumes the approval anyway — a transient handler failure permanently burns a human approval. This is an approval-budget-integrity bug, not just UX. Fix: when approved-execution raises, leave the approval in `pending` (with a retry counter and the original invocation captured) OR move to a new `failed-retryable` terminal state that still records the human's authorization but doesn't permit re-execution without a new approval cycle. Acceptance: approve a queued action whose handler errors → 500 returned → `GET /__approvals` still lists the approval in a non-consumed state → calling `/__approvals/<id>/approve` again retries without re-prompting for a fresh human approval (or, if the design picks the terminal-failed-retryable shape, the new state is visible in the queue with the handler error captured). Adversarial: the failed-execution flow cannot be exploited to skip approval (e.g. by forcing a handler error to re-trigger approval-bypass logic). Filed by anonymous-2026-06-04 P1.2.
  - [ ] 33Q3-trust-guarantee-registration   `@trust(...)` annotations are incompatible with `corvid build --sign` because no `trust.*` guarantee id exists in `GUARANTEE_REGISTRY`. The trust moat and the signed-deploy path are mutually exclusive today. Fix: register a `trust.<dimension>` row in `corvid_guarantees::GUARANTEE_REGISTRY` (`RuntimeChecked`) for each `@trust` dimension currently in the type system, with positive + adversarial test refs. Update `validate_signed_claim_coverage` so `corvid build --sign` accepts `@trust` annotations on exported agents and surfaces them in `claim --explain` output. Acceptance: a cdylib whose entry agent declares `@trust(...)` builds with `--sign` and the resulting `claim --explain` enumerates the `trust.*` guarantee with the declared constraint. Test: `crates/corvid-cli/tests/sign_accepts_trust.rs` (positive + adversarial). Filed by anonymous-2026-06-04 P2.
  - [ ] 33Q4-dockerfile-presence-conditional-copy   The deploy Dockerfile renderer unconditionally `COPY src ./src`, `COPY corvid.toml ./corvid.toml`, `COPY migrations ./migrations`, `COPY evals ./evals`, `COPY traces ./traces`, and never `COPY`s `tools.py` — so a bare `corvid new my_app` followed by `corvid deploy package` produces a Dockerfile that fails `docker build` on the first missing directory. Fix: in `crates/corvid-cli/src/deploy_cmd.rs::render_dockerfile`, emit `COPY` lines only for paths that exist at render time; always emit `COPY tools.py ./tools.py` when `tools.py` is present in the app root. Acceptance: bare `corvid new my_app` → `corvid deploy package "$(pwd)" --out deploy/ --cdylib ...` → `docker build -f deploy/Dockerfile -t my_app:dev .` from the app root succeeds with no `mkdir` workaround. CI gate (`35V2-P33-deploy-dockerfile-builds`, already filed) is this slice's acceptance test: it runs `corvid new` → `deploy package` → `docker build` on a clean runner. Filed by anonymous-2026-06-04 P3.a.
  - [ ] 33Q5-dockerfile-version-default   The rendered Dockerfile's `ARG CORVID_VERSION=latest` resolves to the latest STABLE release (v0.1.0 today), which lacks the `serve` subcommand the image's `CMD` invokes — so the rendered image's entrypoint is a command its own binary doesn't have. Two fix shapes: (a) pin to the SHA of the binary the package was generated against — emit `ARG CORVID_VERSION=<sha>` with the literal SHA from `corvid --version`, so the image is reproducible AND the CLI surface matches what the package was rendered for; (b) emit `CORVID_VERSION=nightly` so the image resolves the most recent nightly which has every shipped command. Default to (a) since it's both reproducible and CLI-surface-stable. Acceptance: `docker build -f deploy/Dockerfile -t my_app:dev .` produces an image whose `corvid --version` matches the SHA at render time, and the `CMD` subcommand exists in that binary. Filed by anonymous-2026-06-04 P3.b.
