...
- [~] `35V2-P42-G0-reprobe`  Rebuild each reference app as a cdylib and file/fix remaining native-codegen gaps. **PKA builds end-to-end as a cdylib (2026-05-29)** — the full chain works on a real rich app (imported structs → registry tool dispatch → struct field access). One gap found + fixed here: a **module agent's local-struct field access** carried `Type::Struct(<module-local id>)` from `checked.types` while `ir.types` keys by the cross-module-remapped id, so `lower_expr`'s remap was extended to translate `Type::Struct` (via `remap_def_id`) in addition to `ImportedStruct` (`remap_struct_type`). Regression test `cli_build_cdylib_links_module_agent_field_access`. **Remaining:** the other four apps just need a scalar `pub extern "c"` entrypoint added (done as part of G-LR); re-probe each as its entrypoint lands. Track closes when all 5 build.
