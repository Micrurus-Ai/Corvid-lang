...
1. **`35V2-P42-E0-serve-5`** — HTTP approval queue: a `POST` to an approval-gated route creates a pending approval (the `approvals` flow) and returns `202` + approval id; a reviewer/queue executes it. Replaces the current `E0-serve-4` `403 approval_required` deny-by-default with the async-approval model. Real execution-model addition because Corvid's `approve` is synchronous. **Lives at line ~2848.** Dependency: serve-4 (closed). Size: medium.
2. **`35V2-P42-G0-tools-3b`** — Struct params/returns in `#[tool]`: restructure the macro so a struct-signature tool skips the typed wrapper and emits only the JSON wrapper + inventory submission. Unblocks receipt-returning tools (`ShareAnswerToChatReceipt`, …) in the apps' real surface. **Lives at line ~2872.** Dependency: G0-tools-3 (closed). Size: small-medium.
3. **`33J6-grammar-drift-gate`** — Drift-gate test that cross-checks `docs/reference/grammar.md` against the parser tests in `crates/corvid-syntax/src/parser/tests.rs`. **Lives at line ~1889.** Dependency: none. Size: small.
