...
- [ ] 45f-list-methods-non-lambda   **[subsumes the non-lambda half of 33R5d-collections]** `length`, `append`/`push`, `contains`, `first`/`last -> Option<T>`, `slice`, `reverse`, `sort` (Ord-able element types), `join(sep)` for `List<String>`, `range(start, end) -> List<Int>` builtin (unblocks counted iteration — no `while` exists until 45k). `map`/`filter`/`fold` wait for 45j lambdas.
