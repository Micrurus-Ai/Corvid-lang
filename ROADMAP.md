...
- [ ] 46h-structured-output-repair-and-judge-assertions   Two audit minors that share the "LLM output quality" concern: (1) optional auto-repair at the typed-decode boundary — when a prompt's structured output fails `json_to_value` decode (today a typed Marshal error, `interp/prompt/cost.rs:204-209`), re-ask the provider with the schema-violation feedback appended, bounded attempts, cost-accounted against the effect budget, trace-recorded for replay; opt-in per prompt (`repair: N` annotation or similar — pre-phase chat). (2) Semantic eval assertions — `eval` declarations today support only exact/typed trace assertions; add LLM-judge and similarity-threshold assertion forms so prompt-quality regressions are testable (`Judgement` in `std/agent.cor` becomes an executing surface instead of an inert envelope). Both respect the existing eval `--max-spend` accounting.
