...
- [ ] `35V2-P42-G0-tools-3`  **(before 2b)** `#[tool]` self-registration + struct support. Extend `ToolMetadata` with a JSON-dispatch fn pointer; have the macro emit a serde-marshalled JSON wrapper (`fn(args_json, len, user_data) -> result_json`, struct-capable — bypasses the scalar-only typed-wrapper limit at `lib.rs:210`) and submit it to inventory alongside the existing metadata. `corvid_runtime_init` registers every inventoried tool into the registry via `corvid_register_tool`. Additive: codegen still calls the typed `__corvid_tool_<name>` wrapper, so nothing breaks yet. Test: a `#[tool]` fn is invokable through `corvid_invoke_tool` after `corvid_runtime_init`.
- [ ] `35V2-P42-G0-tools-2b`  Codegen: the `IrCallKind::Tool` live branch always calls the matching `corvid_invoke_tool_<type>` (passing the already-computed trace payload) and stops declaring/calling the `__corvid_tool_<name>` import symbol — breaking the link wall. Struct returns decode the result JSON via the existing per-struct decoder; record/replay folds in. Add `corvid_register_tool`/`corvid_clear_tools` to the cdylib export list + abi keep-alive markers for the family. Existing native-tool tests stay green via G0-tools-3 self-registration. New host integration test: a Rust host loads a one-tool fixture cdylib, registers a tool, invokes the agent — and the cdylib LINKS with no unresolved `__corvid_tool_*`.
- [ ] `35V2-P42-G0-reprobe`  Rebuild each reference app as a cdylib (tools registered) and file any remaining native-codegen gap (e.g. `imported-struct-4` entry/prompt boundaries, or the field-access "metadata missing" case seen 2026-05-28) as a further `G0` slice. Close the track when all 5 build + link end-to-end.

- [ ] `35V2-P42-G-LR-per-app-claim-files`       `apps/<name>/CLAIM.md` from `corvid claim --explain` per app. **Blocked on the `G0` track** — each app must build + link as a signed cdylib first. Includes authoring each app's `#[tool]` implementations (mock receipts matching the app's mock posture) or a host harness that registers them.
