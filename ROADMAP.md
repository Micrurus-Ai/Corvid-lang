...
- [ ] 49c-mcp-add-and-typed-codegen   `corvid add mcp` (config write + connect + tool discovery listing) and typed-module generation from MCP tool schemas (envelope types, Result returns, effect rows; regeneration via `corvid mcp regen <name>`). Tests: generation golden against a mock MCP server, untrusted approval gating preserved through generated wrappers.
