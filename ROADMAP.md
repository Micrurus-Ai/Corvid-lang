...
- [ ] 47f-contract-module-disposition   Of the 15 `std/` modules, 4 execute (io/http/db/json), 2 get executing slices in this track (rag → 46g, ai → 46c), and 9 remain envelope-only contracts with zero tool declarations and zero runtime dispatch: `agent.cor`, `approvals.cor`, `auth.cor`, `cache.cor`, `effects.cor`, `jobs.cor`, `observe.cor`, `queue.cor`, `secrets.cor`. Several have REAL runtime counterparts shipped by Phases 38–40 (durable jobs, approval queues, auth, observability) that the envelopes don't reach. Per-module disposition audit, one decision each: (a) wire to the existing runtime surface as executing tools (the io/http/db/json dispatch pattern), (b) keep as a documented host-tool contract with an explicit "contract-only — bring your own host tool" header (the `std/db.cor:11-19` non-SQLite precedent), or (c) delete. Acceptance: no std module is silently envelope-only — every module's header states which of the three it is, and the stdlib README table carries an executing-vs-contract column.
