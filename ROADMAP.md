...
- [ ] 52f-authz-enforcement   Route-level enforcement of `requires authenticated|role|permission`: session + CSRF double-submit + role/permission checks run before the handler; the typed `actor` is populated from the verified session. Authenticated + DURABLE approval endpoints (replace the in-memory `/__approvals` with the durable approval queue).
  ...
  **DESIGN (2026-07-23, user chose "full role model now"):** the runtime's permission model was fingerprint-equality only, with no role→permission mapping or role-assignment surface. 52f builds the full model under LEAST PRIVILEGE — an open signup gets NO role unless the app declares a `default_role` (granting authority is never silent). Decomposed into always-green increments:
  ...
  - [ ] **52f-2 role storage + assignment + resolution** — actor role storage (`auth_actor_roles` table); assign the role at provisioning (invitation role name / declared `default_role`); resolve an actor's effective permissions as the union of its roles' permission sets; role/permission membership checks in `corvid-runtime`.
