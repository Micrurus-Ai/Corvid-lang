...
- [ ] 52b-contract-closure   The phase invariant, mechanically enforced: at startup, `corvid serve`/`corvid dev` construct an execution graph over the Application Contract and assert a runtime path exists for every public element (route shape, streaming return, upload, page, auth route, policy). A missing path is a startup error (e.g. `E5204 Contract not executable`, naming the element), never a runtime `501`. + a `contract.runtime_closure` guarantee row (RuntimeChecked). The developer's SOURCE is the forcing function: `public agent chat() -> Stream<Y>` forces the runtime to have SSE or refuse to start.
