...
- [ ] 51c-corvid-ai-metadata   The AI-native artifact: agent invocation shapes, typed STREAMING EVENT protocols (`started`/`chunk`/`tool_started`/`tool_completed`/`approval_required`/`completed`/`failed`), approval contracts, grounding + sources, confidence + below-min routing, review states, cost/latency reporting, tool-call progress, replay identifiers, behavioral contracts.
