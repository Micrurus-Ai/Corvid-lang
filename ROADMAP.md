...
- [ ] 44a-book-syntax-types-realign   Fix `docs/book/04-syntax.md` + `05-types.md`: remove or mark-planned `Map` literals/methods, `match`, enum variants, `fn` + generics, lambdas, `while`, type aliases, named struct literals + `..` update, string/list methods, `to_float`/`to_int_truncated`, overflow build-flag claims, `Unit` (impl name is `Nothing`), doc-comment `#:` claims, tool bodies calling `@host.x.y` (tools are signature-only — `crates/corvid-syntax/src/parser/decl/mod.rs:130-161`). Replace every `let x = …` in examples with the bare-assignment form that actually parses (`parser/stmt.rs:211-233`). Acceptance: every code block in ch 04/05 either compiles through the driver or sits under an explicit "Planned — not in v1.0" heading.
...
- [ ] 45i-match-expression   **[closes audit B1 — the largest single slice]** `match` expression + statement with literal / variant / struct-destructure / binding (`x @ pat`) / wildcard / guard (`if` clause) patterns and compiler-checked exhaustiveness over 45h sum types + `Option`/`Result`/`Bool`. Parser (`KwMatch` token), AST pattern nodes, checker (exhaustiveness + binding types), IR decision-tree lowering, interpreter, codegen or explicit degradation. Book ch 13 moves from Planned back to shipped, block-by-block compile-tested. This also closes audit B6's worst half: `Ok`/`Err`/`Some`/`None` become inspectable.
...
- [ ] 45p-generics-decision   Pre-phase chat only — user-declared generics need a type-variable representation the `Type` enum lacks entirely (`types.rs:20-145`) and would ripple through checker/IR/codegen. Decide: post-v1.0 (recommended — the special-cased `List`/`Option`/`Result` heads + 45c builtin methods cover the v1.0 workload) or scoped v1.0 subset. Record the decision in the book's Planned section either way.
