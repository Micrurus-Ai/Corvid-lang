...
- [ ] `35V2-P42-G0-tools-2b`  Codegen: the `IrCallKind::Tool` live branch calls the matching `corvid_invoke_tool_<type>` (passing the already-computed trace payload) instead of the `__corvid_tool_<name>` import symbol — breaking the link wall; struct returns decode the result JSON via the existing per-struct decoder; record/replay folds in. Add the `corvid_register_tool`/`corvid_clear_tools` cdylib export-list entries + abi keep-alive markers for the whole family. Host integration test: a Rust host loads a one-tool fixture cdylib, registers the tool, invokes the agent, asserts — and the cdylib LINKS with no unresolved `__corvid_tool_*`.
