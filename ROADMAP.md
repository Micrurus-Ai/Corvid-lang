...
- [ ] 50l-judged-output-guard   **[added 2026-07-15]**   Expose the eval harness's LLM-judge (`assert judged`) as a DECLARED runtime output guard on prompts (`guard judged("contains no PII")`): the judge call is itself a traced, budgeted, replay-substituted prompt; guard failure is a typed Err, never a silent pass. Covers the moderation/PII/groundedness-scoring category with machinery that already exists — semantic checks with the moat attached.
