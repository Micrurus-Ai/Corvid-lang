...
- [ ] 35V2-P38-C-replay-quarantine — Cross-layer replay-quarantine wiring for durable jobs. Promotes `jobs.replayable_side_effects` from OutOfScope to RuntimeChecked. Sub-slices:
  ...
  - [ ] 35V2-P38-C-1 — Job→Runtime executor bridge. `cmd_jobs_run` and `WorkerPool` thread a real `Runtime` (with `LlmRegistry`, `HttpClient`, `StoreManager`, `IoRuntime`) into the job executor closure instead of running a no-op. New `JobRuntimeExecutor` trait + default impl. Unit + integration tests cover one persisted job executing through the real Runtime stack.

  - [ ] 35V2-P38-C-2 — Job trace emission. `@replayable` jobs persist a JSONL trace alongside SQLite checkpoints. `QueueJob.replay_key` resolves to the trace path; the schema reuses `corvid-trace-schema` `RunStarted`/`ToolCall`/`LlmCall`/`ApprovalDecision`/`SeedRead`/`ClockRead`/`RunCompleted` events. Integration test: enqueue → run → assert trace file exists, contains the expected event sequence, and round-trips through `corvid-trace-schema` deserialization.

  - [ ] 35V2-P38-C-3 — `replay_job` entry point. New `replay_job(queue, job_id)` resolves the trace from `replay_key`, builds a `Runtime` in `RuntimeMode::Replay(ReplaySource::JobTrace(...))`, and drives the executor through the recorded events. Reuses existing `is_replay_mode()` / `replay_uses_live_llm()` predicates. CLI surface: `corvid jobs replay <job-id>`. Integration test: enqueue → run → replay → assert deterministic event-sequence match.

  - [ ] 35V2-P38-C-4 — LlmRegistry quarantine wrapper. New `QuarantinedLlmAdapter` wraps each registered adapter when `is_replay_mode() && !replay_uses_live_llm()`; any live provider call returns `QuarantineViolation` instead of hitting the network. Adversarial test: a job whose trace was rewritten to point at an unrecorded LlmCall must fail with `QuarantineViolation`, not leak the call.

  - [ ] 35V2-P38-C-5 — HTTP / Store / IO quarantine wrappers. Same pattern as C-4 applied to `HttpClient` (connector calls), `StoreManager` (DB writes), `IoRuntime` (file IO). Each surface gains a quarantined wrapper installed when the Runtime is in job-replay mode. Adversarial test per surface: a trace mutated to require an unrecorded side-effect must fail with the surface's quarantine error.
