...
- [ ] 47b-std-vendoring-hardening   **[closes audit M12]** (1) vendor no-op becomes a loud scaffold-time warning with remediation (`scaffold.rs:138-140` silently prints nothing when no std source found); (2) `corvid upgrade` gains a vendored-std refresh migration (currently rename-level only — `upgrade_cmd.rs:35-43`) so pre-33R5b projects can get `std/json.cor` without manual copying.
