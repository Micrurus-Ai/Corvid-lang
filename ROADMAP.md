...
- [ ] 45a-let-bindings-and-annotated-locals   Restore `let` as a keyword (currently demoted to plain ident — `corvid-syntax/src/lib.rs:120-133`) with optional type annotation `let n: Int = 42`; keep bare `x = expr` working. Parser + AST (`Stmt::Let.ty` is already `Option` — wire it) + checker annotation-vs-initializer agreement + book examples become real.
...
- [ ] 45i-match-expression   **[closes audit B1 — the largest single slice]** `match` expression + statement with literal / variant / struct-destructure / binding (`x @ pat`) / wildcard / guard (`if` clause) patterns and compiler-checked exhaustiveness over 45h sum types + `Option`/`Result`/`Bool`. Also the destructuring forms book ch 13:54-58 teaches: `let Decision { refund, amount, .. } = expr` destructuring-let (on the 45a let surface) and destructuring in `fn`/agent params. Parser (`KwMatch` token), AST pattern nodes, checker (exhaustiveness + binding types), IR decision-tree lowering, interpreter, codegen or explicit degradation. Book ch 13 moves from Planned back to shipped, block-by-block compile-tested. This also closes audit B6's worst half: `Ok`/`Err`/`Some`/`None` become inspectable.
