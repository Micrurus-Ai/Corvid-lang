...
- [ ] 46e-parallel-construct   **[closes audit B8]** The `parallel { a; b }` form the effect spec already sketches (`docs/internals/effect-spec/10-interactions.md:84-96`, "Tracked in Phase 22" — never shipped). Pre-phase chat mandatory: effect-row composition across concurrent arms, budget accounting, deterministic replay of concurrent traces (arm-indexed sub-traces), interaction with the RC memory model. The runtime is already tokio (`ensemble` voting proves concurrent dispatch works — `interp/prompt/voting.rs:103`); the language is the missing layer.
