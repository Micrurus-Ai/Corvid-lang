...
1. **`35V2-P42-G0-tools-3b`** — Struct params/returns in `#[tool]`: restructure the macro so a struct-signature tool skips the typed wrapper and emits only the JSON wrapper + inventory submission. Unblocks receipt-returning tools (`ShareAnswerToChatReceipt`, …) in the apps' real surface. **Lives at line ~2872.** Dependency: G0-tools-3 (closed). Size: small-medium.
2. **`33J6-grammar-drift-gate`** — Drift-gate test that cross-checks `docs/reference/grammar.md` against the parser tests in `crates/corvid-syntax/src/parser/tests.rs`. **Lives at line ~1889.** Dependency: none. Size: small.
3. **`serve-6` (filed during E0-serve-5 closure)** — transition endpoints for the HTTP approval queue: `POST /__approvals/<id>/approve` (a reviewer marks an approval granted, server re-executes the original request and returns the result) and `POST /__approvals/<id>/deny` (record the deny + audit event). Out-of-band reviewer authentication + the re-execution model (replay original args from the queue record or require client re-POST with `X-Corvid-Approval-Id`) are the design decisions to make. **Filed under Phase 42 E0 track.** Dependency: E0-serve-5 (closed). Size: medium.

(Previously slice #1 — `35V2-P42-E0-serve-5` HTTP approval queue — shipped in commit closing the create+poll path; transition endpoints split into the new `serve-6` follow-up rather than rolled into this slice to keep scope honest.)
...
- [ ] `35V2-P42-G0-tools-3b`  Struct params/returns in `#[tool]`. The JSON wrapper marshals via serde and is already struct-capable, but the macro's *typed*-wrapper arg loop calls `abi_type_for` which aborts on struct types (`lib.rs:210`) before the JSON wrapper is reached. Restructure so a struct-signature tool skips the typed wrapper (which can't represent a struct at the scalar C ABI) yet still emits the JSON wrapper + inventory submission. Needed for the apps' receipt-returning tools (`ShareAnswerToChatReceipt`, …). Macro-expand test with a `#[derive(Serialize, Deserialize)]` struct param/return.
