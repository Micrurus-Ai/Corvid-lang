...
- [ ] 45l-option-result-ergonomics   **[closes audit B6 remainder]** On the 45c table: `unwrap_or(default)`, `is_some`/`is_none`/`is_ok`/`is_err`, `ok_or(err)`, `map_err`. Audit the stdlib envelope types (`DbResult`, `HttpResponseEnvelope`, `FileReadEnvelope`) for whether any should become honest `Result` returns now that Result is consumable — separate follow-up slices per surface if yes.
