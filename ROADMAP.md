...
- [ ] 50j-value-refinements   **[added 2026-07-15 from the framework-gap review — the Pydantic gap]**   Field-level value refinements on struct fields (`age: Int where 0..150`, pattern/length constraints), checked at prompt-output decode and FED INTO the `with repair` loop — outputs heal until structurally AND semantically valid, and the repair attempts still count against `@budget`. Beats Pydantic/Instructor at their own game because it composes with the moat.
