...
- [ ] 51g-identity-surface   `identity app_users:` with `provider google|github|microsoft|apple|discord|slack` + generic `oidc` (discovery URL), session config (lifetime/cookie flags). Parsed into AST + checked + wired to the existing auth runtime. NO provider narrowing.
