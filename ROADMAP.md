...
- [ ] 45q-parser-checker-papercuts-and-leniency-hardening   The audit's small-but-real surface stragglers, one slice: (1) `elif` (or `else if` — pick one form, pre-phase chat) so chained conditions stop requiring nested indentation (`parser/stmt.rs:131-137` currently requires `else` + `:`); (1b) **[filed by 44c]** annotation names colliding with reserved keywords fail to parse — `@retry(...)` errors with "got KwRetry, expected an identifier" even though book ch 10 documents it (`parse` uses `expect_ident` for annotation names); allow keyword-named annotations or rename the annotation and fix ch 10; (1c) **[filed by 44f]** named annotation arguments (`@idempotency(key: expr)`, `@retry(max_attempts: 3)`) do not parse — annotation args are dimensional constraint values only; decide the named-arg surface alongside (1b); (1d) **[filed by 44f — defect]** declaring a `mock` alongside its target in the same compile fails typechecking with E0203 "cannot call a value of type mock target" at the mock declaration site — mocks are unusable end-to-end; fix the checker's mock-target resolution so the test runner can substitute them; (2) unary `+` (grammar.md allows it; parser handles only `-` — `parser/expr.rs:164-178`); (3) doc-comment `#:` token decision — the book claims doc comments render in `--help`/LSP hover but the lexer has no DocComment token; implement the token + LSP hover wiring or strike the claim in 44a and file post-v1.0; (4) typechecker leniency hardening: unknown generic-type heads silently resolve to `Type::Unknown` (`checker/types.rs:264-309`) — make them a resolution error with a did-you-mean (pairs with 33R9); audit the `Unknown`-assignable-to-everything rule (`types.rs:220`) for which sites should tighten now that 45a-45l give users real alternatives; add diagnostics (not errors) for the known inference limits (bare `None` infers `Option<Unknown>` — `checker/expr.rs:194`; list element type taken from first element — `checker/expr.rs:82-119`).
