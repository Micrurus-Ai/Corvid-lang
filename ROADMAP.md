...
- [ ] 46c-conversation-history-first-class   **[closes audit B7 second half]** Make `List<AiMessage>` (`std/ai.cor` types exist, nothing consumes them) passable into prompt calls as history; define the trace/replay representation; context-window management policy (truncation strategy + `context_window` in `model_catalog.rs` — today only an `estimate_tokens` heuristic at `llm_dispatch.rs:1072`). Design doc first: this is the multi-turn shape everything downstream (chat apps, agent loops) builds on.
