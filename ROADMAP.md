...
- [ ] `35V2-P42-G0-tools-1`  Runtime tool registry mirroring the approver store (`OnceLock<Mutex<HashMap<String, ToolFn>>>` in `corvid-runtime`): `corvid_register_tool` / `corvid_clear_tools` exports + an internal `corvid_invoke_tool(name, args) -> result` dispatch. Confirm + reuse `corvid_call_agent`'s arg/return marshalling. Unit test: register a tool fn, invoke by name, assert result. No codegen change.
- [ ] `35V2-P42-G0-tools-2`  Codegen: the `IrCallKind::Tool` live branch dispatches via `corvid_invoke_tool` (marshal args; unmarshal the result through the existing struct codec for struct returns) instead of the `__corvid_tool_<name>` import symbol — breaking the link wall. Folds in struct-returning-tool **record/replay** (record the result as JSON, replay decodes it). Host integration test: a Rust host loads a one-tool fixture cdylib, registers the tool, invokes the agent, asserts — and the cdylib LINKS with no unresolved `__corvid_tool_*`.
- [ ] `35V2-P42-G0-tools-3`  `#[tool]` macro (`corvid-macros`): support struct params/returns (currently scalar-only, `lib.rs:210`) via JSON marshalling, and emit a self-registration constructor so a linked `#[tool]` lib registers into the same registry at load (preserves `--with-tools-lib` under the unified model).
