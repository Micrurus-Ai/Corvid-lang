...
- [ ] 46d-real-provider-streaming   **[upgrades audit M6 from fake to real]** SSE token streaming in all four adapters (each has an explicit "deferred" marker today); wire into the existing `Stream<T>` machinery so a `-> Stream<T>` prompt yields real incremental chunks and the already-shipped mid-stream budget/confidence termination + `Partial<T>` + `ResumeToken<T>` algebra finally has real flow through it. Replay semantics for chunked responses (trace records chunk boundaries). 44e's README caveat comes off.
