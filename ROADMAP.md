...
1. **`33J6-grammar-drift-gate`** — Drift-gate test that cross-checks `docs/reference/grammar.md` against the parser tests in `crates/corvid-syntax/src/parser/tests.rs`. **Lives at line ~1889.** Dependency: none. Size: small.
2. **`serve-6` (filed during E0-serve-5 closure)** — transition endpoints for the HTTP approval queue: `POST /__approvals/<id>/approve` (a reviewer marks an approval granted, server re-executes the original request and returns the result) and `POST /__approvals/<id>/deny` (record the deny + audit event). Out-of-band reviewer authentication + the re-execution model (replay original args from the queue record or require client re-POST with `X-Corvid-Approval-Id`) are the design decisions to make. **Filed under Phase 42 E0 track.** Dependency: E0-serve-5 (closed). Size: medium.

(Previously slices #1 + #2 — `35V2-P42-E0-serve-5` HTTP approval queue and `35V2-P42-G0-tools-3b` struct params/returns in `#[tool]` — both shipped; their closure commits leave only the unfiled-but-named drift gate and the deferred transition follow-up in the genuinely-open queue.)
