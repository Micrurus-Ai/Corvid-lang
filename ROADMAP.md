...
- [ ] 50k-call-timeouts-and-breakers   **[added 2026-07-15]**   Runtime `timeout` annotation on tool/LLM calls (typed Err on expiry, traced + replayed) and a circuit-breaker policy (after N consecutive failures, refuse for a cooldown window with an honest Err naming the breaker). The unglamorous production pair every framework has and every outage post-mortem asks for.
