...
- [ ] 33S-executing-io-surfaces   **[parent — launch-material; HTTP + File + SQLite as effect-carrying executing primitives]** Promotes Corvid's HTTP, File, and SQLite stdlib surfaces from typed-envelope-only to executing primitives that flow through the effect system, replay/quarantine machinery, and signed-claim trust boundary. The runtime already executes HTTP and file I/O internally (`crates/corvid-runtime/src/http.rs::HttpClient::send` + `crates/corvid-runtime/src/io.rs::IoRuntime::read_text`; quarantine hooks exist on both); rusqlite is already a workspace dep used by `approval_queue/sqlite.rs`. Most of the work is exposing existing executing surfaces as callable Corvid primitives with proper effect rows + replay wiring + a real security model, plus a genuinely-new (but dependency-free) SQLite surface. Pre-phase chat (2026-06-08) locked design decisions captured below. Tracking shape: 5 sub-slices, each its own commit. **Execution order** (note interleaving with 33R5b): 33S0 → 33S1 → 33S2 → 33S3 → **33R5b (json batteries — gates 33S4's no-Python pipeline demo)** → 33S4. The 33R5b entry stays filed in the 33R market-readiness track; its work order is pulled forward here as the gating slice between 33S3 and 33S4.
  ...
  **Sub-slices:**
  ...
  - [ ] 33S0-foundation   **[shared; no user-facing surface yet]** Register the seven effect rows in `register_builtin_dimensions()` + the new `io_source` dimension. Update `docs/reference/core-semantics.md` so the registry table carries the new rows (kind / class / phase / description / test refs). Scaffold empty `ffi_bridge/io_exports.rs`, `http_exports.rs`, `db_exports.rs` (function stubs that error with "not implemented in 33S0" — they get filled in 33S1/S2/S3). Define the uniform quarantine contract the three surfaces obey (reuse `HttpClient::quarantine` / `IoRuntime::quarantine_writes`; specify the new SQLite quarantine mode for 33S3 to implement). Parse `[io] root` and `[http] allow` from `corvid.toml` (parsing only; enforcement lands in the per-surface slices). Update the claim-coverage allowlist so `corvid build --sign` accepts io_source-bearing effect rows. Acceptance: workspace compiles; effect ids resolve in the registry; `docs/reference/core-semantics.md` drift gate passes; no behavior exposed yet. Commit: `feat(runtime): foundation for executing io/http/db effect surfaces`.

  - [ ] 33S1-file-io-surface   **[invention — full contract]** Wire `io.read_text(path)`, `io.write_text(path, content)`, `io.list_dir(path)` through `crates/corvid-runtime/src/io.rs`. Each returns the typed envelope already declared in `std/io.cor` (FileReadEnvelope / FileWriteEnvelope / DirectoryEntryEnvelope). Enforce the `[io] root` confinement: reject path traversal (`..` escapes), reject absolute paths that escape the root, normalize via the existing `normalize_path` helper. Mark non-deterministic (checker rejects inside `@deterministic`). Record each call in the trace. Quarantine writes on replay (`IoRuntime::quarantine_writes` is the hook; reads pass through from the recorded trace). Invention proof in-slice: README catalog entry, `corvid tour --topic file-io` demo compiling through the normal driver, `docs/reference/stdlib/io.md` reference page, `docs/reference/inventions.md` proof-matrix row, spec link. Tests (four classes): functional (read/write/list), security (path-traversal rejection), determinism (`@deterministic` rejection diagnostic), replay-quarantine fixture in `crates/corvid-runtime/tests/replay_quarantine_corpus.rs`. Acceptance: a Corvid program reads + writes + lists through `corvid run` against a temp root; traversal attempt is rejected with the configured-root diagnostic; replay quarantines the write. Commit: `feat(std): executing file I/O surface (read/write/list)`.
