...
- [ ] 47c-codegen-parity-decision-and-loud-degradation   **[closes audit M10]** Pre-phase chat decision: batteries on compiled tiers are v1.0 or explicitly post-v1.0 (recommendation: post-v1.0 — the tier-picker auto-fallback per Phase 20m is the designed mitigation). Either way: (a) one canonical doc section stating the tier matrix; (b) codegen-py stops emitting `object`-typed hints + unregistered `tool_call(...)` stubs for stdlib tools (the Phase 20l "object-shaped degradation" recurrence) and fails loudly at transpile time instead; (c) `runtime/python`'s README states it serves the transpile tier only.
