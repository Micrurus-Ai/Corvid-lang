...
- [ ] 46a-sampling-parameters   `LlmRequest` (`corvid-runtime/src/llm/mod.rs:33-51`) gains `temperature` / `top_p` / `max_tokens`; surfaced in `model` declarations (dimension fields already parse — `parser/decl/model.rs`) and per-prompt overrides; wired through all four provider adapters; recorded in traces for replay fidelity. Smallest slice, largest credibility-per-line.
