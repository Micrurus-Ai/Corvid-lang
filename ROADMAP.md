...
Struct-returning-tool native replay:

- [ ] `35V2-P42-G0-tool-replay-struct-1`  Native replay of tool calls whose return type is a struct. Today `expr/mod.rs:386-417` wires the replay branch only for `Int` / `Bool` / `Float` / `String` / `Nothing` (+ `Grounded<scalar>`); struct returns hit "native replay for tool … is not implemented yet." Add a `replay_tool_call_struct` runtime entry + record/replay struct codec wiring (record = serialize to JSON via the existing per-struct encoder; replay = decode the recorded JSON to a struct handle via the existing decoder). Every reference app has dangerous tools returning receipt structs (`ShareAnswerToChatReceipt`, …), so this blocks all five.
- [ ] `35V2-P42-G0-reprobe`  After tool-replay-struct lands, rebuild each reference app as a cdylib and file the next native-codegen gap (if any) as a further `G0` slice. Close the track when all 5 apps build end-to-end.

- [ ] `35V2-P42-G-LR-per-app-claim-files`       `apps/<name>/CLAIM.md` from `corvid claim --explain` per app. **Blocked on the `G0` native-backend-codegen track** — each app must build as a signed cdylib first.
