...
- [ ] 52c-boundary-type-runtime   Execute the HTTP-boundary types the contract already advertises: SSE endpoint for `Stream<T>` agents (the typed event protocol), multipart parsing + accepted-MIME + max-size enforcement for `Upload<Format>`, cursor parsing + `{items, next_cursor, has_more}` envelopes for `Page<Item>`. Runtime representations for `Upload`/`Page` (interpreter tier).
