...
- [ ] 49a-skill-format-and-audit   The core invention: `skill.toml` manifest format (name/version/description/exports/capability label/required config); `corvid add skill ./path` vendors into `src/skills/<name>/`, renders the capability-label audit, and requires consent (`--yes` for CI); compile-time ceiling verification — the checker composes the skill's actual effects and rejects any excess over the declared label, naming the dimension; loud UNSIGNED banner scaffolding (signature verification itself is 49b). Tests: manifest parse/validate, vendor layout, ceiling-exceeded rejection naming the dimension, edited-skill re-audit.
