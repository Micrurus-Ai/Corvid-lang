...
- [ ] 50e-declarative-model-routing   Typed routing policy on prompts/effects: try the cheap model first, escalate on low confidence or error, hard-capped by the composed budget. Composes the existing confidence dimension + multi-provider adapters + @budget — inexpressible in any other language.
