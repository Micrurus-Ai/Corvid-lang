...
- [ ] 44b-book-pattern-matching-and-prompts-realign   `docs/book/13-pattern-matching.md` is entirely unimplemented — retitle to a Planned-feature design doc or fold into a roadmap appendix; `docs/book/11-prompts.md:46-55` documents multi-message `system:`/`user:` prompt blocks the parser lacks (no role blocks in `parser/prompt.rs`) — mark planned (implemented by 46b). Acceptance: same compile-or-planned rule as 44a.
