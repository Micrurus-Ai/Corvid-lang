...
- [ ] 50a-structured-output-self-heal   Schema-violating LLM output triggers ONE constrained corrective re-ask (the violation + expected schema fed back), then a typed Err — no hand-written JSON-retry loops. Rides the typed-decoder convention; replay-deterministic (the corrective exchange records as ordinary events).
