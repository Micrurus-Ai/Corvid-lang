...
- [ ] 50f-token-context-budgets   The token sibling of @budget: per-prompt context accounting with a TYPED overflow policy (truncate/summarize/refuse) instead of mystery context-window errors.
- [ ] 50g-latency-budgets   The latency dimension already rides the effect system; make "worst path ≤ Ns" a static check like cost budgets, with the same path-naming diagnostics.
