...
- [ ] 47h-stdlib-result-envelopes   **[filed by the 45l envelope audit, 2026-07-11]** `io_read_text` / `io_write_text` / `io_list_dir`, `http_get` / `http_post_json`, and `db_open` / `db_query` / `db_execute` return bare envelopes and TRAP on failure (file missing, network error, SQL error). Now that `Result` is fully consumable, migrate each to an honest `Result<Envelope, TypedError>` return: shared runtime-dispatch error-marshalling mechanics first, then one surface at a time (io, http, db) with book/guide updates. Typed error enums (45h/45i) are the error side; `map_err`/`unwrap_or` (45l) are the consumption story.
