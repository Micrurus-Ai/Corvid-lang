...
- [ ] 45e-number-string-conversions   `Int.to_string()`, `Float.to_string()`, `Float.to_int_truncated()`, `Int.to_float()` (explicit; implicit widening stays — `types.rs:221`), `String.parse_int() -> Result<Int, String>`, `String.parse_float()`. Kills the `"count: " + n` type-error papercut. General string-interpolation syntax is a separate pre-phase-chat decision (prompt templates already interpolate `{param}` — `interp/prompt/mod.rs:324-334` — reusing that syntax in ordinary strings is the natural candidate).
