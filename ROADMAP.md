...
- [ ] 51r-contract-sweep   Invention contract: book chapter + guides for the application contract and identity, tour topics, inventions.md rows, README catalog, guarantee rows (auth safe-defaults + contract fidelity), regenerated core-semantics, and the "define once, get everything" launch-story polish. `corvid deploy` embeds the contract + optionally serves `/.well-known/corvid` and `/openapi.json`.
