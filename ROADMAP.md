...
- [ ] 45h-user-sum-types   **[closes audit B2]** `type Status: | Pending | Approved(approver: String)` per the grammar.md:68 production that already documents it. Declaration (`parse_field` gains the `|` branch — `type_field.rs:56-68`), `Type`-level variant representation, construction, `==`, exhaustiveness metadata for 45i. The AST comment "Enum/union types arrive in v0.2" (`corvid-ast/src/decl.rs:324`) finally comes due.
