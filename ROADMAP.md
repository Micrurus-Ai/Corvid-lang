...
- [ ] 33R-market-readiness-track   **[launch-blocker — OSS adoption gap remediation]** 14-slice track derived from the 2026-06-08 market-readiness audit (`docs/market-readiness-audit.md`). The audit's verdict: Corvid is engineered far past `0.0.1`, but adoption is blocked by last-mile surfacing — no LICENSE on disk, no package registry, README is invention-catalog-shaped instead of an adoption funnel, stdlib is contracts-only, VS Code extension and crates.io unpublished. None of these are missing-capability problems; they're packaging + hosting + docs. Track runs in tiered order P0 → P3; lower tiers do not start until higher tiers close. Canonical decisions captured at filing time: repo URL = `github.com/Micrurus-Ai/Corvid-lang`; canonical domain = `corvid-lang.org` (to be served from the existing `web/` Cloudflare Worker); copyright holder = `Disan Ssebowa Basalidde`; license = MIT (narrowed from the prior `MIT OR Apache-2.0` claim, which never had on-disk text to back it); registry shape = GitHub Releases for artifacts + static `index.json` from the same Worker.
  ...
  - [ ] 33R5-stdlib-batteries   **[P1 — invention; gated sub-slices]** Promote the stdlib from contracts to batteries so a beginner tutorial requires zero user-authored Python. Each module ships with the full invention contract: README catalog mention, `corvid tour` topic, `docs/reference/stdlib/<module>.md`, spec link, tests.
    ...
    - [x] 33R5b-json   **[closed 2026-06-10 via a/b/c sub-slices]** **[CTO call 2026-06-09 — Option C scope: ship BOTH opaque JsonValue/JsonBuilder primitives AND typed-decoder convention. Three sub-slices.]** Per the "no Python required" load-bearing promise of 33R5 batteries, JSON must cover both typed-language idioms (declare a struct, decode into it — the Corvid-native pattern) AND dynamic JSON (LLM-emitted JSON of unknown shape, debug tooling). Shipping only one is the textbook "softening the spec" shortcut. JSON is stateless (no Arc lifecycle, no policy, no security model beyond serde validation), so the plumbing slice is meaningfully tighter than 33S3a+b.
      - [x] 33R5b-a-json-primitives-and-executing-tools   **[closed 2026-06-09]**   Introduce `Type::JsonValue` and `Type::JsonBuilder` as opaque primitive types in `corvid-types` + `BuiltIn::JsonValue` / `BuiltIn::JsonBuilder` in `corvid-resolve`. Add `Value::JsonValue(Arc<serde_json::Value>)` and `Value::JsonBuilder(Arc<Mutex<serde_json::Map<String, serde_json::Value>>>)` in `corvid-vm`. The JsonValue Arc IS the data (no registry/slotmap needed unlike `DbHandle`); the JsonBuilder Arc-of-Mutex enables fluent mutation across multiple references. Wire through all Value/Type match-arm sites (Clone, type_name, PartialEq, display, conv::value_to_json + json_to_value, codegen-cl/py mangle/native_value/cl_type_for, native_ability tier-picker, prompt-format, abi). Codegen backends emit `not_supported("interpreter-only in 33R5b; future slice lands C-ABI codegen")` for both types — corvid-runtime's existing C-ABI `corvid_json_parse` / `corvid_json_get_field_*` / `corvid_json_object_*` exports already exist; the cdylib bridging is a follow-up. **No opacity gate** is needed at `json_to_value` — JsonValue's payload IS the JSON shape; converting JSON → Value::JsonValue is the natural path. Add executing tool declarations to new `std/json.cor` with inline effect declarations (`json_egress_read` reversible, `json_egress_build` reversible-ish): `json_parse(text) -> Result<JsonValue, String>`, typed accessors `json_get_int / json_get_float / json_get_string / json_get_bool / json_get_object / json_get_array(value, field) -> Result<T, String>`, builder side `json_object_new() -> JsonBuilder`, `json_object_set_int / _set_float / _set_string / _set_bool(builder, key, value) -> JsonBuilder` (fluent — mutates inner Arc and returns the same builder), `json_object_finish(builder) -> String` (snapshots current state and serializes; builder remains usable). Add `JsonRuntime` to `corvid-runtime/src/json.rs` (likely just a marker struct — JSON state lives in Arc payloads not in a runtime registry) + dispatch methods on `Runtime`. Interpreter dispatch in `corvid-vm/src/interp.rs::dispatch_stdlib_json_tool` gated by `is_stdlib_json_tool` (mirrors the io/http/db pattern). Plumbing unit tests: Value::JsonValue Arc clone semantics; PartialEq is structural (two JSONs with same shape are equal); type_name / display; `json_parse("{\"a\": 42}")` round-trip via runtime; `json_get_int(parsed, "a")` returns 42; `json_get_int(parsed, "missing")` returns Err; `json_get_string` on Int field returns Err; builder set + finish round-trip. Commit: `feat(vm): opaque JsonValue + JsonBuilder primitives + executing JSON tools (std/json.cor)`.
      - [x] 33R5b-b-typed-decoder-convention-and-end-to-end   **[closed 2026-06-10]**   Wire typed-decoder dispatch: when an interpreter tool call has return type `Result<T, String>` where T is a user-declared struct, the runtime's stdlib decode dispatch routes through `serde_json::from_str` + `json_to_value(parsed, &T_type, &types_by_id)`. The pattern: user declares `tool decode_user_from_json(text: String) -> Result<User, String>` and the runtime knows to dispatch any `decode_*_from_json` tool name via the generic handler. End-to-end driver tests in new `crates/corvid-driver/tests/executing_json_through_driver.rs`: (1) opaque-path round trip — real Corvid program parses JSON via `json_parse`, accesses fields via `json_get_int` / `json_get_string`, builds a new JSON via `json_object_new + _set_* + _finish`; (2) typed-decoder round trip — real Corvid program declares a User type, calls `decode_user_from_json(text)`, returns `user.id` (proves the per-type decoder convention works end-to-end through the driver pipeline); (3) malformed JSON returns `Err(message)` cleanly (no panic, no escape — pins the `json.parse_safety_no_panic` property). Replay-quarantine fixture: JSON parse during replay is read-only (substitutes from trace or diverges); builder side is also process-internal so no quarantine needed. Loader tests: no separate config required (JSON has no security boundary beyond serde validation; differs from io/http/db which all have `[io] root` / `[http] allow` / `[io] root`-reuse boundaries). Commit: `test(std): end-to-end and typed-decoder acceptance for executing JSON surface`.
      - [x] 33R5b-c-json-invention-proof   **[closed 2026-06-10]**   Two RuntimeChecked guarantees: `json.parse_safety_no_panic` (malformed input returns `Err(message)`; never panics; no escape) and `json.field_type_safety_at_access_boundary` (`json_get_int` against a string field returns `Err`, not a coerced value or a panic — the structural property that JSON field-type access is safe). Two `GUARANTEE_ID_*` anchors in `corvid-runtime/src/json.rs`. Two ids added to `SIGNED_CDYLIB_CLAIM_GUARANTEE_IDS`. The `@deterministic`-rejection property is NOT a separate row (covered by the existing decl-replayability rule, same rationale as 33S1c/33S2c/33S3d); add two pinning tests `deterministic_agent_calling_json_parse_tool_is_rejected` + `deterministic_agent_calling_json_object_finish_tool_is_rejected`. `docs/reference/stdlib/json.md` reference page (~250 lines) parallel to io.md / http.md / db.md: covers both shapes (opaque JsonValue with typed accessors AND the typed-decoder convention with per-type tool declarations), worked LLM-response example (parse polymorphic JSON via opaque path) AND worked typed-API example (decode_user_from_json). `docs/reference/stdlib/README.md` § std.json section. `docs/reference/inventions.md` proof-matrix row. README invention-catalog entry. `corvid tour --topic json` topic (offline — pure parse + build, no network). Commit: `docs(invention): JSON surface proof matrix, tour, and reference doc`.
    - [x] 33R5c-strings   **[closed 2026-07-10 via 45d-string-methods]** **[subsumed by 45d-string-methods — requires the 45c builtin-method-dispatch machinery; see the Language completeness track]**
    - [ ] 33R5d-collections   **[subsumed by 45f-list-methods-non-lambda + 45g-map-type + the 45j lambda-dependent tail; see the Language completeness track]**
    - [ ] 33R5e-datetime   **[subsumed by 45m-datetime-and-math-builtins]**
    - [ ] 33R5f-math   **[subsumed by 45m-datetime-and-math-builtins]**
    - [x] 33R5g-executing-io-primitive   **[closed retroactively 2026-07-09 — the 33S umbrella over-delivered this slice: executing file-I/O (33S1), HTTP client (33S2), SQLite (33S3), and JSON (33R5b) all wire through the runtime boundary with effect/trust dimensions, structural security boundaries, and replay quarantine. See the 33S track + 33S4 batteries-quickstart.]**
  ...
  - [ ] 33R12-quickstart-error-code-drift   **[P2] [subsumed by 44d-quickstart-honesty — which also fixes the quickstart's unparseable tool-body example and adds compile-guards for every quickstart snippet; see the Language completeness track]** Correct `docs/book/02-quickstart.md` (and any other doc that hardcodes error codes) to match `crates/corvid-driver/src/render.rs::detect_error_code` mappings. Specifically: the dangerous-call-without-approve example currently shows `E0301`; the compiler emits `E0101`. Grep all docs, fix every code; doc-only.
...
- [ ] 33S-executing-io-surfaces   **[parent — launch-material; HTTP + File + SQLite as effect-carrying executing primitives]** Promotes Corvid's HTTP, File, and SQLite stdlib surfaces from typed-envelope-only to executing primitives that flow through the effect system, replay/quarantine machinery, and signed-claim trust boundary. The runtime already executes HTTP and file I/O internally (`crates/corvid-runtime/src/http.rs::HttpClient::send` + `crates/corvid-runtime/src/io.rs::IoRuntime::read_text`; quarantine hooks exist on both); rusqlite is already a workspace dep used by `approval_queue/sqlite.rs`. Most of the work is exposing existing executing surfaces as callable Corvid primitives with proper effect rows + replay wiring + a real security model, plus a genuinely-new (but dependency-free) SQLite surface. Pre-phase chat (2026-06-08) locked design decisions captured below. Tracking shape: 5 sub-slices, each its own commit. **Execution order** (note interleaving with 33R5b): 33S0 → 33S1 → 33S2 → 33S3 → **33R5b (json batteries — gates 33S4's no-Python pipeline demo)** → 33S4. The 33R5b entry stays filed in the 33R market-readiness track; its work order is pulled forward here as the gating slice between 33S3 and 33S4.
  ...
  **Sub-slices:**
  ...
  - [x] 33S1-file-io-surface   **[invention umbrella closed 2026-06-08; landed across 3 single-concern sub-slices: 33S1a + 33S1b + 33S1c]** The executing file-I/O surface now ships end-to-end: tool decls in `std/io.cor` declared `read_text` / `write_text` / `list_dir` with `uses io_read/write/list` (33S1a); runtime dispatch interception, `IoToolPolicy` security struct, CLI/driver loader, and 17 acceptance tests across 4 surfaces all live (33S1a + 33S1b); the invention-proof contract is complete (33S1c) — 3 RuntimeChecked guarantees registered in `corvid-guarantees` with real test refs, anchors at the enforcement sites in `corvid-runtime/src/io.rs`, claim-coverage updated (`corvid build --sign` accepts io_source ids), `docs/reference/core-semantics.md` regenerated, `corvid tour --topic file-io` topic added (compiles through the normal driver), `docs/reference/stdlib/io.md` reference page written, `docs/reference/inventions.md` proof-matrix row added, README invention-catalog entry added. The full umbrella acceptance test: a Corvid program reads + writes + lists through `Runtime::call_tool` against a temp root with `[io] root` set; traversal rejection emits the diagnostic naming offending path + root; replay-mode dispatch path doesn't escape to the filesystem; `corvid build --sign` accepts cdylibs that use these tools. Sub-slices below.
    ...
    - [x] 33S1c-invention-proof-artifacts   **[closed 2026-06-08]** Three RuntimeChecked guarantees added to `corvid-guarantees::registry::GUARANTEE_REGISTRY`: `io_source.fs_path_confinement`, `io_source.fs_write_quarantine_on_replay`, `io_source.fs_read_quarantine_on_replay`. Each carries 1–5 positive + 1–5 adversarial test refs pointing at the 33S1a + 33S1b tests. Three matching `pub const GUARANTEE_ID_*` anchors at the enforcement sites in `crates/corvid-runtime/src/io.rs` so the `every_enforced_guarantee_id_is_wired_to_workspace_source` sentinel passes. Claim-coverage: the three ids added to `corvid-guarantees::signed_claim::SIGNED_CDYLIB_CLAIM_GUARANTEE_IDS` so `corvid build --sign` accepts a cdylib whose source uses these tools. `docs/reference/core-semantics.md` regenerated via `corvid contract regen-doc` (drift gate passes). New `docs/reference/stdlib/io.md` reference page (~140 lines) documenting the 3 tools, the `[io] root` security model + `CORVID_IO_ROOT` env override, the 3 guarantees, the @deterministic-rejection property, the replay-quarantine layers, and a worked file-backed-daily-summary example. `docs/reference/stdlib/README.md`'s `## std.io` section updated to mention the new tools + link to `io.md`. `docs/reference/inventions.md` proof-matrix row added. README invention-catalog entry added under Verification with the standard Spec/Tour/Roadmap/Proof/Non-scope footer. `corvid tour --topic file-io` topic added to `crates/corvid-tour-catalog/src/lib.rs`; source compiles through the `corvid_driver::compile` gate (`all_tour_sources_compile` test passes 33/33). Validation gate: 28 guarantee tests pass; 33 tour topics compile; workspace check clean; corpus verify exits 1 only on the two deliberate fixtures.

  - [x] 33S2-http-client-surface   **[closed 2026-06-09 via a/b/c sub-slices]** Wire `http.get(request)` and `http.post_json(request)` through `HttpClient::send`, returning the existing `HttpResponseEnvelope`. Enforce SSRF default-deny (block private RFC1918 / loopback / link-local addresses regardless of allowlist) — load-bearing security floor. Enforce `[http] allow` allowlist on top — every reachable host must be in the project's declared list. Cap response body size + honor envelope timeout + retry policy. Mark POST side-effecting + non-deterministic; mark GET reversible + non-deterministic. Record in trace. Quarantine on replay (`HttpClient::quarantine` is the hook). Invention proof: README entry, `corvid tour --topic http-client` (loopback test server in the tour; real endpoint optional), `docs/reference/stdlib/http.md`, inventions.md row, spec link. Tests (five classes): functional GET, functional POST, blocked-private-IP rejection, allowlist-pass success, timeout enforcement, replay-quarantine fixture. Acceptance: a Corvid program performs a real GET + a real POST through `corvid run` against a configured allowlist; a request to a private IP is blocked regardless of allowlist; replay quarantines the POST. Commit: `feat(std): executing HTTP client surface (get/post_json)`.
    - [x] 33S2a-http-tool-plumbing   **[closed 2026-06-08]** Renamed `std/http.cor`'s envelope-builder agents from `http_get` / `http_post_json` to `http_request_get` / `http_request_post_json` so the unprefixed names host the executing tools. Declared the two executing `tool` rows in `std/http.cor` with inline `effect http_egress_get` (reversible) / `effect http_egress_post` (side-effecting) declarations — effect names differ from tool names so the resolver namespace doesn't collide (tools and effects share the resolver namespace; same trap as 33S1's `io_*` rename). Added `HttpEgressPolicy` in `corvid-runtime/src/http.rs` enforcing always-on SSRF block (RFC1918 + loopback + link-local + IPv6 equivalents including bracketed `[::1]:port` form + `localhost` DNS alias) and required `[http] allow` allowlist with `is_configured()` / `allow_list()` / `check()` API. Threaded `http_policy` through `Runtime` + `RuntimeBuilder::http_policy(...)`. Refactored dispatch interception from prefix-matching (which would silently steal user tool names starting with `io_` or `http_`) to exact-name matching via `is_stdlib_io_tool` / `is_stdlib_http_tool` helpers — closes a latent 33S1 bug surfaced during 33S2a planning. Wired executing dispatch: `http_get` / `http_post_json` call `policy.check(url)` then `HttpClient::send`, marshal `HttpResponseEnvelope`. Also fixed a latent 33S1 bug: `std/io.cor` and `std/http.cor` now declare their effects inline because the resolver doesn't consult the `EffectRegistry` — registry-only registration left the standalone-compile tests failing throughout 33S1 (only `cargo check` ran in 33S1's gate, which compiles but doesn't fail-on-test-failure). 9 plumbing tests + 45 stdlib tests + workspace check + corpus verify gate all green. Commit: `feat(runtime): tool declarations + policy plumbing for executing HTTP-client surface`.
    - [x] 33S2b-http-cli-loader-and-end-to-end   **[closed 2026-06-09]** Shipped CLI loader + end-to-end acceptance + replay-quarantine fixture. (1) **CLI loader** at `crates/corvid-driver/src/run.rs::load_http_egress_policy`: reads `CORVID_HTTP_ALLOW` env override first (comma-separated host list, whitespace trimmed, empty entries stripped, whitespace-only falls back); falls back to `corvid.toml`'s `[http] allow = [...]`; falls back to `HttpEgressPolicy::unset()` for the fail-closed default. Installed via `RuntimeBuilder::http_policy(...)` in the `run_via_interpreter_tier` path so live `corvid run` invocations gate executing HTTP through the policy. (2) **`corvid new` scaffold** now writes BOTH `[io] root = "."` (33S1b had promised this and missed it; closed here) AND `[http] allow = []` (fail-closed-by-default) — the scaffolded corvid.toml now makes the executing-I/O security boundaries explicit on day one. (3) **Test injection** via `HttpClient::with_reqwest_client(reqwest::Client)` + `RuntimeBuilder::http_client(HttpClient)`: end-to-end tests build a reqwest client with `.resolve("api.example.com", loopback_wiremock_addr)` so a publicly-named URL routes at the TCP layer to a loopback `wiremock::MockServer`. SSRF policy stays GENUINELY UNCONDITIONAL in production code — no test-only carve-outs, no flag, no env knob; the URL string carries a public-looking host (passes SSRF), reqwest's DNS override silently points the connection at loopback. (4) **3 driver-layer end-to-end tests** in new `crates/corvid-driver/tests/executing_http_through_driver.rs`: real corvid program performs GET through executing dispatch; structural SSRF block rejects loopback even when allowlist contains it (proves SSRF is the floor); missing-allowlist fails closed with diagnostic naming `[http] allow` + `CORVID_HTTP_ALLOW` env + the fail-closed contract. (5) **5 loader tests** in `crates/corvid-driver/src/run.rs::http_policy_loader_tests`: configured corvid.toml produces configured policy; empty `allow = []` produces unconfigured policy (same fail-closed shape as missing section, intentional UX); absent `[http]` section produces unconfigured policy; `CORVID_HTTP_ALLOW` env beats corvid.toml; whitespace-only env falls back to corvid.toml. (6) **1 scaffold-content assertion** in `crates/corvid-driver/src/tests.rs::scaffold_corvid_toml_declares_io_and_http_security_boundaries`: proves `corvid new` writes both `[io] root = "."` and `[http] allow = []` and the result parses through `corvid_types::CorvidConfig`. (7) **2 replay-quarantine fixtures** in `crates/corvid-runtime/tests/replay_quarantine_corpus.rs`: `replay_blocks_executing_http_post_tool_dispatch_from_escaping_to_network` (POST through dispatch refuses with ReplayDivergence OR `QuarantineViolation { surface: "http" }`, even with a fully-configured allowlist) + `replay_blocks_executing_http_get_tool_dispatch_without_recorded_event` (GET also gated by replay substitution). Total: **11 new tests** across 4 surfaces; all pass with workspace check clean and `corvid verify --corpus tests/corpus` exits 1 only on the two deliberate fixtures. Guarantees + tour topic + reference doc + inventions row + README catalog land in 33S2c. Commit: `test(std): end-to-end and replay-quarantine acceptance for executing HTTP-client surface`.
    - [x] 33S2c-http-invention-proof   **[closed 2026-06-09]** Three RuntimeChecked guarantees added to `corvid-guarantees::registry::GUARANTEE_REGISTRY`: `io_source.http_ssrf_structural_block`, `io_source.http_allowlist_enforcement`, `io_source.http_quarantine_on_replay`. Each carries 1–6 positive + 1–6 adversarial test refs pointing at the 33S2a + 33S2b tests. Three matching `pub const GUARANTEE_ID_*` anchors at the enforcement sites in `crates/corvid-runtime/src/http.rs` so the `every_enforced_guarantee_id_is_wired_to_workspace_source` sentinel passes. Claim-coverage: the three ids added to `corvid-guarantees::signed_claim::SIGNED_CDYLIB_CLAIM_GUARANTEE_IDS` so `corvid build --sign` accepts a cdylib whose source uses these tools. The @deterministic-rejection property is NOT a separate guarantee row (same rationale as 33S1c's io.* surface — covered by the existing decl-replayability rule that rejects all tool calls inside `@deterministic` bodies); 33S2c adds two pinning tests at `crates/corvid-types/src/tests.rs::deterministic_agent_calling_http_get_tool_is_rejected` + `deterministic_agent_calling_http_post_json_tool_is_rejected` so the rule can't quietly relax. `docs/reference/core-semantics.md` regenerated via `corvid contract regen-doc` (drift gate passes). New `docs/reference/stdlib/http.md` reference page (~210 lines) documenting the 2 tools, the dual-layer security model (always-on SSRF block + required `[http] allow` allowlist), the `CORVID_HTTP_ALLOW` env override, the 3 guarantees, the @deterministic-rejection property, the replay-quarantine layers, and a worked webhook-fan-out example showing allowlist scoping. `docs/reference/stdlib/README.md`'s `## std.http` section rewritten to highlight the executing tools, security model summary, and link to `http.md`. `docs/reference/inventions.md` proof-matrix row added. README invention-catalog entry added under Verification (immediately after the file-I/O entry) with the standard Spec/Tour/Roadmap/Proof/Non-scope footer. `corvid tour --topic http-client` topic added to `crates/corvid-tour-catalog/src/lib.rs`; source compiles through the `corvid_driver::compile` gate (`all_tour_sources_compile` test passes 34/34). Validation gate: 28 guarantee tests pass; 34 tour topics compile; 45 stdlib tests pass; workspace check clean; corpus verify exits 1 only on the two deliberate fixtures. Commit: `docs(invention): HTTP-client surface proof matrix, tour, and reference doc`.

  - [x] 33S3-sqlite-surface   **[closed 2026-06-09 via a/b/c/d sub-slices]** Add `db_open(path)`, `db_query(db, sql, params)`, `db_execute(db, sql, params)` via a new `ffi_bridge/db_exports.rs` over rusqlite. Introduce `Value::DbHandle` (opaque, refcounted) in `corvid-vm` for `db_open` to return — threaded through `query` / `execute`. Parameter-bound only: the API takes `(sql: String, params: List<DbParam>)` and binds via `rusqlite::params` internally — no string interpolation. Support `:memory:`. Mark `execute` side-effecting + non-deterministic; mark `query` reversible + non-deterministic. Record in trace. Add the new SQLite quarantine mode — reads substitute from the recorded trace; writes refuse and raise QuarantineViolation. Invention proof: README entry, `corvid tour --topic sqlite`, `docs/reference/stdlib/db.md` (replacing the "envelopes only" framing for the SQLite path), inventions.md row, spec link. Tests (four classes): functional open + create-table + parameterized-insert + query round-trip, parameter-binding injection-attempt proof (sql with `'; DROP TABLE` in params stays escaped), determinism (`@deterministic` rejection), replay-quarantine fixture. Acceptance: a Corvid program opens an SQLite DB, creates a table, inserts parameterized rows, queries them back through `corvid run`; replay quarantines the writes; the parameter-binding test demonstrates an injection-shaped param is bound, not interpolated. Commit: `feat(std): executing SQLite surface (open/query/execute)`.
    - [x] 33S3a-value-and-type-plumbing   **[closed 2026-06-09]** Introduced `Value::DbHandle(Arc<DbHandleInner>)` in `corvid-vm/src/value/mod.rs` with `DbHandleInner` carrying `handle_id` + `path` for diagnostics (the runtime-callback closer that completes the lifecycle story lands in 33S3b). Added `Type::DbHandle` opaque primitive in `corvid-types` + `BuiltIn::DbHandle` in `corvid-resolve` so user code can name the type in agent signatures. Wired the name through 5 type-mapping sites: `named_type_to_type`, `named_type_in_module`, `type_ref_to_type_readonly`, IR `lower::type_ref_to_type`, abi `emit::resolve_typeref_to_type`. Updated 11 Value/Type match-arm sites across `corvid-vm` (Clone, type_name, PartialEq, display/Display, repl_display, conv::value_to_json + json_to_value + type_label), `corvid-codegen-cl` (mangle_type_name, is_native_value_type, cl_type_for, check_entry_boundary_type), `corvid-codegen-py` (python_type_hint_of), `corvid-driver` (native_ability::is_native_value_type), `corvid-prompt-format` (schema_for_inner), `corvid-abi` (type_description, emit). Backend codegen-cl emits a structured `not_supported(..."executing SQLite surface is interpreter-only in 33S3; a future slice lands C-ABI opaque-pointer codegen...")` diagnostic — clean failure path for cdylib builds, no silent corruption. JSON marshalling carve-out: `value_to_json` emits a `{"tag": "db_handle_opaque_sentinel", ...}` sentinel purely for trace-debug visibility; `json_to_value` REFUSES to mint a handle from the sentinel with a structured "opaque handles cannot be reconstructed from JSON" diagnostic — the LOAD-BEARING opacity guarantee that makes "you cannot fabricate a SQLite connection in user code" true at the language level rather than at the documentation level. 5 plumbing tests pass: (1) `db_handle_clones_share_inner_arc_and_refcount_returns_to_one_on_drop` proves the refcount story; (2) `db_handle_equality_is_arc_pointer_identity_not_structural` proves equality is Arc::ptr_eq (independent handles to the same path are NOT equal); (3) `db_handle_type_name_is_the_language_level_name` proves `value.type_name() == "DbHandle"`; (4) `json_to_value_refuses_to_mint_a_db_handle_even_when_shape_matches_sentinel` proves the opacity gate fires even on a perfect sentinel match; (5) `db_handle_named_type_resolves_to_the_opaque_primitive` proves the resolver+typechecker round-trip carries `Type::DbHandle` end-to-end. NO SQLite-runtime code yet. NO `std/db.cor` changes yet. NO dispatch interception. Validation gate: `cargo check --workspace --tests` clean; 109 vm + 248 types + 41 resolve + 38 ir + 28 guarantees + 45 stdlib all pass; corpus verify exits 1 only on the two deliberate fixtures. Commit: `feat(vm): opaque DbHandle value + type primitive for the executing SQLite surface`.
    - [x] 33S3b-runtime-and-executing-tools   **[closed 2026-06-09]** Moved `DbHandleInner` from `corvid-vm` to `corvid-runtime::db` so the runtime can mint Arcs and hand them back to the interpreter's typed-Value dispatch branch. Added `DbHandleRegistry` in `corvid-runtime/src/db.rs` — `Arc<Inner>` wrapper around `(AtomicU64 id, RwLock<HashMap<u64, Arc<Mutex<Connection>>>>, AtomicBool quarantine_writes)` so the registry is `Clone` and all clones share the same backing slotmap + write-quarantine flag. Threaded `db_registry` through `Runtime` (`Runtime::db_registry()` accessor) + `RuntimeBuilder::build()` (flips quarantine_writes during Substitute-mode replay alongside io/http/store). Added three typed-dispatch methods on Runtime: `db_open_tool(path) -> Arc<DbHandleInner>` resolves the path through `self.io_policy` (with `":memory:"` as the documented bypass), opens via the registry, returns the Arc; `db_query_tool(&Arc<DbHandleInner>, sql, params) -> JsonValue` and `db_execute_tool(...)` look up the connection by handle id, run parameter-bound SQL through `rusqlite::params_from_iter` over typed `DbValue`s, marshal to envelope JSON. All three emit `ToolCall` / `ToolResult` trace events with the opaque-sentinel handle shape (same as `value_to_json`). Wired interpreter dispatch in `corvid-vm/src/interp.rs`: when `is_stdlib_db_tool(callee_name)`, route to `dispatch_stdlib_db_tool` which extracts `Arc<DbHandleInner>` from the first arg's `Value::DbHandle` for query/execute, converts `List<DbParam>` Corvid values to `Vec<DbValue>` via `extract_db_params` (reads the `value_kind` discriminator + the matching value field), then calls the typed runtime methods. `db_open`'s result is wrapped in `Value::DbHandle(arc)` directly; query/execute results round-trip through `json_to_value` against the declared envelope shape. Updated `std/db.cor`: extended `DbParam` type with value-carrying fields (`int_value`, `float_value`, `string_value`, `bool_value`) for the executing path; added typed-value constructor agents `db_param_int` / `db_param_float` / `db_param_text` / `db_param_bool` / `db_param_null`; renamed envelope-builder agents `db_query` → `db_request_query` and `db_execute` → `db_request_execute` (33S2-style rename to free the names for executing tools); added inline `effect db_egress_open` (reversible) / `effect db_egress_read` (reversible) / `effect db_egress_write` (side-effecting); added `public tool db_open(path: String) -> DbHandle uses db_egress_open` plus `db_query` and `db_execute` tool declarations. Renamed registry entries in `corvid-types/src/effects.rs` from `db_query`/`db_execute` to `db_egress_open`/`db_egress_read`/`db_egress_write` to avoid tool↔effect resolver namespace collision; updated the 33S0 effect-registration test to assert the new names. Updated `examples/backend/state_app/src/main.cor` to use the renamed `db_request_query` / `db_request_execute` envelope agents. **5 load-bearing plumbing tests in `corvid-runtime/src/db.rs::tests`** all pass: (1) `db_handle_registry_round_trip_against_memory_database` proves the registry round-trips a `:memory:` connection through CREATE + parameterised INSERT + SELECT; (2) **`db_param_text_with_sql_metacharacters_is_bound_as_data` — THE injection-proof test** — inserts `"'; DROP TABLE users; --"` as a typed `DbValue::Text` parameter, then verifies (a) the users table still exists, (b) the stored email is the EXACT verbatim metacharacter string, never parsed as SQL; (3) `db_handle_registry_quarantine_blocks_execute_with_db_surface_violation` proves write-quarantine refuses `execute` with `QuarantineViolation { surface: "db", .. }`; (4) `db_handle_registry_quarantine_does_not_block_query` proves reads pass through during replay (33S3c integrates the trace-substitution upper gate); (5) `db_handle_registry_rejects_unregistered_handle_id` proves forged handle ids are refused with a structured diagnostic. Validation gate: workspace `cargo check --tests` clean; 109 vm + 248 types + 41 resolve + 9 runtime-db + 45 stdlib + 12 replay-quarantine + 3 executing-http-e2e + 1 executing-io-e2e + 34 tour topics + 28 guarantees all pass; corpus verify exits 1 only on the two deliberate fixtures. Commit: `feat(runtime): executing SQLite tool surface (db_open/db_query/db_execute)`.
    - [x] 33S3c-cli-loader-and-end-to-end-and-replay   **[closed 2026-06-09]** Shipped end-to-end driver acceptance + replay-quarantine fixtures. NO separate CLI loader needed — the SQLite surface inherits `[io] root` for path confinement (the deliberate design that makes `db_open` strictly narrower than `io_write_text`); `corvid new` scaffold needs no change (33S2b already added `[io] root = "."` + `[http] allow = []`). **3 driver-layer end-to-end tests** in new `crates/corvid-driver/tests/executing_sqlite_through_driver.rs`: (1) `real_corvid_program_round_trips_data_through_executing_sqlite_dispatch` — compiles a real `.cor` source via `compile_to_ir_with_config_at_path`, runs through `run_ir_with_runtime`, opens `:memory:`, CREATE + parameterised INSERT (with `db_param_int` + `db_param_text` constructors) + parameterised SELECT, returns the envelope's `rows_affected`. Proves the interpreter's `dispatch_stdlib_db_tool` branch (33S3b) routes correctly through the runtime's `DbHandleRegistry` against real rusqlite. (2) `db_open_with_path_outside_io_root_is_refused_by_policy` — pins the `[io] root` policy reuse: a program with `[io] root = "."` trying to `db_open("../../etc/passwd")` is refused at the policy boundary with the SAME diagnostic the io tools emit. Load-bearing security reuse property — SQLite is structurally as narrow as the file-I/O surface, with no separate `[db]` allowlist required. (3) **`db_param_text_with_sql_metacharacters_survives_round_trip_through_real_corvid_program` — the load-bearing injection-proof test through a REAL Corvid program**. The program calls `db_param_text("'; DROP TABLE users; --")`, inserts it, then runs `count(*)`. If SQL interpolation existed anywhere on the path, the DROP would fire and the count would error. The fact that the program returns 0 (envelope's `rows_affected` for the SELECT) proves the table survived AND the metacharacter string was bound as data through `rusqlite::params_from_iter`. 33S3b's unit test proved the property at the registry layer; this test proves it at the language-level surface with the canonical `db_param_text` constructor. **2 replay-quarantine fixtures** in `crates/corvid-runtime/tests/replay_quarantine_corpus.rs`: (4) `replay_blocks_executing_db_execute_dispatch_from_escaping_to_database` — POST/UPDATE/DELETE/DDL through `Runtime::db_execute_tool` during Substitute-mode replay refuses with `QuarantineViolation { surface: "db", .. }` regardless of SQL contents (Phase 33S3b's `DbHandleRegistry::quarantine_writes` flag is the registry-layer enforcement; `RuntimeBuilder::build` flips it during replay). (5) `replay_does_not_block_executing_db_query_dispatch_during_write_quarantine` — companion: reads pass through during write-quarantine (no trace-substitution upper gate yet for db_query rows; a follow-up slice can add it once the trace schema carries row events). The fixture uses the runtime's typed-Value dispatch path (`db_execute_tool` / `db_query_tool`) directly rather than `call_tool` JSON dispatch, because that's where the executing SQLite tools live by design (DbHandle can't round-trip through JSON). Validation gate: workspace `cargo check --tests` clean; 3 sqlite-e2e + 3 http-e2e + 1 io-e2e + 45 stdlib + 9 runtime-db + 14 replay-quarantine + 28 guarantees all pass; corpus verify exits 1 only on the two deliberate fixtures. Commit: `test(std): end-to-end + injection-proof + replay-quarantine acceptance for executing SQLite surface`.
    - [x] 33S3d-sqlite-invention-proof   **[closed 2026-06-09]** Three RuntimeChecked guarantees added to `corvid-guarantees::registry::GUARANTEE_REGISTRY`: `io_source.sqlite_parameter_binding_only` (structural parameter-binding via `rusqlite::params_from_iter`; SQL injection is prevented by the language's type system + the runtime's binding path, not by escaping), `io_source.sqlite_write_quarantine_on_replay` (`db_execute` during Substitute-mode refuses with `QuarantineViolation { surface: "db", .. }`), `io_source.sqlite_read_passthrough_on_replay` (`db_query` not blocked by write-quarantine; the trace-substitution upper gate lands in a follow-up slice). Three matching `pub const GUARANTEE_ID_*` anchors at the enforcement sites in `crates/corvid-runtime/src/db.rs` so the `every_enforced_guarantee_id_is_wired_to_workspace_source` sentinel passes. Claim-coverage: the three ids added to `corvid-guarantees::signed_claim::SIGNED_CDYLIB_CLAIM_GUARANTEE_IDS` so `corvid build --sign` accepts a cdylib whose source uses these tools. **Path confinement is NOT a separate guarantee** — `db_open` reuses `IoToolPolicy::resolve` and inherits the existing `io_source.fs_path_confinement` guarantee (avoids drift between io_* and db_open path boundaries). **The @deterministic-rejection property is NOT a separate guarantee** (mirrors 33S1c / 33S2c) — covered by the existing decl-replayability rule; 33S3d adds two pinning tests at `crates/corvid-types/src/tests.rs::deterministic_agent_calling_db_query_tool_is_rejected` + `deterministic_agent_calling_db_execute_tool_is_rejected` so the rule can't quietly relax. `docs/reference/core-semantics.md` regenerated via `corvid contract regen-doc` (drift gate passes). New `docs/reference/stdlib/db.md` reference page (~270 lines) — REPLACES the prior "envelopes only" framing with an executing-surface reference doc parallel to `http.md` and `io.md`. Covers the 3 tools, the typed `DbParam` constructors, the opaque `DbHandle` primitive type (with the load-bearing "user code cannot fabricate a handle" structural argument), the 3-property security model (parameter-binding-only + [io] root reuse + replay write-quarantine), the worked typed-user-store example demonstrating the injection-resistant `db_param_text("'; DROP TABLE users; --")` survival path, and explicit "post-v1.0 — what's NOT in scope" section preserving the Postgres "via tool wrapper" framing. `docs/reference/stdlib/README.md` gained the `## std.db` section linking to `db.md` + summarising the 3 executing tools + the 5 typed param constructors + the 3 RuntimeChecked guarantees + the IoToolPolicy reuse rationale. `docs/reference/inventions.md` proof-matrix row added under the HTTP-Client row. README invention-catalog entry added under Verification (immediately after the HTTP-Client entry) with the standard Spec/Tour/Roadmap/Proof/Non-scope footer and three-bullet load-bearing-property summary. `corvid tour --topic sqlite` topic added to `crates/corvid-tour-catalog/src/lib.rs`; source uses `:memory:` so the tour runs OFFLINE (no test database file needed); compiles through the `corvid_driver::compile` gate (`all_tour_sources_compile` test passes 35/35). Validation gate: 28 guarantee tests + 35 tour topics + 45 stdlib + 9 runtime-db + 14 replay-quarantine + 3 sqlite-e2e + 3 http-e2e + 1 io-e2e + 2 deterministic-rejection all pass; corpus verify exits 1 only on the two deliberate fixtures. Commit: `docs(invention): SQLite surface proof matrix, tour, and reference doc`.

  - [x] 33S4-batteries-quickstart   **[closed 2026-06-10]** Shipped the adoption-payoff slice. New `docs/book/18-talking-to-the-outside-world.md` chapter walks through the end-to-end HTTP → typed-decoder JSON → SQLite → read-back pipeline in pure Corvid — zero Python glue, zero host-language plumbing. The chapter covers project setup, the pipeline code, the four executing-I/O guarantees (SSRF block, allowlist, parameter binding, JSON safety) with worked "trigger each boundary, watch it fire" examples, replay-quarantine semantics across all four surfaces, and the signing-claim audit trail. Updated `docs/book/02-quickstart.md` — added a new Step 4 ("Read a real file (the executing-I/O surface)") demonstrating `io_read_text` against a project-staged `note.txt`, with the structural `[io] root` confinement promise; renumbered subsequent steps (Step 5 = dangerous tool, Step 6 = approve, Step 7 = replay) so the executing-I/O example is the first concrete I/O surface a new user sees. CI guards in new `crates/corvid-driver/tests/book_outside_world_pipeline.rs`: (1) `book_chapter_no_python_pipeline_runs_end_to_end_through_real_corvid_program` — lifts the chapter's `src/main.cor` body verbatim, stages it as a project with `[io] root = "."` + `[http] allow = ["api.example.com"]`, spins up wiremock serving `/users/1` → `{"id": 7, "email": "alice@example.com"}`, builds a reqwest client with `.resolve()` DNS override pointing `api.example.com` at the loopback wiremock port (same pattern 33S2b established), compiles the source via `compile_to_ir_with_config_at_path`, runs through `run_ir_with_runtime`, asserts the agent returns `Ok(0)` (the SELECT envelope's `rows_affected`). Proves the chapter's load-bearing claim: this exact source runs end-to-end. (2) `quickstart_executing_io_snippet_compiles_and_reads_the_file` — stages `note.txt` under the project's `[io] root`, runs the quickstart's `io_read_text` snippet, asserts the read returns the staged contents. Proves the quickstart's first executing-I/O example actually works. The four executing-I/O tour topics (file-io, http-client, sqlite, json) are already CI-covered by `crates/corvid-cli/src/tour.rs::all_tour_sources_compile` (36 topics; passes after 33R5b-c). Validation gate: workspace check + tests clean; 36 tour topics + 5 sqlite-e2e + 3 http-e2e + 1 io-e2e + 5 json-e2e + 2 book-pipeline-e2e + 46 stdlib all pass; corpus verify exits 1 only on the two deliberate fixtures. Acceptance: the no-Python pipeline runs end-to-end through `corvid run` with NO Python in the project; CI compiles + runs all four new tour topics + the book pipeline + the quickstart snippet. Commit: `docs(book): end-to-end I/O pipeline with no host glue + CI coverage`.
...
- [x] Provenance test **[dispositioned 2026-07-14 by slice 48b — re-scoped to shipped semantics and test-pinned]** The universal "every connector return is `Grounded<T>`" shape still follows from the post-v1.0 `connector ... grounded` syntax (`35V2-P41-I`) and stays there. What ships NOW, pinned by two checker tests + a guide section: (1) the OPT-IN pattern works today with zero new syntax — a connector-backed tool declared with a `data: grounded` effect has its return auto-wrapped `Grounded<T>` at call sites and the provenance flows to declared-Grounded returns (`grounded_connector_tool_return_is_wrapped_and_flows`); (2) the "strips fails typecheck" clause as written contradicted the SHIPPED deliberate semantics — `Grounded<T>` → `T` is the legacy coercion whose drop is tracked by the explicit IR provenance-discard node (four-tier-verified by the `legacy_grounded_coercion` corpus fixture; newly pinned at the checker level by `grounded_connector_return_strip_is_the_tracked_legacy_coercion`); the strict no-implicit-strip mode rides `35V2-P41-I`. Both tests added as refs on `grounded.provenance_required` (the unified enforcement row; `grounded.propagation_across_calls` remains documentary per 35V-T1-B). docs/guides/connectors.md gained the "Grounding connector reads today" section.
- [x] AI helpers landed (or follow-ups filed) **[closed 2026-07-14 via the filed branch, dispositioned by slice 48c]**: `corvid connectors check --narrate` (RAG-grounded drift narrator) SHIPPED 2026-05-20 (`35V2-P41-H-LR-drift-narrator`, `connector.drift_narration_grounded` RuntimeChecked). The two remaining sub-slices are RE-FILED POST-v1.0 with rationale: `corvid connectors mock-fixture-gen` — generative fixture synthesis needs a live LLM at authoring time, while the shipped connector test-kit's hand-written mock fixtures already cover every named threat scenario; generation is an authoring accelerator, not a guarantee gap. `corvid connectors fail-sim` — LLM-driven fault synthesis broadens what the shipped adversarial corpus (7 named threats, 14 deterministic tests) already pins; breadth-via-LLM is additive, not launch-blocking. Both entries added to the Post-v1.0 roadmap list.
...
- [x] `35V2-P42-G0-imported-struct-4`  **[dispositioned 2026-07-14 by slice 48d — confirmed unreached; closed as explicitly post-v1.0]**  The "only if reached" condition never fired: the reference apps use scalar `pub extern "c"` entrypoints and no prompts, so no pre-launch surface hits the boundary. The refusal is LOUD and actionable (codegen `check_entry_boundary_type` names `ImportedStruct` and the file-local-alias workaround — now pinned by `imported_struct_entry_boundary_refuses_loudly_with_workaround`), which is what the tier matrix promises for shapes a compiled tier does not carry. The lowering itself stays with the tracked cross-file native layout follow-up (`lang-cor-imports-basic` driver-integration).
...
- [x] AI helpers landed: 2/5 shipped; remaining 3 dispositioned 2026-07-14 by slice 48c. SHIPPED: `corvid release notes` (`release.notes_grounded` RuntimeChecked) + `corvid claim audit --explain-failures` (`claim.audit_explain_failures_grounded` RuntimeChecked), both 2026-05-20. DISPOSITIONED: `corvid deploy tailor` RE-FILED POST-v1.0 — the deterministic deploy surface (package/Dockerfile/attestation + compose/PaaS/k8s manifests) shipped in 43B/43C; agentic tailoring is sugar on a complete surface. `corvid upgrade assist` RE-FILED POST-v1.0 — the mechanical upgrade path is covered (`corvid upgrade` rename migrations, `corvid upgrade refresh-std`, `corvid release notes`); agentic assistance is additive. `corvid beta synthesize-feedback` MOVED TO THE LAUNCH PHASE (not post-v1.0): it operates on beta-program feedback that will not exist until 43H runs — it rides with the externally-gated beta items and should ship as the beta opens. Post-v1.0 roadmap list updated with the re-filed pair.
...
## Language completeness track — Phases 44–47 (filed 2026-07-09 from the language gap audit)

Source: [`docs/meta/language-gap-audit-2026-07-09.md`](docs/meta/language-gap-audit-2026-07-09.md) — a four-dimension evidence-backed survey (language surface, type system, stdlib/backend parity, AI-native coverage) taken at commit `73ea143`. Headline: the governance layer (effects, approve, replay, cost, evals, signing, executing io/http/sqlite/json) is deep and test-backed; the expression layer is far behind the project's own book — at least 20 documented features do not parse, and nine gaps rate as blockers for the "general AI-native language" claim.

**OPEN DECISION (pre-phase chat required before any Phase 44–47 slice starts):** which of these phases gate v1.0. Default recommendation recorded here: Phase 44 (doc honesty) and Phase 45 core slices (45a–45l) gate launch — shipping a book that teaches an unimplemented language fails the no-shortcuts rule and would be found by any HN reviewer in minutes; Phase 46 gates the "general AI-native" *positioning* (46a/46b at minimum); Phase 47 is honesty-fixes that ride along. Per-phase sequencing below is dependency-ordered, not calendar-committed.

**DECISION PRINCIPLE (CTO, 2026-07-09): every design call in this track is judged on three criteria — does it make Corvid more INVENTIVE, more STRONG (safer, more verifiable), and more EASY for developers.** When criteria conflict, prefer the option coherent with Corvid's existing Python-flavored, effect-typed surface. Rationale recorded per slice; no per-decision a/b escalation needed.

**DIRECTION LOCKED (CTO chat 2026-07-09): implement the book.** The book is treated as the spec — every feature it teaches gets an implementation slice rather than a permanent "Planned" demotion. Phase 44's Planned markers are *interim* honesty: they go on now so no reader is misled, and each one comes off when its implementation slice ships. Three design calls resolved in the same chat (delegated to maintainer judgment):

1. **`fn` pure functions: implement (45r).** Plain non-generic `fn` as a fourth callable — statically effect-free, callable from `@deterministic` contexts, zero agent overhead for pure helpers. Developers are never forced to use it; agents keep working unchanged. **User generics: post-v1.0** (45p resolved); **`#:` doc comments: post-v1.0** (45q resolved — token + `--help`/LSP-hover rendering ship together with the LSP work) — no type-variable representation exists in the `Type` enum and the ripple through checker/IR/all backends is the largest single work item in the track; the book's generic examples (`fn first<T>`) sit under a Planned heading until then. Recorded in the post-v1.0 section.
2. **`@host` tool bodies: revise the book, do not implement.** The registered host-tool/FFI dispatch model (Phases 22/30/33S) superseded the early inline-`@host` sketch and carries the effect-typing, replay-quarantine, and signing story; a second FFI mechanism would double the security surface forever. 44a rewrites book 04's tool section to teach the shipped model. This is a fix-the-docs case, not a fix-the-surface case, because the shipped surface is the better design.
3. **Int overflow: always-checked is the semantics; the book aligns (44a), pinning tests land (47e).** No `--overflow` saturation flag: silent saturation is data corruption feeding LLM-facing values, and one arithmetic semantics keeps replay deterministic across interpreter + codegen tiers.

### Phase 44 — Documentation honesty + spec-surface realignment (~2–3 weeks)

The cheapest, highest-leverage phase: make the docs describe the language that exists. Every slice is doc/test-only except 44c. The rule applied is CLAUDE.md's own: a shipped surface conflicting with the spec means fixing the surface — but where the "spec" documents pure aspiration (book ch 13 has zero implementation), the honest fix is an explicit **Planned** marker, not silent deletion and not leaving it.

- [x] 44a-book-syntax-types-realign   **[closed 2026-07-09]** Fix `docs/book/04-syntax.md` + `05-types.md` per the locked direction: features with implementation slices in this track (`Map`, `match`, enum variants, plain `fn`, lambdas, `while`, `let`, type aliases, named struct literals + `..` update, string/list methods, `to_float`/`to_int_truncated`, doc comments) get **interim "Planned — lands in Phase 45/46" markers** that come off as each slice ships; the three resolved-by-design items get **rewrites to the shipped semantics**: tool sections teach the signature-only + registered host-tool/FFI model instead of `@host.x.y` bodies (`crates/corvid-syntax/src/parser/decl/mod.rs:130-161`), overflow docs state always-checked trap semantics (no `--overflow` flag), generic `fn first<T>` examples move under a post-v1.0 Planned heading (45p resolution). `Unit` → `Nothing` naming fixed throughout. Until 45a ships, examples use the bare-assignment form that parses today (`parser/stmt.rs:211-233`). Acceptance: every code block in ch 04/05 either compiles through the driver or sits under an explicit Planned heading naming its slice.
- [x] 44b-book-pattern-matching-and-prompts-realign   **[closed 2026-07-09]** `docs/book/13-pattern-matching.md` is entirely unimplemented — retitle to a Planned-feature design doc or fold into a roadmap appendix; `docs/book/11-prompts.md:46-55` documents multi-message `system:`/`user:` prompt blocks the parser lacks (no role blocks in `parser/prompt.rs`) — mark planned (implemented by 46b). Acceptance: same compile-or-planned rule as 44a. Shipped: ch 13 converted to the 45h/45i design doc (all blocks `corvid-planned` under Planned markers) + a new compiling "What ships today" section demonstrating the real `?` + `if`/`else` consumption story; ch 11 prompt bodies fixed to the single-template-string form with `{param}` interpolation (incl. the non-string-param fact: an `Int` interpolates as JSON, no conversion needed — the old `.to_string()` example was doubly wrong), multi-message section marked Planned → 46b; both chapters added to the `book_snippets_compile.rs` guard (now 4 chapters).
- [x] 44c-grammar-drift-gate-strengthening   **[closed 2026-07-09]** `docs/reference/grammar.md:3-10` claims a drift gate cross-checks every production against the parser; the actual gate (`crates/corvid-syntax/tests/grammar_drift.rs:1-65`) checks only internal EBNF consistency. Two-part fix: (1) annotate unimplemented productions (`match_expr`, `map_literal`, `struct_literal`, `pattern`, sum-type variants, `type_alias_body`, `let`-form, unary `+`) with a machine-readable `# PLANNED` marker; (2) extend the gate so every non-planned production must be exercised by at least one parser test, and the gate's doc header honestly states what it checks. Acceptance: gate fails if a new production lands unmarked + untested. Shipped: 12 productions PLANNED-marked (45a/45g/45h/45i/45n/45q/46b); new `every_non_planned_production_has_parse_evidence` gate test with a curated SNIPPETS + EVIDENCE table (75 productions → 12 snippets, each parsed through the real `parse_file`); the gate's development iteration caught SIX further grammar.md drifts beyond the audit's list, all fixed in the same commit: (a) import use-lists are braceless with per-item `as` aliases + `import python "…"` form undocumented, (b) route decls carry HTTP method + optional `query`/`body` typed clauses + `-> json Type` response + a handler body block (grammar had `route "/path" -> handler` one-liners), (c) `schedule` requires `zone` (grammar had it optional) + takes an optional uses clause, (d) fixtures/mocks take params + return types (grammar had bare-name + block), (e) retry backoff is mandatory with a bare ms literal (`backoff exponential 250`, not `exponential(2)`), (f) `weak_effect` accepts only the builtin effect classes `tool_call`/`llm`/`approve`/`human` (grammar said IDENT). Also fixed: keywords paragraph now documents the contextual-vs-reserved split (`use`/`Nothing`/`let` are not reserved); the false `0x…` hex-INT claim dropped; `fixture_body`/`mock_body` phantom productions deleted. NEW FINDING filed into 45q: `@retry(...)` annotations are unparseable (keyword collision — `expect_ident` rejects `KwRetry`) though book ch 10 documents them.
- [x] 44d-quickstart-honesty   **[closed 2026-07-09]** Subsumes 33R12 (error-code drift E0301 → E0101 per `render.rs::detect_error_code`) and fixes the quickstart's refund example, which uses the unparseable tool-body `@host.payment.refund(...)` form. Extend `crates/corvid-driver/tests/book_outside_world_pipeline.rs`'s compile-guard pattern to every code block in `02-quickstart.md`. Acceptance: quickstart snippets compile through the driver in CI. Shipped: ch 02 fully realigned with programs verified against the LIVE compiler (`corvid check` run on each; real ariadne-style E0101 diagnostic embedded; real `ok:` output; real scaffold tree incl. `src/std/` + `tools.py`); prompt template fixed to `{text}` form; new `corvid-error` fence tag in the snippet guard — the dangerous-call-without-approve block MUST FAIL to compile in CI, pinning the book's load-bearing "does not compile" claim mechanically; ch 02 added to `GUARDED_CHAPTERS` (now 5 chapters); same E0301→E0101 + fake-diagnostic + trust-narrative fixes applied to ch 03 (surgical), ch 08, and `docs/guides/debugging.md`. **MAJOR FINDING (filed as 47g):** the approve gate keys ONLY on the `dangerous` keyword — the quickstart's original trust-dimension-based example compiled CLEAN against the live checker; the load-bearing claim was false as written. Docs now teach the shipped `dangerous` model; the trust-coupling semantics decision is 47g. Full ch 03 realign filed as 44f.
- [x] 44f-remaining-book-chapters-realign   **[closed 2026-07-09 — closes Phase 44]** **[filed by 44d]** Sweep the chapters 44a–44d didn't cover — 03 (tutorial: full realign; prompt bodies use `+` expressions and `@retrieve(...)` / `unwrap_with_citation()` forms that need verification against the shipped surface), 06-modules, 07-effects, 09-grounded, 10-agents (documents the unparseable `@retry` — pairs with the 45q fix), 12-errors, 14-project-layout, 15-testing (verify against the REAL fixture/mock signatures 44c discovered), 16-building, 17-replay, plus `docs/guides/*.md` — apply the fence-tag convention, verify every claim against the parser/checker the way 44d did (run the real compiler, don't trust the doc), and register each realigned chapter in `GUARDED_CHAPTERS`. Acceptance: every book chapter + guide is guard-registered with compile-true examples or explicit Planned markers. Shipped: 26 files guard-registered (17 book chapters incl. ch 18 + 9 guides with corvid blocks or fence hygiene). Live-compiler probes drove every rewrite; EIGHT new findings beyond the audit: (1) the budget checker is REAL and E0250 fires — and the ORIGINAL ch 03 tutorial's `@budget($0.50)` over a $100 refund effect could never have compiled; the chapter now teaches the failure as a feature (set budget low → E0250 → raise to cover worst case); (2) Grounded's real rule is asymmetric — ungrounded→grounded slots are always E0208, grounded→ungrounded coerces SILENTLY ("legacy rule") unless `@grounded_pure`; ch 09 rewritten around the asymmetry with two CI-pinned corvid-error blocks; (3) the named unwrap trio (`unwrap_with_citation` etc.) is fiction — filed into 45c; (4) `public effect` and `public import` are parse errors — visibility applies only to type/store/tool/prompt/agent; grammar decl production fixed; ch 06 re-export section marked Planned; (5) named annotation args (`@idempotency(key: …)`) don't parse — grammar annotation production fixed, filed into 45q(1c); (6) declaring a `mock` alongside its target fails typecheck E0203 at the declaration site — mocks unusable end-to-end; defect filed as 45q(1d), documented honestly in ch 15; (7) ch 14's `corvid.toml` reference documented seven fictional sections (`[package]`/`[dependencies]`/`[build]`/`[runtime]`/`[approvals]`/`[budgets]`/`[replay]`) — real schema is top-level name/version + `[llm]`/`[io]`/`[http]`/`[effect-system]`/`[package-policy]`/`[run]` (`corvid-types/src/config.rs:29`); rewritten from the actual scaffold; (8) module-prefix calls + braceless use-lifts through string imports work end-to-end (probed) — ch 06 rewritten around the shipped model. Guides were already written in the shipped style (later-phase docs) — registered with fence hygiene only. Guard runs 26 files: corvid blocks compile, corvid-error blocks verifiably fail, planned blocks sit under markers, no bare fences.
- [x] 44e-readme-streaming-claim-alignment   **[closed 2026-07-09]** README markets "Streaming" as a headline construct; all four provider adapters defer token streaming (`llm/anthropic.rs:13`, `openai.rs:11`, `gemini.rs:16`, `ollama.rs:89-91`) and a `-> Stream<T>` prompt wraps one blocking response in a singleton chunk (`interp/prompt/mod.rs:35-86`). Align the headline with the shipped boundary (the README body already discloses it at line ~456); cross-link 46d as the implementation slice. Acceptance: no README claim implies live token flow before 46d ships. Shipped: the Streaming Effects non-scope now states plainly that provider token streaming is not wired (one blocking call → single chunk; all four adapters defer SSE) and that the shipped algebra applies to agent-produced `yield` streams, with 46d cross-linked; the Partial<T> and ResumeToken<T> non-scopes gained matching one-line 46d pointers (LLM partials arrive complete, not progressively; live mid-stream interruption needs 46d). The old non-scope line misled by implying the gap was only provider-native *continuation*. The "streaming" entries in the constructs lists stay — `Stream<T>`/`yield`/the effect algebra ARE shipped language constructs; the corrected non-scope makes the provider boundary unambiguous.

### Phase 45 — Core expression completeness — **CLOSED 2026-07-11** (17 slices shipped 2026-07-09 → 2026-07-11; 45p resolved post-v1.0)

Closes the six core-language blockers (B1–B6 in the audit) plus the majors that share machinery. Dependency-ordered: binding/assignment surface first, then builtin-method machinery, then the data types, then `match` (the largest single item), then the lambda-dependent tail. Every slice ships the full invention contract where user-visible (tour topic, reference/book section, tests, inventions row where applicable).

- [x] 45a-annotated-assignment   **[closed 2026-07-09]** **[re-scoped 2026-07-09 under the decision principle]** Original scope was "restore Rust-style `let`". Judged against inventive/strong/easy: `let` bolted onto Corvid's Python-flavored surface adds a SECOND binding form (choice paralysis, migration burden, foreign idiom) for zero added safety. The coherent form is **Python-consistent annotated assignment**: `x: Int = 42` — the same `name: Type` shape every field, param, and effect dimension already uses. One binding form stays (bare `x = expr`); the annotation is opt-in and gives the checker an agreement contract (annotation vs initializer type, mismatch = compile error). Ship: parser (`IDENT (':' type_ref)? '=' expr` — `Stmt::Let.ty` slot already exists), checker agreement + mismatch diagnostic, grammar.md assign_stmt update + evidence, book ch 04/05 Planned notes flip to shipped compiling examples, `let` dropped permanently from the reserved-word plan (grammar keywords note updated). Destructuring (45i) binds without a keyword: `Decision { refund, .. } = expr`.
- [x] 45b-assignment-targets   **[closed 2026-07-09]** `x.field = v`, `xs[i] = v`, compound `+= -= *= /=` (`parser/stmt.rs:211-233` currently parses bare `IDENT = expr` only). Checker + IR + interpreter + all three codegen backends or explicit `not_supported`. Shipped end-to-end: lexer compound tokens (`+= -= *= /= %=`), `Stmt::Assign` AST node (operator kept in the AST/IR — never desugared, so index expressions evaluate exactly once), parser place-validation (Ident/FieldAccess/Index only, clear diagnostic otherwise), checker (target-type agreement, compound ops through the ONE operator table, `InvalidAssignTarget` when the path root isn't a local, cost analysis covers BOTH sides so an LLM call in an index expression can't hide from `@budget`), `IrStmt::Assign` + `IrPathSeg`, interpreter execution through the shared heap cells (`StructValue::set_field` + new `ListValue::set`; nested paths; checked compound arithmetic; bounds/type traps mirror the read paths), tier-picker `PlaceAssignmentNotNative` reason (auto-fallback to interpreter), loud `not_supported`/`NotImplementedError` in codegen-cl/py/wasm, conservative arms in the cl ownership/dataflow passes (mutation = effect barrier). **Semantics decision under the principle: Python-style reference semantics** — structs/lists are shared heap cells (the Phase 17 `Arc<Mutex>` memory model exists precisely for this), so mutation through one binding is visible through every alias; COW value semantics would have fought the entire runtime. Pinned end-to-end in `crates/corvid-driver/tests/place_assignment_e2e.rs` (aliasing visible through struct AND list aliases; nested `acct.wallet.balance -= 100.0`; compound single-eval; out-of-bounds store traps). 6 parser + 3 checker tests; grammar.md gains `place`/`assign_op` productions with evidence; book ch 05 records/lists sections gain compiling mutation examples with the reference-semantics callout.
- [x] 45c-builtin-method-dispatch-machinery   **[closed 2026-07-10]** The enabling slice: lift the "methods currently work only on user-declared struct types" restriction (`corvid-types/src/checker/call.rs:704`) into a builtin-method table keyed by receiver `Type`, with IR lowering to intrinsic ops. No methods ship in this slice — machinery + one pilot (`String.length()`) + exhaustive checker tests. Shipped: `corvid-types/src/builtin_methods.rs` — ONE shared table (a function, not a static map, so generic receivers like `List<T>.first() -> Option<T>` compute return types naturally) driving all three consumers: checker (`check_method_call` looks up sig, checks arity + arg types), IR lowerer (`try_builtin_method_call` re-derives the lookup from the receiver's checked type → `IrExprKind::BuiltinMethod`), interpreter (one arm per `BuiltinMethodKind`). Pilot `String.length()` counts Unicode scalar values (Python's `len(str)`, not UTF-8 bytes — e2e pin uses "héllo" = 5 scalars / 6 bytes). The old "Built-in receiver methods are not implemented yet" diagnostic now names the table and the slice map. Tier honesty: `BuiltinMethodNotNative` reason (auto-fallback), codegen-cl `not_supported`, codegen-py loud raise, wasm unsupported; conservative walk arms in the cl ownership/dataflow passes (builtin methods are pure value ops — effect-free, non-consuming borrows). Grounded receivers deliberately NOT auto-unwrapped in 45c — the method-call contagion rule is decided alongside the Grounded unwrap batch. 3 checker tests + 1 e2e Unicode pin; book ch 04/05 notes flipped (length shipped, batches 45d/45f pending). **[44f addendum]** The `Grounded<T>` named unwrap trio (`unwrap_with_citation()`, `value()`, `unwrap_discarding_sources()`) rides this table too — book ch 09 documents them as the audit-visible consumption story, but they don't exist today (the only unwrap is the silent legacy coercion); ship them in the first method batch after the pilot so the `@grounded_pure` strict mode gets its explicit escape hatches.
- [x] 45d-string-methods   **[closed 2026-07-10]** **[subsumes 33R5c-strings]** On the 45c table: `length`, `to_upper`, `to_lower`, `split(sep) -> List<String>`, `trim`, `contains`, `starts_with`, `ends_with`, `replace`, `substring(start, end)`, `chars`-iteration decision. A Corvid program can finally uppercase a string. Shipped: all nine methods as table arms + one interpreter helper (`eval_builtin_method` in `interp/expr.rs`). Semantics decided once on the `BuiltinMethodKind` docs: Unicode-scalar indices/lengths everywhere; full Unicode casing; `split` with an empty separator TRAPS (Python-like — Rust's empty-pattern split yields surprising empty pieces; the diagnostic points at `for c in s`); `replace` replaces ALL occurrences; `substring` clamps out-of-range and returns "" when start >= end; NO `chars()` method — strings are already `for c in s` iterable (the decision the slice named). 2 checker tests + 2 e2e pins (batch incl. HÉLLO Unicode casing + clamp edges; empty-separator trap); book ch 05 strings section now shows the full compiling method set with the semantics callout. 33R5c ticks with this slice.
- [x] 45e-number-string-conversions   **[closed 2026-07-10]**   `Int.to_string()`, `Float.to_string()`, `Float.to_int_truncated()`, `Int.to_float()` (explicit; implicit widening stays — `types.rs:221`), `String.parse_int() -> Result<Int, String>`, `String.parse_float()`. Kills the `"count: " + n` type-error papercut. General string-interpolation syntax is a separate pre-phase-chat decision (prompt templates already interpolate `{param}` — `interp/prompt/mod.rs:324-334` — reusing that syntax in ordinary strings is the natural candidate). Shipped as pure 45c-table batch (+ `Bool.to_string()`, trivially coherent): 7 conversion kinds, one interpreter dispatch extension. Semantics decided on the enum docs: Float→String rendering is Python-style (`42.0` → "42.0", ALWAYS shows `.` or exponent — string output feeds LLM prompts/JSON and must stay visibly typed); `to_int_truncated` truncates toward zero and TRAPS on NaN/out-of-i64-range (the always-checked rule; NaN pin in e2e); parses trim whitespace (`" 42 ".parse_int()` works) and return `Result<_, String>` naming the offending input. 2 e2e pins (conversion batch incl. `parse_pair()` `?`-chaining + Err equality; NaN truncation trap) + book ch 05 Numbers section flipped to a compiling example (the corvid-planned block is gone). The `"count: " + n` papercut is dead.
- [x] 45f-list-methods-non-lambda   **[closed 2026-07-10]**   **[subsumes the non-lambda half of 33R5d-collections]** `length`, `append`/`push`, `contains`, `first`/`last -> Option<T>`, `slice`, `reverse`, `sort` (Ord-able element types), `join(sep)` for `List<String>`, `range(start, end) -> List<Int>` builtin (unblocks counted iteration — no `while` exists until 45k). `map`/`filter`/`fold` wait for 45j lambdas. Shipped: 9 list methods + the `range` builtin function. GENERIC returns work exactly as the 45c table promised — `first()`/`last()` return `Option<T>` computed from the receiver's element type, `slice` returns `List<T>`, `append`/`contains` take `T` params. In-place mutation for `append`/`reverse`/`sort` returning `Nothing` (Python-coherent with the 45b reference-semantics rule; e2e pins that an alias sees the append). `sort` is TABLE-GATED to Int/Float/String elements — `List<struct>.sort()` gets the standard no-builtin-method diagnostic (pinned); Float sorts by IEEE total order. `join` offered on `List<String>` only. `range(start, end)` is a free builtin (`BuiltIn::Range`) lowered through the existing BuiltinMethod IR with start-as-receiver — no new IR variant, no exhaustive-match ripple. 3 checker tests + 1 e2e batch pin; book ch 05 lists section flipped to the compiling method set (only the 45j lambda-taking block remains Planned). Counted iteration unblocked ahead of 45k's `while`.
- [x] 45g-map-type   **[closed 2026-07-10]**   **[closes audit B3]** `Type::Map(K, V)` + `Value::Map` + `{"k": v}` literal + `m[k]` index read returning `Option<V>` (decision: `Option` vs trap — pre-phase chat) + `insert`/`get`/`contains_key`/`keys`/`values`/`remove`/`length` methods + `for (k, v) in m` iteration + `json_to_value`/`value_to_json` conversions (the JSON surface finally gets a typed read-write map counterpart to the opaque `JsonBuilder`). Codegen: interpreter-first with explicit `not_supported` degradations, mirroring the 33S3/33R5b pattern. Shipped full-pipeline: `Type::Map(K,V)` (display/assignability/generic head/py `dict[K,V]` hint/prompt-schema String-keyed→object else pair-array/abi opaque desc/cl `not_supported`/tier fallback), `{"k": v}` literals (dup key: LAST wins, Python; trailing comma; `{}` empty), `Value::Map` heap cell (Vec-backed, structural key equality, insertion order, FULL cycle-collector integration via `ObjectRef::Map`), reference semantics like List. THE two design wins, judged under the principle: `m[k]` READS as `Option<V>` (no KeyError like Python, no zero-value like Go, no `.get().cloned()` ceremony like Rust — absence is a typed value handled by `?` or `==`), while `m[k] = v` WRITES as insert-or-update (the easy Python write); compound `m[k] += v` traps on missing keys; the checker types the read as `Option<V>` but the assignment SLOT as `V` with a type-level compound-operator rule (numeric slots take all five ops, String/List only `+`). Methods on the 45c table: `length`/`get`/`contains_key`/`keys`/`values`/`remove` with generic returns; iteration via `m.keys()` until 45i destructuring lands `for (k, v)`. JSON interop both directions (`Map<String,V>` ↔ object; other keys ↔ pair arrays) — the typed read-write counterpart to the opaque JsonBuilder. Audit blocker B3 CLOSED. 2 checker tests + 1 e2e pin (dup-key/Option hit+miss/insert+update+compound/aliasing/order/remove); grammar `map_literal`/`map_entry` PLANNED markers off + evidence; book ch 05 Maps section flipped to compiling examples.
- [x] 45h-user-sum-types   **[closed 2026-07-10]**   **[closes audit B2]** `type Status: | Pending | Approved(approver: String)` per the grammar.md:68 production that already documents it. Declaration (`parse_field` gains the `|` branch — `type_field.rs:56-68`), `Type`-level variant representation, construction, `==`, exhaustiveness metadata for 45i. The AST comment "Enum/union types arrive in v0.2" (`corvid-ast/src/decl.rs:324`) finally comes due. Shipped: `|` variant lines in type decls (record XOR sum, parser-enforced; new `Pipe` token), variants as BARE file-scope constructors — unit variants are plain values (`p = Pending`), payload variants construct like calls (`Approved("alice")`, positional, field types enforced with the field name in the diagnostic; bare payload references get "construct it with `Approved(...)`"); the value's static type is the OWNING sum type (nominal `Type::Struct(owner)` — zero new Type-enum ripple); duplicate variant names across types = duplicate-decl error (v1, documented). Runtime: `Value::Enum` cell (positional payload, structural == on type+variant+fields, full cycle-collector integration via `ObjectRef::Enum`), display as `Approved("alice")`, tagged JSON form. IR: `IrCallKind::EnumConstructor` (small call-kind ripple, no new expr kind) + `IrType.variants` metadata staged for 45i exhaustiveness. Codegen tiers: loud interpreter-only degradations. 2 checker tests + 1 e2e pin (unit values, structural ==/!= across variants and payloads, sums in lists); grammar variant productions PLANNED-off + evidence; book ch 05 sums section flipped (construction + `==` compile; `match` block stays 45i-planned). Audit blocker B2 CLOSED.
- [x] 45i-match-expression   **[closed 2026-07-10 — audit blockers B1 + B6 CLOSED]**   **[closes audit B1 — the largest single slice]** `match` expression + statement with literal / variant / struct-destructure / binding (`x @ pat`) / wildcard / guard (`if` clause) patterns and compiler-checked exhaustiveness over 45h sum types + `Option`/`Result`/`Bool`. Also the destructuring forms book ch 13 teaches: `Decision { refund, amount, .. } = expr` destructuring bindings (keyword-free, matching the 45a annotated-assignment decision) and destructuring in `fn`/agent params. Parser (`KwMatch` token), AST pattern nodes, checker (exhaustiveness + binding types), IR decision-tree lowering, interpreter, codegen or explicit degradation. Book ch 13 moves from Planned back to shipped, block-by-block compile-tested. This also closes audit B6's worst half: `Ok`/`Err`/`Some`/`None` become inspectable. Shipped end-to-end: `KwMatch` + pattern AST (wildcard / literal incl. negatives / bare-name with RESOLVER disambiguation between unit-variant-test and binding / `x @ pat` / `Variant(subpats)` incl. builtin Some/Ok/Err/None / `Type { field: pat, shorthand, .. }` record patterns); `match` is an EXPRESSION (arms `pattern -> expr`, guards `if`); block-in-expression parsing mirrors replay_expr with a new `block_expr_terminated` parser flag so `x = match s:` works mid-block; resolver `resolve_pattern` introduces bindings; checker `match_check.rs` types every pattern against the scrutinee, unifies arm result types (Int→Float widening), and enforces EXHAUSTIVENESS (sum = every variant irrefutably covered or catch-all — nested coverage deliberately conservative in v1, documented; Option = Some+None; Result = Ok+Err; Bool = true+false; else catch-all; GUARDED ARMS NEVER COUNT) with `NonExhaustiveMatch` naming what's missing; IR `IrExprKind::Match` + `IrPattern`; interpreter `pattern_matches` collects bindings transactionally before applying, then guard, then body; cost analysis over-approximates (scrutinee + all arms) so `@budget` stays sound; loud interpreter-only degradations on compiled tiers. Book ch 13 REVIVED (all blocks compile; the non-exhaustive example is a CI-pinned corvid-error block; destructuring-binding statements deferred to ship with 45n's shared `Type { }` surface); ch 05 sum/Option and ch 12 Result/typed-error-enum sections flipped to compiling `match` examples. 2 checker tests + 1 e2e full-surface pin + grammar productions PLANNED-off with parse evidence.
- [x] 45j-lambdas   **[closed 2026-07-11]**   `fn (x) -> expr` lambdas shipped end-to-end as FIRST-CLASS values. Syntax: `KwFn` reserved; expression-bodied lambdas with optional param annotations (`fn (x: Int) -> x + 1`); block-bodied form deferred (nothing in the book claims it). Function types are REAL now — the audit hole went deeper than reported: `(Int) -> Int` was never even PARSED, not just resolved to Unknown; 45j adds `function_type` to the type grammar, resolves `TypeRef::Function` to `Type::Function` (both local and imported paths), and gives it assignability (contravariant params, covariant ret, Unknown-lenient). CAPTURE SEMANTICS (decided under the principle): by-value snapshot of the visible environment at lambda creation — no Python late-binding footgun (rebinding an outer Int later is invisible to the closure) while heap cells still share (a captured list observes in-place mutation); e2e-pinned. Lambda params live in their own resolver scope and SHADOW outer locals. Checking is CONTEXTUAL: the expected function type at the use site supplies unannotated param types and checks the body against its return type (`filter`'s non-Bool body errors AT THE BODY); annotations win when both exist; no context = Unknown-lenient. Table: `map`/`filter`/`fold`/`any`/`all` land on the 45c table with SEQUENTIAL SIGNATURE REFINEMENT — `fold`'s `init` argument fixes the accumulator type in both the lambda param and the result, and `map`'s checked lambda return type fixes the result element type (`xs.map(fn (x) -> x.to_string())` is `List<String>`, pinned via `join`). `any`/`all` short-circuit. First-class calls: a function-typed local is callable (`IrCallKind::ClosureLocal`; checker validates args against its params); calling a non-function local now names the actual type (`cannot call a value of type Int`) instead of `<local value>`. VM: `Value::Closure` cell (`ClosureValue`, identity equality like Python function objects) with full cycle-collector integration (`ObjectRef::Closure`: children = captured env, clear_payload breaks closure-in-captured-list cycles); `apply_closure` swaps envs, binds params, restores on error; `?` inside a lambda body is a LOUD trap, not silent conversion. The interpreter's `eval_expr` lifetime was relaxed from `&'ir` so closure bodies (owned by values, not the IR arena) evaluate through the same entry point. Cost analysis: zero-cost body = free lambda; any effectful body = unbounded warning (call count statically unknown) so `@budget` stays sound. Loud interpreter-only degradations on all compiled tiers + native-ability gate. 2 checker tests + 2 e2e pins (full surface returning LAMBDAS WORK; capture-snapshot-vs-shared-cell) + grammar productions with parse evidence (incl. `(Int) -> Int` annotated assignment); book ch 05 lambda block flipped from Planned to compiling; grammar.md keyword list also gains the missed `match` from 45i.
- [x] 45k-while-loop   **[closed 2026-07-11]**   `while cond:` shipped end-to-end AND NATIVELY: the interpreter re-evaluates the condition before every iteration (Grounded<Bool> accepted, same rule as `if`), and codegen-cl gained a real `lower_while` (header/body/exit CLIF blocks; `continue` jumps to the header since a while loop has no step block; body scope releases refcounted locals per iteration) — the native probe with `break`+`continue` returned the hand-checked sum. `break`/`continue`/`pass` PROMOTED from sentinel-`Ident` encoding to real `Stmt::Break`/`Continue`/`Pass` variants; the entire sentinel machinery (BuiltIn::Break/Continue/Pass scope entries, resolver bindings, the lower.rs special-case, the checker's Nothing-typed arm) is DELETED, not deprecated. NEW compile error: `break`/`continue` outside any loop (`LoopFlowOutsideLoop`, checker tracks loop depth across both loop kinds) — under the sentinel encoding a stray `break` lowered silently. Cost analysis: a non-zero-cost while body marks the estimate UNBOUNDED (no static iteration count) so `@budget` stays sound; a zero-cost body stays bounded. CFG dataflow models while with the same head/body/backedge/exit shape as for (new `IrNavStep::WhileBody`); wasm degrades loudly alongside for; Python transpile emits a real `while`. 1 checker test (Bool cond + stray-break + ok program) + 2 e2e pins (full surface incl. condition-over-mutating-list + break-exits-innermost-only) + grammar `while_stmt` production with parse evidence; book ch 04 Planned block flipped to a compiling example (match pointer now references ch 13).
- [x] 45l-option-result-ergonomics   **[closed 2026-07-11 — audit B6 remainder closed]**   Eight kinds on the 45c table: `Option<T>.unwrap_or(default: T) -> T` / `is_some` / `is_none` / `ok_or(err: E) -> Result<T, E>` and `Result<T, E>.unwrap_or` / `is_ok` / `is_err` / `map_err(f: (E) -> F) -> Result<T, F>`. Generic error types come from the 45j SEQUENTIAL REFINEMENT machinery, not generics: `ok_or`'s `E` is its checked argument's type, `map_err`'s `F` is the lambda's checked return type (both pinned by a checker test that feeds the refined types onward). `map_err` rides the 45j async closure path and applies its lambda ONLY on the Err side (e2e-pinned via a "never runs" closure on an Ok). STDLIB ENVELOPE AUDIT (the slice's second mandate): `io_read_text -> FileReadEnvelope`, `http_get -> HttpResponseEnvelope`, `db_query -> List<DbResult>` all return BARE envelopes and trap at runtime dispatch on failure. Verdict: YES, they should become honest `Result` returns now that Result is fully consumable (match + `?` + the 45l shorthands) — filed as slice 47h below (one slice, shared runtime-dispatch mechanics, three surfaces). 1 checker test + 1 e2e pin; book ch 12's last Planned block flipped to a compiling example.
- [x] 45m-datetime-and-math-builtins   **[closed 2026-07-11 — subsumes 33R5e-datetime + 33R5f-math; FULL INVENTION CONTRACT shipped]**   THE DESIGN CALL: clock reads and random draws are TOOLS, never builtins — that single decision buys the whole reproducibility story (tool calls are traced + substituted under Substitute-mode replay, and `@deterministic` bodies reject them at compile time with ZERO new machinery; the determinism catalog stays deliberately empty). `std/time.cor`: `time_now_utc() -> TimeInstant` (epoch_ms + pre-rendered RFC 3339 iso + effect_meta), `time_monotonic_ms() -> Int` (differences only; origin = first read), `time_parse_iso -> Result<Int, String>` (malformed input is an Err, never a trap), `time_format_iso` (UTC, trailing Z, ms precision). Durations are plain Int milliseconds — checked arithmetic IS the duration API. `std/random.cor`: `random_float() -> Float` [0,1) (53-bit recipe over OS entropy) + `random_int(min, max) -> Int` INCLUSIVE both ends (Python randint contract) with rejection sampling (no modulo bias); min > max is a runtime error. Runtime dispatch mirrors the 33S1 exact-name-gate pattern (`is_stdlib_time_tool` / `is_stdlib_random_tool` in llm_dispatch.rs); vendor_std ships the new modules automatically. MATH: 12 pure kinds on the 45c table under the always-checked rule — Int `abs`/`min`/`max`/`pow` (abs(i64::MIN), pow overflow, and negative exponents TRAP), Float `abs`/`min`/`max`/`pow`/`sqrt` (negative sqrt TRAPS — no silent NaN poisoning) and `floor`/`ceil`/`round -> Int` (NaN/out-of-range trap like to_int_truncated; round is half-AWAY-FROM-ZERO, deliberately not Python's half-to-even). INVENTION CONTRACT: README catalog section (Executing Time & Randomness Surface), `corvid tour --topic deterministic-time` (compiles through the driver guard), inventions.md entry (Deterministic Time And Randomness), docs/reference/stdlib/time.md + random.md specs, and the LOAD-BEARING replay test (`replay_substitutes_recorded_time_and_random`: a fixed recorded instant + draw come back under replay — the live clock could never match them). 1 checker test + 1 driver e2e (math + ISO round-trip + monotonic + random ranges) + book ch 05 math section.
- [x] 45n-type-aliases-and-named-struct-literals   **[closed 2026-07-11 — did NOT split; also ships the destructuring-binding statements deferred from 45i]**   THREE forms on one shared `Type { }` surface. (1) TYPE ALIASES: `type CustomerId = String` — TRANSPARENT (the checker's `expand_possible_alias` hook in `named_type_to_type` makes `CustomerId` literally `String` everywhere; no newtype semantics), alias CYCLES error (`AliasCycle`, depth-guarded expansion), an alias is NOT a constructor (explicit diagnostic), and aliases-of-records construct via `Alias { ... }` because expansion runs before literal checking. (2) NAMED STRUCT LITERALS: `Decision { refund: true, amount }` — every declared field required (any order), shorthand `{ amount }` reads the local of that name (the RESOLVER binds it as a value read), duplicate/unknown/missing fields all error with the field named, `..base` update spread must be LAST and the SAME struct type, and builds a NEW cell whose fields share handles (the base is untouched — e2e-pinned). Positional `TypeName(v1, v2)` stays, and stays native; named literals are interpreter-only v1 (`IrExprKind::StructLiteral`, loud degradations on all compiled tiers). (3) DESTRUCTURING BINDINGS: `Decision { refund, amount: renamed, .. } = e` — the statement parser reinterprets a struct literal followed by `=` as a PATTERN (bare names + renames + `..` only; refutable shapes are a parse error pointing at `match`), the checker reuses 45i's `check_pattern` + `pattern_is_irrefutable`, and the interpreter reuses 45i's `pattern_matches` transactional binding (`IrStmt::Destructure`). A bare `..` inside a literal EXPRESSION is rejected by the checker. 2 checker tests (transparency + cycle + missing-field + wrong-type spread) + 1 e2e pin + grammar productions un-PLANNED with parse evidence (incl. destructure_stmt); book ch 05 flips BOTH Planned blocks (aliases + named literals/spread/destructuring) and ch 13 flips its deferred destructuring section — Phase 45's book debt is now ZERO Planned blocks outside 45q/45r scope.
- [x] 45o-effect-and-model-exports   **[closed 2026-07-11]**   `public effect x:` and `public model m:` are real: EffectDecl + ModelDecl gained module-level visibility, the parser accepts the `public` prefix, and `collect_public_exports` exports both kinds — so `import "./std/json" use json_egress_read` resolves (a PRIVATE effect stays unimportable, e2e-pinned). THE COMPOSITION HALF: imported PUBLIC effects join the importing file's EffectRegistry (built in checker.rs from local decls + root-import modules' public effect decls; LOCAL DECLARATIONS WIN on name collision — last-wins ordering, matching shadowing everywhere else), so `uses json_egress_read` composes dimensions exactly like a locally-declared effect. MODELS: route/requires/escalate/rollout/ensemble model-ref sites (6 in checker/prompt.rs) now accept a use-import whose target is a model via `entry_kind_is_model`; field-level validation (output_format, capability) runs where the model is DECLARED — the importing file trusts the defining module's own check (explicit v1 scope). EXTEND AUDIT: extend methods ride the underlying type's export and are not top-level exports themselves — no change needed (recorded in the modules.rs comment). MIGRATION: all 13 stdlib effect rows across io/http/db/json/time/random are now `public effect`; json.md documents that the typed-decoder convention can import `json_egress_read` instead of declaring its own row (declaring your own stays valid); book ch 06 visibility + re-export sections and the grammar visibility comment flipped. 2 driver e2e tests (public effect+model import compiles; private effect refuses) + parse evidence (`public effect` / `public model` in the statements snippet).
- [x] 45p-generics-decision   **[RESOLVED 2026-07-09 pre-phase chat: post-v1.0.]** User-declared generics need a type-variable representation the `Type` enum lacks entirely (`types.rs:20-145`) and would ripple through checker/IR/codegen — the largest single work item surveyed. The special-cased `List`/`Option`/`Result` heads + 45c builtin methods + 45r plain `fn` cover the v1.0 workload. The book's generic examples (`fn first<T>`) sit under a Planned heading (44a) until the post-v1.0 slice ships; entry added to the post-v1.0 roadmap section.
- [x] 45r-fn-pure-function-declarations   **[closed 2026-07-11 — CLOSES PHASE 45]**   Plain non-generic `fn name(params) -> Ty:` as the fourth callable kind. THE ARCHITECTURE CALL: `fn` gets its own decl kind through AST/resolver/checker (purity is a CHECKER guarantee), then lowers into the agent IR with `pure_fn: true` and call sites lower as agent calls — so every execution tier (interpreter, native, wasm, py) runs fns through machinery that already exists and already passes its gates; "no async machinery / direct call" is the existing agent call path's own optimization territory. PURITY WALK (`fn_check.rs`): rejects tool/prompt/agent/fixture/imported calls (callee NAMED in the error), `approve`, `ask`/`choose`, `replay` blocks, and `yield`, recursively through lambdas/match/literals; fn-to-fn calls compose freely. `@deterministic` bodies may always call fns (explicit DeclKind::Fn arm in the deterministic walker); fn calls bump NO weak effects (the call that leaves the effect row untouched). `public fn` exports through collect_public_exports like an agent; imported-callable calls INSIDE fn bodies are rejected (cross-module purity is invisible to the walk — recorded v1 scope). LSP completion + navigation ship fn symbols. 3 checker tests (compose + purity violation naming `fetch` + return-type mismatch) + 1 e2e (fn chain through @deterministic agent returns "PURE FUNCTIONS WORK"; IR entry asserts pure_fn) + fn_decl grammar production with parse evidence (incl. `public fn`); book ch 04 fn section flips to a compiling example (generic forms stay post-v1.0 per 45p) and the stale ch 04 Map-Planned note cleaned. NOT an invention-contract feature (standard language machinery per the direction lock).
- [x] 45q-parser-checker-papercuts-and-leniency-hardening   **[closed 2026-07-11 — all eight items]**   (1) `elif` — chose the PYTHON form per the decision principle (coherence with the Python-flavored surface beats novelty on a commodity feature); desugars IN THE PARSER to `else:` + nested `if`, so zero downstream stages know it exists. (1b) keyword-named annotations — `@retry` parses (the parser accepts keyword tokens as annotation names; `backoff:` needed the same treatment as an ARGUMENT name). (1c) named annotation args DECIDED + SHIPPED: `@retry(max_attempts: N[, backoff: linear|exponential MS])` and `@idempotency(key: param_name)` become real `AgentAttribute::Retry/Idempotency` variants; the checker validates max_attempts >= 1 and that `key` names a String/Int parameter; both lower into IrAgent metadata (retry_max_attempts/retry_backoff_ms/idempotency_key_param) next to is_replayable. EXPLICIT FOLLOW-UP (recorded, not hidden): `corvid jobs enqueue` reading annotation-derived defaults is the queue's follow-up; flags rule today and ch 10 says so. (1d) mock defect FIXED: the target may be a tool OR a prompt (prompt targets were rejected with E0203, making prompt mocks unusable); the signature check generalizes over both decl kinds; ch 15's Known-defect note replaced with the shipped rule. (2) unary `+` — checked like `Neg` (Int/Float only, `+"s"` errors naming unary `+`), then ELIDED at IR lowering (numeric identity, zero backend footprint). (3) doc comments `#:` — DECIDED post-v1.0: the rendering surface belongs with the LSP-hover work, so shipping the token alone would be a hidden no-op; ch 04 claim struck with the rationale, filed in the post-v1.0 section. (4) leniency hardening: unknown generic heads are now ERRORS with a Levenshtein did-you-mean (`Lst<Int>` -> did you mean `List`?; imported-module context stays lenient — validated by its own check); W0290 inference diagnostics (warnings, not errors) fire on unannotated `x = []` / `x = None` bindings with the exact annotation to write; the Unknown-assignable-to-everything AUDIT concluded: keep — it is the error-recovery spine (every checker error returns Unknown to avoid cascades), and the two real user-facing leak paths (generic heads, [] / None inference) are now closed at the source. Tests: 1 checker suite (annotation validation + did-you-mean + 2 W0290 + annotated-quiet + unary-plus rejection) + 1 e2e (elif/pos/mock end-to-end, returns "mid52.5", pins IR retry/idempotency metadata) + parse evidence for elif/`+`/named-arg annotations in the drift snippet; grammar if_stmt/unary_expr/annotation productions rewritten; book ch 04 (elif + doc-comment strike), ch 10 (annotations shipped), ch 15 (mock fix) flipped.

### Phase 46 — AI-native expressiveness — **CLOSED 2026-07-12** (8 slices, 2026-07-12; pre-phase chat 2026-07-11; B7/B8/B9 + M6/M8 + both quality minors closed)

Closes the three AI-native blockers (B7–B9) + the majors. The audit's framing: Corvid's *governance* of LLM calls is exceptional; the *expressive power* of the calls trails every mainstream SDK. These are the features an evaluator probes first.

**Agreed plan (pre-phase chat, 2026-07-11):** slice order **46a → 46b → 46c → 46d → 46g → 46h → 46e → 46f** — the a–d dependency chain (sampling → messages → history → streaming) in ascending size, self-contained RAG/eval next, design-heavy parallel + MCP last so the trace machinery is at its most mature. **46e:** design doc written at slice time and reviewed before implementation. **46f decision (decision principle):** CLIENT-ONLY, with full moat integration — every MCP-sourced tool gets a typed effect row, unknown MCP tools default to approval-required trust (safe by default, loosened explicitly), calls are budget-accounted + trace-recorded with replay quarantine; "MCP with governance" is the invention framing (a bare MCP client is commodity); serving Corvid tools over MCP is post-v1.0. **46h decision (decision principle):** repair is a PROMPT-BODY modifier (`repair: N` beside `min_confidence`/`max_tokens`) — per-prompt LLM-call quality policy lives at the same altitude as the streaming modifiers, while `@retry` stays an agent-level durable-job policy; repair attempts are cost-accounted against the effect budget and trace-recorded so replay reproduces the repair sequence deterministically.

- [x] 46a-sampling-parameters   **[closed 2026-07-12 — first Phase 46 slice]**   `SamplingParams { temperature, top_p, max_tokens }` on `LlmRequest`/`LlmRequestRef`, wired through ALL FOUR adapters (anthropic: top-level fields, max_tokens replaces the 4096 default; openai: `max_completion_tokens`; gemini: `generationConfig` merge that preserves the schema keys; ollama: `options` + `num_predict`). TWO SURFACES with clear precedence — model declarations (`temperature:`/`top_p:`/`max_tokens:` fields, range-checked at the declaration: temperature 0..=2, top_p 0..=1, max_tokens positive integer — `ModelFieldInvalid`) and per-prompt `with temperature F` / `with top_p F` modifiers (beside the existing `with max_tokens`; ranges enforced at parse). THE ARCHITECTURE CALL: model declarations became LOAD-BEARING at dispatch for the first time — `IrModel` lowers the sampling fields (Phase 20h's decl catalog had zero runtime lowering), and the VM resolves prompt-override > model-field > adapter-default in `resolve_sampling` at all three LlmRequest build sites (single-shot, ensemble, progressive). REPLAY FIDELITY: the resolved params are recorded in `TraceEvent::LlmCall.sampling` (serde-default — pre-46a traces stay readable; absent when every knob is default). Tests: 2 adapter body tests (params land / defaults omit) + 1 checker range test + 1 IR-precedence e2e + parse evidence (model fields + with-lines in the drift snippets); grammar model_field + with-modifier notes; book ch 11 gains the Sampling section with the precedence rule.
- [x] 46b-system-prompts-and-message-blocks   **[closed 2026-07-12 — closes audit B7 first half]**   `system:` / `user:` / `assistant:` role blocks in prompt bodies, exactly the form ch 11 documented. A prompt body is EITHER role blocks OR the single template — mixing is a parse error pointing at "give the final message a role"; at least one non-system message is required (Anthropic's constraint, enforced at parse). Each block's template interpolates `{param}` independently. PLUMBING: AST `PromptMessage` -> `IrPromptMessage` -> `LlmMessage` on `LlmRequest.messages` (empty = classic single-template form, zero behavior change). THE CANONICAL-STRING DECISION: `rendered` stays the single source of truth as the role-labeled concatenation (`[system] ...\n[user] ...`) — traces, cache fingerprints, token estimates, cites checks, and mock keying all keep working with ZERO new trace schema; adapters build provider-native shapes from `messages` (anthropic: top-level `system` string + messages array; openai: roles verbatim; gemini: `systemInstruction` + user/model contents; ollama: messages array). ESCALATION RULE: when the stream-upgrade path appends continuation text to `rendered`, `render_messages` detects the canonical prefix and appends the suffix as a final user message — nothing silently dropped. Tests: 2 adapter body tests (anthropic system extraction; openai roles verbatim) + 1 IR e2e (messages lowered + concat template) + role_clause parse evidence; ch 11 Planned block flips to a compiling example; grammar prompt_body/role_clause productions un-PLANNED.
- [x] 46c-conversation-history-first-class   **[closed 2026-07-12 — closes audit B7 second half; design doc at `docs/meta/46c-conversation-history-design.md`]**   THE SURFACE DECISION: history is a TYPED PARAMETER, not new syntax — a prompt param typed `List<AiMessage>` (syntactic recognition by type name; the std/ai import and local decls both work) splices between the declaration's system blocks and the current turn, in list order. Composes with every existing prompt feature for free (routing, ensembles, sampling, streaming). RULES: one history param per prompt (compile error), `{history}` interpolation is a compile error (splices structurally — JSON-dumping it is a bug), roles validated at dispatch (system/user/assistant, typed error naming the index). CANONICAL STRING (46b's decision extends): history renders into the role-labeled concat, so traces, cache fingerprints, token estimates, and mock keying stay coherent; the history list rides `args`, so replay matching is exact with ZERO new trace schema. CONTEXT-WINDOW POLICY: `context_window: N` on model declarations (lowered on IrModel next to the 46a sampling fields); at dispatch, budget = window − completion reserve (max_tokens or the default estimate); over-budget requests drop history OLDEST-FIRST (whole messages only; system blocks and the current turn are never dropped); still-over-budget is a typed error, not a silent provider 400. Truncation is a pure function of its inputs — replay reproduces it identically. SEGMENTED IMPLEMENTATION: `MessageSegments { system, history, turn }` in the VM; the single-shot path owns truncation; ensemble/adversarial paths use the flat form. NON-SCOPE (recorded): tool-call turns in history (post-46f), AiSession persistence, token-exact budgeting, smarter eviction. Tests: 1 checker suite (valid + interpolation + duplicate) + 1 VM splice-order unit test + 1 IR e2e (history_param index + context_window) + parse evidence; ch 11 gains the Conversation-history section.
- [x] 46d-real-provider-streaming   **[closed 2026-07-12 — upgrades audit M6 from fake to real]**   SSE token streaming in ALL FOUR adapters: a new `LlmAdapter::stream` method (default impl = whole call as one final chunk, so the mock and openai_compat keep working) with native overrides — anthropic (`stream: true`, content_block_delta/message_stop, provider `error` events surface as typed errors), openai (`stream: true`, `data: [DONE]`), gemini (`:streamGenerateContent?alt=sse`), ollama (JSONL with done flags) — all over a shared buffered line-splitter (`llm/sse.rs`) that survives UTF-8 splits across HTTP chunks. STRUCTURED OUTPUTS fall back to whole-call single-chunk in every adapter (partial tool_use JSON is not a streamable surface — recorded non-scope). REPLAY SEMANTICS: `TraceEvent::LlmResult.chunk_boundaries` records byte offsets where provider chunks ended (serde-default — old traces stay readable); `Runtime::call_llm_stream` in replay mode substitutes the recorded text and RE-CHUNKS at the recorded offsets, so a streamed run replays with identical chunk structure; live mode accumulates and emits the LlmCall/LlmResult pair with boundaries on completion. VM: plain-path `Stream<String>` prompts (no dispatch clauses, no mocks) stream live through a feed task — provider-reported usage/confidence on the final chunk are AUTHORITATIVE for the mid-stream token-limit and confidence-floor checks (the pre-46d tests pinning those behaviors now pass THROUGH the real streaming path unchanged); cost rides the final chunk exactly once; stream-setup failures fall back to the whole-call path. Everything else (dispatch clauses, structured outputs, mocked prompts) keeps whole-call-then-singleton. Tests: wiremock SSE integration test (three deltas + done through real parsing) + the two existing mid-stream termination tests now exercising the live path + 325 runtime lib tests; README stream-algebra caveat REWRITTEN to the shipped truth; adapter doc headers updated.
- [x] 46e-parallel-construct   **[closed 2026-07-12 — closes audit B8; design doc at `docs/meta/46e-parallel-design.md`, reviewed per the pre-phase agreement]**   `parallel:` blocks with NAMED ARMS (`name = call(...)`, two or more) — no tuples, no closures, nothing new to learn; arm names bind after the block. All arms start together and JOIN when all complete. THE THREE INVENTIONS THAT MAKE IT GOVERNED CONCURRENCY: (1) REPLAY — each arm runs against a BUFFERED per-arm tracer (`Tracer::buffered` + `Runtime::with_arm_tracer`, tracer-only clone that must NOT rebuild the recorder — schema headers would corrupt the parent trace); buffers flush IN ARM ORDER at the join, so the recorded trace is indistinguishable from sequential arm-order execution and replay works through the UNCHANGED sequential cursor with zero trace-schema changes (the load-bearing test records a concurrent run, asserts arm-ordered llm events, and replays byte-identically). (2) BUDGET — arm costs charge to the parent budget at the join, in arm order (the effect-spec parallel operator: cost Sum, latency Max, trust Max, confidence Min; cost.rs sums arms so `@budget` stays sound). (3) DETERMINISTIC FAILURE — the error rule is arm-ordered (first failed arm BY INDEX, not completion order); no cancellation in v1 (arms run to completion; their effects happened and are recorded). EXECUTION: `futures::join_all` over per-arm sub-interpreters (cloned env — shared cells stay shared per the 45b reference-semantics model; cloned mock table; budget snapshot) — concurrency without Send/'static machinery, matching the ensemble precedent. V1 RULES (checker-enforced): each arm is a CALL (wrap richer logic in an agent — the error says so); stream-returning arms rejected; fn purity rejects parallel blocks; `parallel` is CONTEXTUAL (no reserved-word breakage). Interpreter-only with loud degradations on all compiled tiers. Full invention contract: tour topic `parallel` (Governed Concurrency), inventions.md row, README catalog section, grammar parallel_stmt with parse evidence, book ch 10 section. Tests: join+bind e2e + the arm-order/replay round-trip + 216 syntax incl. the <2-arms parse rejection.
- [x] 46f-mcp-client   **[closed 2026-07-12 — closes audit B9; CLOSES PHASE 46; scope per the pre-phase decision: CLIENT-ONLY with full moat integration]**   "MCP with governance": ONE executing surface — `mcp_call(server_name, tool_name, args_json) -> Result<String, String> uses mcp_egress` in `std/mcp.cor` — makes external MCP tools subject to the entire moat. (1) UNTRUSTED BY DEFAULT: servers declared in `[mcp.servers.<name>]` (corvid.toml) route every call through the runtime Approver BEFORE any transport I/O (live-probed: `corvid run` hits the StdinApprover `approve? [y/N]` prompt); `trust = "autonomous"` loosens a server explicitly; DENIAL IS AN ERR VALUE naming the loosening path — recoverable, never a trap, and the test proves no transport I/O happens on denial. (2) REPLAY QUARANTINE FOR FREE: `mcp_call` is a standard tool dispatch, so tool_call/tool_result events record every call and substitute-mode replay never contacts a server and never prompts (the replay branch in call_tool runs before dispatch). (3) BUDGET-VISIBLE via the mcp_egress effect row. TRANSPORTS: stdio (spawned child, newline-delimited JSON-RPC with the initialize handshake, connection cached per server + respawned on failure, blocking I/O off-executor) and HTTP (one JSON-RPC POST per call). MCP `tools/call` content blocks concatenate to text; `isError` surfaces as Err. PARAM-NAME REALITY: `server`/`tool` are reserved keywords — the tool declares `server_name`/`tool_name`. Config: `[mcp.servers.<name>] command|url + trust` parsed in corvid-types, wired in the driver. Tests: wiremock JSON-RPC round-trip + approval-denial (asserts the Err message AND zero transport I/O) + tool-side isError + extract_content units. Full invention contract: tour topic `mcp` (MCP With Governance), inventions.md row, README catalog section, docs/reference/stdlib/mcp.md; FEATURES.md deferred-marker re-scoped (client shipped; server stays post-v1.0). NON-SCOPE (recorded): per-tool typed imports (compile-time introspection), `import mcp` sugar, SSE server-transport streaming, tools/list discovery CLI.
- [x] 46g-rag-stdlib-dispatch   **[closed 2026-07-12 — closes audit M8; FULL INVENTION CONTRACT shipped]**   "Governed Retrieval" — retrieval with the moat attached, the part every RAG framework leaves to convention: (1) INDEX PATHS resolve through the SAME `[io] root` policy as file I/O (fails closed — e2e-pinned: no root => `rag_ingest` returns Err, not a policy hole); (2) HONEST FAILURES — missing index / bad args / embedder errors / policy rejections are `Result::Err` values, never traps (the 47h envelope direction, shipped from day one on this surface); (3) PROVENANCE ON EVERY CHUNK — each retrieved RagChunkEnvelope carries provenance_key + effect_meta as ordinary checkable values; (4) REPLAY — the tools record/substitute as ordinary tool events, so the embedder NEVER fires on replay (the 45m quarantine pattern, no new machinery); (5) HONEST DEGRADATION — no `[rag]` embedder in corvid.toml => TERM-SCORED lexical search over the same index (dispatch-level scoring: per-term LIKE + hit-count ranking — the raw index method is a phrase LIKE, too strict for multi-word queries), identical program behavior with lower recall. PLUMBING: `[rag]` corvid.toml table (embedder openai|ollama, model, endpoint) -> `load_rag_embedder` in the driver (OPENAI_API_KEY from env; ollama endpoint override) -> `Runtime.rag_embedder` builder; `is_stdlib_rag_tool` gate + `dispatch_stdlib_rag_tool` (the 5th executing surface); ingestion uses the existing `chunk_document` chunker (20% overlap) + `RagSqliteIndex` + `insert_embedding_vectors`; search uses cosine `search_embeddings` when embedded. V1 SCOPE DECISION (recorded in std/rag.cor + spec): `rag_read` does NOT carry `data: grounded` — the Grounded<T> wrapper at the import boundary trips the cross-module provenance-composition gap (already post-v1.0); provenance travels EXPLICITLY in envelope values instead. Contract: README Governed Retrieval section, `corvid tour --topic governed-retrieval`, inventions.md row, docs/reference/stdlib/rag.md, 2 driver e2e tests (ingest+search returns "RAG RETRIEVES"; fail-closed policy).
- [x] 46h-structured-output-repair-and-judge-assertions   **[closed 2026-07-12 — both audit minors]**   (1) STRUCTURED-OUTPUT REPAIR — `with repair N` prompt-body modifier (the pre-phase surface decision: per-prompt LLM-call quality policy lives beside min_confidence/max_tokens, NOT at the @retry agent altitude): on a typed decode failure the runtime re-asks with the FAILED RESPONSE + the exact validation error appended, up to N extra attempts. Every attempt is a fully traced LLM call (LlmCall/LlmResult pairs — replay consumes the recorded sequence deterministically); failed attempts still cost and the accumulated waste lands on the final result's cost, so `@budget` sees the truth; exhausted repair surfaces the ORIGINAL typed Marshal error (never a silent wrong answer). Proven by a flaky-adapter test (first response violates the schema, second matches — returns "repaired") + a zero-repair test pinning the error path. (2) QUALITY ASSERTIONS — `assert similar <expr>, <expr> min F` (deterministic word-set Jaccard, zero LLM cost — the cheap regression gate; failure prints the score AND both texts) and `assert judged <expr>, "criteria" min F` (LLM judge scoring 0..1 through the NORMAL call_llm path — traced, cost-accounted, mockable, so eval --max-spend accounting and replay both see it; structured score schema; no numeric score = assertion error). Thresholds range-checked at parse (0..=1). Evals and tests share the runner, so both surfaces get both forms; the eval report gains a `quality: N/M passed` bucket. Live-probed: a deliberate similarity failure prints `similarity 0.000 is below min 0.7 (actual: ..., expected: ...)`. Grammar assertion + with-modifier productions updated with parse evidence; book ch 11 (repair) + ch 15 (quality assertions) sections added. NOTE: `Judgement` in std/agent.cor remains an envelope — the EXECUTING judge surface is the assertion form (recorded scope: the envelope's disposition rides 47f's per-module audit).

### Phase 47 — Batteries parity + scaffold honesty (~3–5 weeks) — **pre-phase chat held 2026-07-12**

The stdlib/parity majors that don't belong to 45/46.

**Agreed plan (pre-phase chat, 2026-07-12):** slice order **47a → 47b → 47h → 47f → 47g → 47c → 47e → 47d** (scaffold pair first, envelopes before the module audit that reuses their patterns, decisions and tests after, the scheduler runner last as the biggest unknown). **47c decided:** batteries on compiled tiers are POST-v1.0 — the tier-picker auto-fallback is the designed mitigation; this slice ships the tier-matrix doc + loud transpile-time failures. **47d decided (no-shortcuts bar):** ship the MINIMAL SCHEDULER RUNNER — "governed cron": `corvid schedule run` drives the existing durable-jobs queue so scheduled agents inherit tracing, budgets, and replay; documenting the limitation would have been the shortcut. **47g decided (no-shortcuts bar):** option (a) — DERIVE the approve requirement from the trust tier (`supervisor_required`/`human_required` in a tool's composed row ⇒ approve required at call sites, same machinery as `dangerous`). Breaking but safer; the warning would have been the half-measure, and the silent-footgun shape (declared high trust, forgotten `dangerous`, zero protection) is exactly what the language exists to prevent. Migration cost is honest: high-trust call sites need `approve`, and the error names the deriving effect + dimension.

- [x] 47a-no-python-default-scaffold   **[closed 2026-07-13 — closes audit M11]**   The default `corvid new` scaffold is PURE CORVID: the starter agent imports `./std/time` and greets with the current instant through an EXECUTING, replay-traced stdlib tool — the language's whole promise live in the first program (`agent main()` zero-arg so `corvid run src/main.cor` works immediately; live-probed end-to-end: scaffold → run → "Hello, Corvid! It is 2026-07-13T13:09:09.593Z"). `tools.py` + a `src/python_tools_example.cor` declaration example move behind `corvid new --with-python-tools`; the post-scaffold output text now tells the pure-Corvid story by default and the Python story only when opted in. Tests: default-scaffold structure asserts NO tools.py + executing-surface main.cor + zero-arg entrypoint; opt-in template test; the Gap-#1 vendor round-trip test's synthetic stdlib gained a compatible time.cor so the scaffold's import typechecks in the round-trip. LIVE FINDING for 47b: probing against the INSTALLED `~/.corvid/std` (stale, pre-45m) broke the starter's `./std/time` import — exactly the vendoring-staleness gap 47b exists to close; `corvid upgrade` refresh must also cover this.
- [x] 47b-std-vendoring-hardening   **[closed 2026-07-13 — closes audit M12; the staleness gap was live-confirmed during 47a's probe]**   (1) The silent vendor no-op is GONE: `vendor_std` returns a `VendorOutcome` (Vendored/AlreadyPresent/NoSourceFound) and `corvid new` prints a LOUD warning with the exact remediation on NoSourceFound (set CORVID_HOME, then `corvid upgrade refresh-std <project>`) — live-probed with a bogus CORVID_HOME. (2) `corvid upgrade refresh-std <project>` refreshes the vendored `src/std/` from the current install: per-module added/updated/unchanged report; stale modules are OVERWRITTEN (the vendored stdlib is not a user-edit surface; git history preserves anything lost — stated in the subcommand help). Live-probed against a synthetic stale project: the pre-45m module set gained agent/ai/db/json/time/mcp/... in one command. Tests: refresh report precision (added vs updated vs unchanged + actual rewrite) + the existing never-overwrite-on-new guard migrated to the VendorOutcome shape.
- [x] 47c-codegen-parity-decision-and-loud-degradation   **[closed 2026-07-14 — closes audit M10; decision per the pre-phase chat: batteries on compiled tiers are POST-v1.0, the tier-picker auto-fallback is the designed mitigation]**   (a) CANONICAL TIER MATRIX: docs/book/16-building.md now opens with the execution-tier table (interpreter = reference tier, everything runs; native/cdylib/wasm and the legacy Python transpile tier each state exactly what they carry and that they refuse LOUDLY — never silently degraded output); the stale "default: native" comment fixed to reality (the CLI's build default is the legacy python transpile). (b) LOUD TRANSPILE REFUSAL: new `corvid-codegen-py::stdlib_scan` — `STDLIB_EXECUTING_TOOLS` (all 30 tools across io/http/db/json/mcp/rag/random/time) + an IR walk collecting every stdlib tool call with its span; the driver's two transpile pipelines refuse before emit with a per-call diagnostic naming the tool ("cannot be transpiled to Python — the transpile tier has no stdlib dispatch...") and a hint routing to `corvid run` + the tier matrix. The object-shaped degradations (DbHandle/JsonValue `object` hints, unregistered `tool_call(...)` stubs) are unreachable for stdlib programs because the refusal fires first. ANTI-DRIFT: a codegen-py test parses the repo's `std/*.cor` sources and asserts the tool list matches exactly — a stdlib slice that adds a tool without updating the scan fails CI. (c) runtime/python README opens with a Scope callout: transpile tier only, no stdlib dispatch, interpreter is where stdlib programs run. Pinned by a driver test (refusal message + hint) + live-probed both directions (`corvid build --target=python` refuses with the exact call span; `corvid run` on the same program dispatches through the SSRF boundary). FILED: whether the CLI build default should flip from legacy python to native now that the matrix is canonical — small decision, rides the next audit pass.
- [x] 47d-schedule-execution-decision   **[closed 2026-07-14 — closes audit M16; the pre-phase decision shipped: the MINIMAL SCHEDULER RUNNER, "governed cron"]**   New top-level `corvid schedule run --source <file> [--state --workers --poll-ms --lease-ttl-ms --max-runtime-ms --max-missed-per-schedule]`: parses the source AST for `schedule` declarations (they are audit/runtime manifests, deliberately not IR), registers each as a durable-queue `QueueScheduleManifest` (STABLE id `sched_<target>_<n>` so re-running after an edit upserts cron/args in place while the fire cursor + dedup table carry over; replay_key_prefix = the id; missed_policy = fire-once-on-recovery), then runs a scheduler tick thread (each tick = the same `recover_schedules` primitive `corvid jobs schedule recover` uses — idempotent per (schedule, fire-ms), DST-aware, missed-fire-policy honoring) alongside the existing worker pool. Scheduled agents inherit EVERYTHING the durable queue gives directly-enqueued jobs: interpreter execution, per-job JSONL traces for `@replayable` agents, retries with backoff, dead-letters, budgets metadata. BRIDGE FIX: schedule fires wrap payloads in the `{schedule_id, scheduled_fire_ms, payload}` envelope but `DefaultJobRuntimeExecutor` expected a bare args array — the executor now unwraps the envelope (unit-pinned), closing a latent mismatch that would have failed every cron-fired job with PayloadShape. SEMANTIC LINE: schedule args must be LITERALS — the manifest is a durable artifact evaluated at fire time with no enclosing frame; the refusal names the argument and the fix (move computation into the agent). W0280 REWORDED from "the cron will NOT execute" (now false) to "fires only under `corvid schedule run` — a plain `corvid run` executes main once", with the hint pointing at the runner. Live-probed: an every-second cron fired 4/4 successful jobs in a 4s bounded run; non-literal args and schedule-less sources refuse with actionable errors. 3 CLI e2e tests (fires-through-pool with succeeded>=1 + failed=0, literal-args refusal, empty-source refusal) + the executor envelope unit test. Docs: guides/jobs.md runner section (both runners, one queue), README "Governed Cron" entry. v1 non-scope: no daemonization/PID management (bounded runs or Ctrl-C; process supervision is the host's job), literal args only.
- [x] 47e-hardening-tests   **[closed 2026-07-14 — original scope + the filed python-smoke item, which turned out to be a REAL C-ABI hole]**   (1) PYTHON-SMOKE DIAGNOSIS → PORTABLE ALLOCATOR: the empty-stdout crash was heap corruption — the CorvidToolFn contract requires callback result buffers from the cdylib's OWN Rust allocator (reclaimed via `CString::from_raw`); host `malloc` happens to match on linux-gnu but on Windows Rust allocates from the process heap and the CRT-malloc'd buffer corrupted it. Fixed for real, not skipped: new `corvid_alloc_result(content_len)` export on the catalog C API (space-filled, NUL-terminated, exactly the from_raw shape; abandon via `corvid_free_result`), declared in the generated C header alongside newly-declared `corvid_register_tool`/`corvid_clear_tools`/`CorvidToolFn` (previously exported but undeclared), kept alive in the cdylib export list, surfaced in the python bindings as `Client.make_result(content)`, and the integration smoke migrated off msvcrt malloc — `host_bindings_integration` passes ON WINDOWS now. Two bridge unit tests pin the round-trip + zero-len reclaim, and the bridge test module gained a test-serialization guard (the registry is process-global; the pre-existing intermittent failures were exactly that race). (2) INT-OVERFLOW PINNING: `i64::MAX + 1` traps naming overflow; `@wrapping` wraps to `i64::MIN`; `1/0` traps naming division by zero — the book's checked-arithmetic claims now have e2e teeth (there is no release-mode saturation and no `--overflow` flag). (3) NUMERIC-CONVERSION EDGES: out-of-range Float→Int truncation traps (45e's NaN trap already had coverage). (4) RECURSIVE STRUCTS: first-ever coverage — self-recursive `Node { value, next: Option<Node> }` builds/traverses through the interpreter, and MUTUALLY recursive structs (Ping/Pong via Option) typecheck and run. All six pins in the new `hardening_e2e.rs` driver suite. Codegen struct-layout coverage for recursion stays with the compiled-tier parity track (post-v1.0 per 47c) — the tier matrix already states compiled tiers refuse what they can't carry.
- [x] 47i-executing-secrets-and-cache   **[promoted to slice 48a at the 2026-07-14 Phase 48 pre-phase chat — tracked there; design decided at that chat]** The two contract modules whose runtime counterparts justify a real executing surface. (1) SECRETS: `secret_read(name) -> Result<SecretReadEnvelope, String>` over `secrets::SecretRuntime` is blocked on one genuine design problem — executing tools record their results into traces, and traces must NEVER persist secret values, yet Substitute-mode replay substitutes from recorded results. A replay-safe design must pick: redacted recording + re-read-on-replay (breaks strict determinism honestly), value-channel separation (envelope in the trace, value never serialised — a `Value`-side secret handle like DbHandle), or sealed traces. Inventive opportunity: no language has replay-safe secret access; this is a moat composition problem, not a marshalling problem. (2) CACHE: `cache_put`/`cache_get`/`cache_invalidate` over `cache::CacheRuntime` — provenance-keyed caching of expensive results composes with budgets (cost savings visible to `@budget`) and replay (cache hits are deterministic); design must decide what is cacheable (String? JsonValue? any encodable T?) and how invalidation keys surface in the language.
- [x] 47g-trust-dangerous-coupling-decision   **[closed 2026-07-14 — option (a) shipped per the pre-phase decision: DERIVE the approve requirement from the trust tier]**   A tool whose composed effect row carries `trust: supervisor_required` or `trust: human_required` now requires `approve` at every call site, exactly like `dangerous` — same lexical machinery, both the local and the imported-tool call paths. The derived case gets its OWN diagnostic (`UnapprovedHighTrustCall`) naming the deriving effect AND tier, with a help that offers both honest fixes (add the approve, or lower `trust:` if the tier overstates the tool's authority), and its OWN guarantee row `approval.trust_tier_requires_token` (static/typecheck; core-semantics regenerated). RULES: `autonomous` and `autonomous_if_confident(...)` (typechecks as autonomous, escalates at runtime) derive nothing; a tool that is BOTH dangerous and high-trust reports only the established dangerous diagnostic (one obligation, one error); the `approval.token_lexical_only` out-of-scope discrimination applies to derived obligations identically. SCOPE: tool call sites only — prompts/agents with high-trust effects interact with the `@trust` ceiling and runtime escalation instead (an LLM render is not an action; a tool call is). MIGRATION was honest and small: exactly two checker tests called a supervisor-required tool without approve (both gained `approve LogRefund(id)`); every example app's high-trust tool already carried `dangerous` + approve, so zero example/book/corpus churn (the corpus high-trust fixtures use PROMPTS, out of scope by design). 5 new pinning tests (derived error + names/id, approved-clean, autonomous-clean, dangerous-precedence). Book ch07 (trust bullet states the derivation; stale trust-tier examples in the dimension table fixed to the real `autonomous`/`supervisor_required`/`human_required` set) + ch08 (protected = declared OR derived, with the derived error rendered). Live-probed both directions through `corvid check`.
- [x] 47h-stdlib-result-envelopes   **[closed 2026-07-13 — filed by the 45l envelope audit]**   All eight executing tools migrated to honest `Result<_, String>` returns: `io_read_text -> Result<FileReadEnvelope, String>` / `io_write_text -> Result<FileWriteEnvelope, String>` / `io_list_dir -> Result<List<DirectoryEntryEnvelope>, String>`, `http_get` / `http_post_json -> Result<HttpResponseEnvelope, String>`, `db_open -> Result<DbHandle, String>` / `db_query -> Result<List<DbResult>, String>` / `db_execute -> Result<DbResult, String>`. SEMANTIC LINE (documented in each std module): recoverable failures — policy refusals, missing files, transport errors, SQL/binding errors — are Err VALUES naming the cause; malformed argument SHAPES stay hard errors (the checker prevents them). HTTP rule: an error STATUS (4xx/5xx) is still Ok — the transport succeeded; inspect `status`. MECHANICS: shared `stdlib_result_ok`/`stdlib_result_err` JSON marshalling in llm_dispatch for io/http; db rides the VM typed-Value path (`Value::ResultOk`/`ResultErr` wrapping in `dispatch_stdlib_db_tool` — DbHandle can't ride JSON), decoding the ok payload against the declared `Result<T, _>`'s T. SECURITY UNCHANGED: the boundaries are as hard as before — the three policy-refusal e2e tests (SSRF, missing allowlist, `[io] root` escape) now assert the FULL diagnostic survives into the Err payload; guarantee registry rows note the Err-value surface (core-semantics regenerated). String error type (not typed enums) is the deliberate v1 shape — it matches `json_parse`/rag/mcp and the typed-decoder convention; typed error enums remain open on the 45h/45i track. CONSUMERS: 9 driver e2e tests + 5 runtime io tests migrated to Result shapes (`?` at every step); book ch02/ch06/ch18, stdlib reference io/http/db/json/README, repo README, and the file-io/sqlite/http-client tour topics all teach the Result idiom. LIVE-PROBED (6 probes through `corvid run`): missing file names the real OS cause; io+db pipeline with `?` at every step returns Ok(0); `[io] root` escape, SQL error, and SSRF (with 127.0.0.1 deliberately allowlisted) all return Err values carrying the full boundary diagnostic. BONUS FIXES found by the probes: (1) a relative `corvid run src/main.cor` invocation produced a relative `[io] root` anchor that false-fired the confinement check on EVERY path (the whole io/db surface was unusable from the CLI; quickstart's own example) — `IoToolPolicy::new` now anchors any still-relative root against the CWD, pinned by a policy test; (2) the stale `build_cdylib` non-scalar fixture (flat scalar-field struct, legal since 33Q8) replaced with a nested struct + current-phrasing assertions.
- [x] 47f-contract-module-disposition   **[closed 2026-07-14 — per-module decisions recorded; one deletion; two executing candidates filed as 47i]**   Every non-executing std module now carries an explicit disposition header, and the stdlib README opens with a full 16-module disposition table (Executing / Contract-only / Pure vocabulary + the runtime counterpart per row). DECISIONS: `effects.cor` PURE VOCABULARY (permanently — the shared type layer everything imports); `agent.cor` + `ai.cor` PURE PATTERNS (the types ARE the feature; the executing surfaces are `assert judged` and `prompt`/`call_llm` respectively — ai.cor added to the audit since it is envelope-only too); `approvals.cor` CONTRACT-ONLY **BY DESIGN** (the principled row: approval decisions are made OUTSIDE the program — an agent that could list/decide its own queue could approve itself, exactly what the moat prevents; in-language flow is the `approve` statement; envelopes serve host tools that REPORT queue state); `auth.cor` CONTRACT-ONLY (names `auth::SessionAuthRuntime` + `corvid auth` and the tool-wrapper pattern); `jobs.cor` CONTRACT-ONLY (names `queue::DurableQueueRuntime` + `corvid jobs`; in-language enqueue is a candidate after 47d's scheduler runner); `observe.cor` CONTRACT with a specific role (the typed shape of the `std.observe.summary` host event — observation flows runtime→program, so no tool belongs here); `secrets.cor` CONTRACT-ONLY for a DESIGNED reason (an executing `secret_read` would record its result into the trace, and traces must never persist secret values — the envelope deliberately has no value field); `cache.cor` CONTRACT-ONLY today (`cache::CacheRuntime` exists; executing surface filed). DELETED: `queue.cor` — superseded by `jobs.cor`, zero consumers outside its own guard tests (removed with it); README row records the deletion so it is not silent. The two (a)-candidates worth real design get slice 47i below instead of a rushed corner of an audit slice.

---

### Phase 48 — Pre-launch close-out (order agreed with the CTO, 2026-07-14)

**Pre-phase decision (2026-07-14):** finish ALL remaining phase work before starting the launch phase. The launch phase (Phase 33's 33J website / WASM playground / runtime core-host split / benchmark page / blog track, plus the externally-gated 42/43 items — external reviewer signoff, 20-developer beta, friends-and-family round — which need the public surface to exist first) starts only when this queue is empty. Phases 37–40 are fully closed; Phase 17's `17b-*` RC-optimization innovations stay deferred performance work outside the launch criteria.

Queue, in order:

- [x] 48a-executing-secrets-and-cache   **[closed 2026-07-14 — both surfaces shipped with the full invention contract]**   (1) REPLAY-SAFE SECRETS: `secret_read(name) -> Result<SecretReadEnvelope, String>` (envelope gained the `value` field) — the program receives the REAL value; the recorded ToolResult carries a REDACTED copy (`<redacted:XY>` marker + `value_redacted: true` — new RuntimeChecked guarantee `secrets.trace_never_carries_value` with anchor + registry row + regenerated core-semantics); Substitute-mode replay RE-READS the live environment instead of substituting (the secrets branch bypasses the replay source in `call_tool` — the db_query read-passthrough rule), pinned by a replay-corpus test proving a replay-mode runtime observes the LIVE env value with no ReplayDivergence. A missing secret is Ok with `present: false` (absence is modeled, not an error). Residual channel stated everywhere (module header, spec, README, tour non-scope): forwarding a secret into another tool's args records it in THAT tool's events — the structural SecretHandle taint is the post-v1.0 deepening. (2) PROVENANCE-KEYED CACHE: `cache_put` / `cache_get` / `cache_invalidate` / `cache_invalidate_provenance` over a new `Arc<Mutex<CacheRuntime>>` shared across Runtime clones — (namespace, subject) addressing with ONE entry per address (evict-before-put so writer-side key metadata never accumulates ambiguous duplicates; `CacheRuntime` gained `get_by_namespace_subject` + `evict_namespace_subject`), misses are Ok `hit: false` envelopes, provenance eviction drops everything derived from a changed source across namespaces (e2e-pinned: 2 evicted, unrelated survivor stays), and all four tools record/replay-substitute as ordinary tool events. CONTRACT: docs/reference/stdlib/secrets.md + cache.md, stdlib README rows flipped to Executing (also fixed the 47f table's accidentally-missing cache row), README catalog entries (Replay-Safe Secret Access + Provenance-Keyed Cache), two tour topics (`replay-safe-secrets`, `provenance-cache` — compile-gated), two inventions.md rows, codegen-py anti-drift list extended (transpile refusal covers the 5 new tools automatically). 6 driver e2e tests + 1 replay-corpus test; live-probed through `corvid run`: secret → cached → provenance-evicted → miss in one program.
- [x] 48b-connector-grounded-returns-disposition   **[closed 2026-07-14 — decided: the middle option, honestly scoped]**   The opt-in grounding pattern ships today (a `data: grounded` effect on a connector-backed tool auto-wraps returns — checker-pinned both directions), the guide documents it, and the aspirational "strips fails typecheck" clause is corrected to the shipped tracked-legacy-coercion semantics (IR discard node, corpus-verified). The universal connector-side default wrap stays with post-v1.0 `35V2-P41-I`. Phase 41's open provenance item is closed with the disposition recorded in place.
- [x] 48c-ai-helper-sweep   **[closed 2026-07-14 — per-helper dispositions recorded in place]**   Both phase checkboxes close via their own explicitly-offered "follow-ups filed" branch, with honest per-helper rationale instead of a blanket deferral: `mock-fixture-gen` / `fail-sim` / `deploy tailor` / `upgrade assist` re-filed post-v1.0 (each is an agentic authoring accelerator over a deterministic surface that shipped complete; live-LLM authoring loops can't be honestly CI-validated offline; no guarantee gaps), and `beta synthesize-feedback` moved to the LAUNCH phase — it consumes beta feedback that won't exist until 43H runs. Zero helpers silently dropped: four in the Post-v1.0 list with reasoning, one riding the beta items.
- [x] 48d-imported-struct-4-disposition   **[closed 2026-07-14 — confirmed unreached; closed as explicitly post-v1.0 with the loud refusal pinned]**   No pre-launch surface reaches the ImportedStruct entry boundary (scalar extern entrypoints, no prompts in the reference apps); the codegen refusal is loud with the file-local-alias workaround and is now test-pinned. The lowering rides the tracked cross-file native layout follow-up. THE PHASE 48 QUEUE IS EMPTY — the launch phase is next, per the 2026-07-14 pre-phase decision.

### Phase 49 — Capability surface: skills, MCP ergonomics, connector scaffolds (pre-launch; pre-phase chat held 2026-07-14)

**CTO decision (2026-07-14):** ships BEFORE the launch phase, "in an inventive, powerful way and easier to use than any other language available"; skill shape and distribution delegated ("choose the best one, no shortcuts... innovative and inventive").

**Design decisions (recorded at the pre-phase chat):**

1. **A skill is an effect-audited, vendored package** — the merged best of "package" and "recipe": `corvid add skill <source>` vendors VISIBLE, EDITABLE Corvid source into `src/skills/<name>/` (the std-vendoring philosophy; git-diffable, no hidden code) carrying a `skill.toml` manifest that declares the skill's **capability label**: composed effect ceiling (cost / trust tier / data class), external reach (hosts, paths), exports, and required config (env secrets, corvid.toml sections).
2. **The label is enforced twice.** At add-time: a rendered consent audit (the "nutrition label" — what the skill can reach and at what trust, BEFORE any code lands). At every compile: the checker verifies the skill's ACTUAL composed effects fit the declared ceiling — including after local edits — and fails loudly naming the excess dimension. Editable skills that cannot silently outgrow their label; no other agent-skill system (Claude skills = prose+scripts, MCP servers = opaque toolboxes) can make this claim.
3. **Signed skills without a registry.** Reuse the DSSE attestation machinery from cdylib signing: skills MAY carry a signature; `corvid add` verifies and names the signer; unsigned installs work behind a loud UNSIGNED banner. Sources: `./path`, `git:<url>[#ref]`, `github:org/repo[@ref]`, with content-hash pinning recorded in the lockfile for reproducible re-adds and `corvid skill update`. The hosted registry remains post-v1.0.
4. **MCP ergonomics**: `corvid add mcp <name> --cmd|--url [--trusted]` writes the `[mcp]` entry, connects, discovers tools, and GENERATES a typed module `src/mcp/<name>.cor` — one typed tool per discovered MCP tool (JSON schema → envelope types, Result returns, per-tool effect rows; untrusted stays approval-gated) so calls are typed instead of stringly `mcp_call`.
5. **Connector scaffolds**: `corvid add connector <provider>` generates the tool-wrapper module + corvid.toml scopes + env checklist for each SHIPPED connector. The third-party connector SDK remains post-v1.0.

- [x] 49z-verify-temp-leak   **[closed 2026-07-14]**   Two-part fix in `corvid-differential-verify`: (1) each fixture's built native binary — the heavy artifact — is deleted immediately after its run (the trace stays for debugging); (2) a once-per-process best-effort sweep removes `%TEMP%/corvid-verify` entries older than 24h, keeping recent runs debuggable and growth bounded. Measured: a full corpus verify now leaves 194K of traces instead of hundreds of MB of binaries.
- [x] 49a-skill-format-and-audit   **[closed 2026-07-14 — the nutrition label is live]**   `skill.toml` manifest (meta + capability label: `uses` groups, `max_trust`, `max_cost_usd`, `data`; declared reach + required env) in `corvid-driver::skills`. The audit is COMPUTED from source, never self-reported: capability groups by token-level scan of stdlib executing-tool names (deliberately over-approximating — mention counts as use, the conservative direction), trust/cost/data ceilings from parsed `effect` declarations, prompt decls/`call_llm` → the `llm` capability, `dangerous` tools surfaced on the label, unparsable files REFUSE the audit. `corvid add skill <path> [--yes]` verifies label-covers-source (a dishonest label refuses install), renders the label + UNSIGNED banner, requires consent (TTY prompt; non-TTY without `--yes` refuses), vendors into `src/skills/<name>/`. The second enforcement half: `corvid check` and `corvid run` re-verify every vendored skill and fail naming the exceeded dimension — live-probed: an installed cache-only skill ran end-to-end through main.cor, then a post-install edit adding `http_get` flipped check to "capability `http` is used but the label allows only [cache]". BREAKING (recorded): package adds moved under the unified capability verb — `corvid add package <spec>` (was `corvid add <spec>`); no hosted registry runs, usage was internal-docs-only. Tests: 6 driver unit tests (honest pass, capability/trust/cost violations named, edited-vendored-skill caught, dishonest-label refusal) + 2 CLI help pins. Skill authors write imports for the INSTALLED layout (`../../std/<module>` from `src/skills/<name>/`) — documented in 49e's guide. Signature verification + git sources + update ride 49b.
- [x] 49b-signed-skills-and-sources   **[closed 2026-07-14 — registry-free trust chain live]**   (1) SIGNED SKILLS over the existing DSSE machinery (`corvid-abi` ed25519): the signed payload is the skill's canonical CONTENT MANIFEST (name/version + sha256 per file), so one verification proves publisher identity AND content integrity — a post-signing edit refuses at add time (live-probed: "content does not match its signature — modified after signing"). `corvid skill sign <dir> --key|CORVID_SIGNING_KEY` writes `skill.sig` and prints the hex VERIFYING key to distribute; `corvid add skill --publisher-key <path>` verifies (label shows "signed — publisher key `<id>` verified"); signature-present-but-unverified and unsigned each get their own loud banner. (2) SOURCES: `github:org/repo[/subdir][@ref]` + `git:<url>[#ref][//subdir]` via shallow clone (the `//` subdir separator is scheme-safe), local paths unchanged; tested against a REAL file:// git repo. (3) THE PIN: `skill.lock` inside the vendored dir (source string + consented content hash + signer key id — travels with the project; Corvid.lock stays package-only until the registry story). `corvid skill update <name>` re-fetches the pinned source, hash-diffs (up-to-date short-circuits), refuses name-swapped updates, re-audits + re-renders the label for FRESH consent on change, then re-pins — live-probed both directions. Tests: sign/verify/tamper roundtrip, source parsing (github shorthand, scheme-safe subdir), real-git resolution, update up-to-date/changed/apply cycle.
- [x] 49c-mcp-add-and-typed-codegen   **[closed 2026-07-14 — stringly MCP is over]**   (1) `McpRuntime::list_tools` (tools/list over both transports, mirroring tools/call). (2) `corvid add mcp <name> --cmd ...|--url ... [--trusted]`: discovers the server's tools FIRST (config only lands for a server we actually reached), generates `src/mcp/<name>.cor` — one typed public agent per tool from its JSON `inputSchema` (string→String, integer→Int, number→Float, boolean→Bool; params built with the std/json BUILDER so escaping is runtime-handled, never string-concat; schemas outside the v1 mapping fall back to one `args_json: String` param with the reason in the generated comment; hyphen/keyword/digit-leading tool names sanitize deterministically) — then appends `[mcp.servers.<name>]` to corvid.toml (untrusted by default with the approval note; `--trusted` writes `trust = "autonomous"`). (3) `corvid mcp regen <name>` refreshes the module from the configured server. Untrusted approval gating is unchanged — generated wrappers delegate to the governed `mcp_call`. Live-probed against a stdio mock MCP server: add discovered 2 tools, the generated `echo_shout`/`add` wrappers ran end-to-end through the interpreter (`Ok("CORVID SHIPS TYPED MCP = 42")`), regen round-tripped. Tests: typed-wrapper + fallback + empty-schema generation golden, generated-module-compiles, sanitization pins.
- [x] 49d-connector-scaffolds   **[closed 2026-07-14 — manifest-derived, not hand-curated]**   `corvid add connector <provider>` generates `src/connectors/<provider>.cor` FROM THE SHIPPED MANIFEST: each scope becomes a `public effect` whose dimensions reflect the manifest (approval-required scopes render `trust: human_required` + `reversible: false`; data-class-carrying scopes render `data: private` with the actual classes in the comment), and each replay-declared operation becomes a `public tool` bound to the matching effect — quarantine-write operations are `dangerous`, so call sites need compile-checked `approve`. Header carries the setup checklist (mock works immediately; real mode = `corvid connectors oauth` + CORVID_PROVIDER_LIVE=1) and states plainly that tool bodies live host-side. Effect names take an `_effect` suffix (the guide's own convention) so they never collide with the same-stem tool names; provider-prefixed scope/op ids don't double the prefix. Unknown providers refuse with the shipped list. Tests: every shipped connector's scaffold compiles + gmail's write path pins dangerous/human_required. Live-probed: gmail scaffold (4 effects, 4 tools) compiles through `corvid check`.
- [x] 49e-capability-surface-contract   **[closed 2026-07-14 — PHASE 49 COMPLETE]**   The invention contract for the capability surface: `docs/guides/capabilities.md` (extending your agent — skills/MCP/connectors, added to the book snippet guard), FEATURES.md capability-surface section, README catalog entries (Effect-Audited Skills — the nutrition-label flagship — and Typed MCP In One Command with the connector scaffolds), two inventions.md rows (run-it commands + test/spec pointers per the CLI-invention pattern). Tour topics deliberately NOT added: these are CLI-surface inventions whose "run it" is the command itself, the Governed Cron precedent. With 49a–49e closed, Phase 49 is complete — the launch phase is next, per the standing Phase 48 decision.

### Phase 50 — Developer-magnet features (pre-launch; pre-phase chat held 2026-07-14)

**CTO direction (2026-07-14):** before launch, add the features that make Corvid so powerful "every developer wants to use it for AI" — demos must make every developer feel like trying it. Positioning locked the same day: lead with the UNIVERSAL pains (unreproducible failures, surprise bills, glue-code weeks); safety/approval is the retention depth, not the acquisition story. Feature set chosen under the standing delegation; the launch phase (demo doctrine + five moments + playground/website track) follows as Phase 51, fed by these. **Filed for Phase 51 (2026-07-15):** OpenTelemetry trace export — platform-team credibility; our traces are richer than anyone's but must speak the ecosystem's protocol; dashboards stay third-party.

- [x] 50a-structured-output-self-heal   **[closed 2026-07-14 — the feature itself shipped in 46h; 50a closed its budget-composition gap]**   Survey found `with repair N` already implements the corrective re-ask exactly as designed (schema-violation feedback appended, every attempt a fully traced LLM call, replay reproduces the sequence, failed attempts' cost accumulates; book ch11 documents it). Opt-in stays the RIGHT design: an unasked-for extra LLM call would change cost semantics silently. What was MISSING was moat composition: the static worst-case analysis ignored repair, so `@budget` could verify a bound the runtime may exceed (a $0.30 prompt with `repair 2` is a $0.90 worst case). `effect_node_for_prompt` now multiplies all per-call cost dimensions (cost/tokens/latency_ms) by `1 + N`; pinned both directions (`repair_attempts_multiply_worst_case_cost_for_budgets` rejects, the repair-free twin passes) and documented in ch11's repair section.
- [x] 50b-behavioral-diff   **[closed 2026-07-15 — the surface existed (Phase 21 `corvid test --from-traces`); 50b made its verdicts honest]**   Probing the shipped prod-as-test-suite harness end-to-end found THREE false-verdict bugs: (1) a replay run that carried an error (e.g. the cross-tier refusal) counted as PASSED while printing the error underneath — now Errored; (2) same false positive on the differential path; (3) actual behavioral drift classified as harness ERROR instead of DIVERGED because the CLI's dispatch adapter flattened the typed `ReplayDivergence` into `ReplayTraceLoad` — `ReplayOutcome` now threads the structured divergence and the adapter re-raises it typed, so the harness's (always-correct) Diverged arms finally fire. Live-probed the full loop: record → same source = `1 passed`; edit source = `1 diverged` with the exact expected-vs-got payload; revert = `1 passed`; cross-tier trace = `1 errored`. Verdict taxonomy pinned by `verdicts_classify_error_divergence_and_pass_honestly`. An initial duplicate `corvid behavior diff` command was built then DELETED on discovering the existing surface — one canonical surface, no porcelain aliases; the pain-first documentation of this feature rides Phase 51's launch content.
- [x] 50c-model-upgrade-diff   **[closed 2026-07-15 — shipped surfaces confirmed; hardened by 50b's fixes]**   The model axis already ships twice over: `corvid test --from-traces --replay-model <ID>` (corpus differential against a candidate model) and `corvid eval --swap-model <ID> --source <FILE> <TRACES>` (retrospective migration analysis with semantic divergence reporting). Both ride the differential harness whose errored-run false positive 50b fixed, so an errored model-diff run can no longer masquerade as "no divergence". Launch framing ("answer the deprecation email with evidence") rides Phase 51.
- [x] 50d-token-streaming   **[closed 2026-07-15 — the runtime was ahead of the last mile; three real gaps closed]**   The deep machinery already shipped (46d): adapter-level streaming (Anthropic/Gemini/Ollama native + whole-call fallback so mocks stream), the VM's Stream prompts with backpressure/token-limit/confidence-floor pumps, and replay that RE-CHUNKS at recorded boundaries. What was broken/missing: (1) VM BUG — `return <stream>` from a Stream-returning agent DISCARDED the returned stream (the spawned body's `Ok(Flow::Return(_)) => {}`), so the most natural program (`agent main() -> Stream<String>: return tell(...)`) silently produced an EMPTY stream; returns now forward the inner stream's chunks (non-stream returns become the single final chunk), pinned by `returning_a_stream_from_a_stream_agent_forwards_its_chunks`. (2) `corvid run` printed the stream HANDLE's debug form; it now drains chunks to stdout as they arrive (flush per chunk), inside the run's own async context. (3) `corvid serve` responds to Stream-returning routes as Server-Sent Events (`text/event-stream`, `data:` per chunk, `event: done`/`event: error` terminators) — live-probed with curl. BONUS latent gap found: serve's runtime builder wired NO LLM adapters at all (served apps could not call models, period); the env-driven wiring (`CORVID_MODEL`, mock, Anthropic/OpenAI/Ollama) is now a shared `apply_env_llm_wiring` helper used by run AND serve — every entry point wires models identically.
- [x] 50e-declarative-model-routing   **[closed 2026-07-15 — Phase 20h already shipped a routing surface BEYOND the slice's design; 50e closed its budget hole and its book silence]**   Shipped since 20h: `route:` (guard-based model arms), `progressive:` (cheap-first confidence-threshold escalation chains — exactly the slice's target semantics), `rollout` (A/B percentages), `ensemble [..] vote majority` (concurrent voting with calibration-accuracy weighting + disagreement escalation), and `adversarial` (propose/challenge/adjudicate). TWO GAPS CLOSED: (1) the 50a-class soundness hole — the runtime SUMS member costs but the static analysis counted every prompt once, so `@budget` verified bounds ensembles exceed; worst-case dimensions now multiply by the dispatch-call count (ensemble members + escalation member; progressive stage count; adversarial = 3; route/rollout = 1), composing with the repair multiplier, pinned by three tests (3-member ensemble rejected, 2-stage progressive rejected, single-dispatch twin passes). (2) The book's prompts chapter documented NONE of it — ch11 gains the Model Routing section (compiling `progressive:` example + the full form catalog with their budget semantics).
- [x] 50f-token-context-budgets   **[closed 2026-07-15 — shipped in 46c; verified + already book-documented]**   `context_window: N` on model declarations gives the typed overflow policy the slice asked for: history truncates OLDEST-FIRST (whole messages, never system blocks or the current turn) until the request fits N minus the completion reserve; truncation is a pure function of its inputs so replays truncate identically; when nothing fits the call fails with a TYPED error instead of a silent provider rejection. Completion-side caps ride `max_tokens` (compile-checked ranges, `TokenLimitExceeded` mid-stream). Pinned by `history_segments_splice_and_truncate` + the 46c design doc; ch11 documents both. The remaining idea from the slice text — an LLM summarize-on-overflow policy — is genuinely NEW machinery (an LLM call inside overflow handling) deferred post-v1.0: truncate + typed-refuse already kill the mystery-crash pain.
- [x] 50g-latency-budgets   **[closed 2026-07-15 — the generic dimensional-constraint machinery already enforces it; 50g pinned + documented it]**   `@latency(fast)` on an agent is a STATIC latency SLA: the latency dimension composes by MAX through the call graph, and any reachable call whose class exceeds the constraint fails at compile time naming the dimension (live-probed: `constraint requires fast, but composed value is slow`). It worked because effect constraints are generic over dimensions — but it was untested (only the streaming-backpressure form appeared anywhere) and undocumented. Now pinned both directions (`latency_constraint_rejects_slow_composed_path` / `_accepts_fast_path`) and documented in ch07's dimension list alongside @budget. Numeric `latency_ms` worst-path bounds (beyond classes) stay future work; classes are the honest 80% of the SLA pain.
- [x] 50h-cost-attribution   **[closed 2026-07-15 — shipped across Phase 40's observability surface]**   The FinOps question is already answerable twice over: `corvid observe list` renders local lineage runs WITH per-run costs (plus failures, approvals, slowest span), and `corvid observe cost-optimise` aggregates cost-by-event-name across a trace dir, ranks the top-N cost centres, and proposes TYPED suggestions (cache / skip-pre-validate / model-swap) each carrying `sources` back-references — the Grounded shape. 13 unit tests cover the observe command family. The per-TENANT axis deliberately waits for the multi-tenancy story (post-v1.0), where tenant ids become first-class in traces beyond the connector layer. Launch framing ("the monthly FinOps question, one command") rides Phase 51 content.
- [x] 50j-value-refinements   **[closed 2026-07-15 — the Pydantic gap closed, composed with the moat]**   Record fields now declare VALUE constraints: `age: Int where between(0, 150)` (inclusive Int range) and `name: String where len_between(1, 80)` (char count) — contextual `where`, zero new lexer tokens. Threaded through every layer: AST `Field.refinement` (+ mechanical updates across the Field-literal sites), parser (named-form syntax with negative-bound support and precise form errors), checker (`RefinementInvalid` — form-vs-type and min≤max validated at the DECL, never a decode surprise), IR `IrField.refinement`, and DECODE enforcement in `json_to_value`'s struct path where a violation renders "field `age` on `Person`: value 969 violates the declared refinement `between(0, 150)`" — that exact string is what `with repair N` feeds back to the model, so refined prompt outputs HEAL until structurally AND semantically valid, with attempts still counted by the 50a budget multiplier. Live-probed (Methuselah at 969 refused with the actionable message; mismatched form refused at check). Tests at all three layers (parser round-trip, checker mismatch + well-formed, decode reject with message-content pins); ch05 gains the Field refinements section, ch11 notes the repair composition.
- [x] 50k-call-timeouts-and-breakers   **[closed 2026-07-15 — the unglamorous production pair, replay-safe by construction]**   (1) TIMEOUTS widen the try grammar: `try expr timeout <ms> [on error retry N times backoff ...]` — the bound applies PER ATTEMPT and expiry counts as a retryable error, so timeout and retry compose naturally; a timeout clause also lifts the old Result/Option-only restriction (any call can hang), while retry-only keeps it. Interpreter-enforced via tokio timeout; the NATIVE tier refuses `timeout` loudly (no cancellation point in synchronous lowering — the tier-matrix pattern). Tests: parser (timeout-only, combined, bare-try rejection), VM (slow tool fails fast naming the bound; flaky-then-fast recovers under timeout+retry). (2) BREAKERS on tool declarations: `tool x() -> T breaker N uses e` — after N CONSECUTIVE failures (runtime errors or Err-envelope results) the breaker opens and further calls refuse BEFORE dispatch with an error naming the breaker; success resets. RUN-SCOPED by design: wall-clock cooldowns would break replay determinism, and every refusal is an ordinary traced event. State shared across sub-interpreters (agent calls). Test: 2 failures open it, the third call never reaches the tool (dispatch count pinned at 2). ch12 gains Timeouts + Circuit breakers sections.
- [x] 50l-judged-output-guard   **[closed 2026-07-15 — one scoring contract from evals to production]**   `with judged "criteria" min S` on a prompt puts the SAME LLM judge that powers `assert judged` between every output and the program: each decoded output is scored 0..=1 against the criteria; a below-threshold score fails the attempt naming score/threshold/criteria — never a silent pass, and judge transport failures error too. The failure is a MARSHAL-class error, so it FEEDS `with repair N`: guarded outputs heal until they pass the judge. The judge call is an ordinary LLM call (traced, replay-substituted) and the static budget counts it — one extra call per attempt (×2), composing with the repair and dispatch multipliers (50a/50e). The eval harness's judge was factored into the shared `judge_score` helper (test_runner now reuses it — one implementation, two surfaces). Tests: parser (criteria + min range), VM reject/pass against mock judge scores, budget doubling; ch11 gains the Judged output guards section.
- [x] 50i-injection-taint-v1   **[closed 2026-07-15 — THE invention: prompt injection is a compile error; design doc at docs/meta/50i-injection-taint-design.md]**   `data: untrusted` on an effect marks its results `Tainted<T>` (the `Grounded<T>` machinery inverted — tracking where untrusted data must NOT go). Taint is contagious (binary-op operands, AND prompt output — an LLM that read attacker-controlled text produces attacker-influenced output) and NEVER assignable to `T` (no silent laundering, unlike Grounded's legacy coercion). The SINK RULE: passing a tainted value to an approval-requiring call (dangerous marker, or trust-derived supervisor/human tier) is a compile error naming the argument and the two ways out. `trusted(expr)` is the sole, greppable unwrap boundary. Threaded through every crate: `Type::Tainted` + parsing + assignability; `effect_row_is_untrusted`; source/contagion/sink in the checker; `TaintedDangerousArgument` diagnostic with two-way help; `trusted(expr)` as a primary expr (parser peek_ahead, checker unwrap, IR `TrustBoundary`, VM/codegen identity — compile-time only, at runtime `Tainted<T>` IS `T`); guarantee row `taint.untrusted_cannot_reach_dangerous` (Static). Live-probed: tainted prompt output refused at `pay`, `trusted()` unblocks. Tests: 4 checker (prompt-output refusal, trusted() clears, direct-source refusal, concatenation contagion). Contract: ch08 injection section, README + inventions rows, `injection-taint` tour topic, core-semantics regenerated. **PHASE 50 COMPLETE — all twelve developer-magnet slices shipped.**

### Phase 51 — Full-stack application surface: Application Contract, SDKs, console, identity (pre-phase chat 2026-07-15)

**CTO decision (2026-07-15):** ship the ENTIRE application-contract + identity program BEFORE launch, at full scope, NO NARROWING and NO SHORTCUTS. Corvid behaves like a modern backend compiler: it describes its public application interface precisely enough that existing frontend languages (React/Vue/Svelte/Swift/Flutter/...) consume it safely — WITHOUT Corvid becoming a frontend language. It owns the AI-backend↔frontend boundary, not the frontend. The promise: *define your AI application once; Corvid generates its server, schemas, frontend contract, typed SDK, development console, identity, and production guarantees.* The launch phase (demos/playground/website/benchmark/beta) becomes Phase 52, sequenced after this.

**Foundations already in the tree** (this is a surface over existing runtime, not from scratch): the full auth stack (`corvid-runtime/src/auth/`: oauth, csrf, sessions, api_keys, audit) + `jwt_verify/jwks`, the approval queue/authorization/UI, connectors with per-provider OAuth, `corvid serve` routes, SSE streaming (50d), and — the direct precedent for the contract emitter — `corvid-abi::emit_abi`, which already emits a machine-readable JSON descriptor of the exported surface. The gap is the SOURCE-LANGUAGE surface (no `identity`/`provider`/route-policy today) + the contract emitter + the TS/console competency.

Two contracts share one set of JSON schemas: standard **OpenAPI 3.1** (any OpenAPI tool consumes it) + **corvid-ai.json** (agents, streaming events, approvals, grounding, confidence, review, cost/latency, replay). Semantic constraints (compiler/runtime-enforced) are distinguished from `@ui` display hints (frontend may ignore hints, never constraints).

Emitted after resolve + type/effect/permission checking, before backend codegen:

- [x] 51a-application-contract-core   **[closed 2026-07-15 — the dependency root; emitter live]**   `corvid-abi::app_contract` emits the `ApplicationContract` (versioned, `CONTRACT_VERSION=1`): public types with field refinements → JSON-Schema constraints (min/max, minLength/maxLength from 50j `where`), routes (method/path/params/query/body/response/requires-from-effect-row), and public agents + prompts each with AI-NATIVE capabilities computed from the return type + composed effect row — `streaming` (Stream<_>), `grounded` (Grounded<_>), `tainted_input` (Tainted<_> param, 50i), `approvals_possible` (trust-derived), `confidence_min` (only when a REAL gate <1.0), `max_cost_usd`, `latency_class`. Mirrors `emit_abi`, shares the type-description machinery. Driver `compile_to_application_contract_with_config` + `corvid contract app [file] [--out -|path]` writes `target/contracts/app.corvid.json`. Live-probed on a route + streaming agent + grounded agent + refined types: streaming/cost/latency/constraints all correct. 4 emitter unit tests. 51b (OpenAPI projection) + 51c (richer AI-native event/approval metadata) layer on this model.
- [x] 51b-openapi-projection   **[closed 2026-07-15 — standard OpenAPI 3.1 from the shared model]**   `corvid-abi::openapi::emit_openapi` is a pure transform of the 51a `ApplicationContract`: routes → `paths` (method/params/query/requestBody/responses), public types → `components/schemas` as JSON Schema with field refinements as value constraints (minLength/maxLength/minimum/maximum), generic wrappers unwrapped to the shape a JSON client receives (Grounded<T> → {value, sources}, List<T> → array, Result<T,E> → oneOf, Option → nullable, Stream/Tainted → inner), and a `corvidSession` cookie security scheme with approval-bearing routes advertising `security`. `corvid contract openapi [file] [--out -|path]` writes `target/contracts/openapi.json`; the two artifacts share schema names. Live-probed: valid 3.1.0 doc, constraints + security present. 2 tests. Any OpenAPI tool (client gens, Swagger UI) consumes it with zero Corvid awareness.
- [x] 51c-corvid-ai-metadata   **[closed 2026-07-15 — the AI-native artifact, beyond OpenAPI]**   `corvid-abi::corvid_ai::emit_corvid_ai` projects the shared contract model into `corvid-ai.json`: per public agent/prompt, the ordered typed STREAMING EVENT PROTOCOL (`started` → `chunk` if streaming → `tool_started`/`tool_completed` for agents → `approval_required` if approvals-possible → `completed`/`failed`), grounding shape (`Grounded<T>` → sources), confidence routing (floor + `below_minimum: human_review`), cost/latency, and the `tainted_input` flag (50i). Event tags match the SSE `event:` field the runtime writes (50d), so `@corvid/client` (51l) gets a fully-typed `for await` event union. `corvid contract ai [file] [--out -|path]` writes `target/contracts/corvid-ai.json`; shares schemas with openapi.json. Live-probed; 3 tests.
- [x] 51d-ui-hints   **[closed 2026-07-15 — semantic constraints and display suggestions in separate channels]**   `@ui(key: value, ...)` on a struct field carries optional presentation hints (`label`, `placeholder`, `currency`, `multiline`, ...) with string/bool/int literal values; multi-line form parses naturally (lexer suppresses newlines in parens). AST `Field.ui: Vec<UiHint>` + `UiHintValue` (Str/Bool/Int); the contract emitter surfaces them in a SEPARATE `ui` map on `ContractField`, distinct from the refinement constraints — a frontend may ignore hints but never the semantic constraints. Contract-only (not threaded to IR/runtime). Live-probed: `explanation` carries BOTH `min_length`/`max_length` (semantic) and `{label, multiline}` (display) in distinct channels. Parser test covers inline + multi-line + a field with both a hint and a refinement + a hint-free field.
- [x] 51e-typed-errors-contract   **[closed 2026-07-16 — error variants carry status, presentation, and payload; Result routes project per-status error responses]**   Sum-type variants take optional `@status(code)` / `@ui(key: value, ...)` attributes before the `|` (parser lookahead `variant_attrs_precede_pipe` disambiguates a variant-attribute group — followed by `|` — from a field's `@ui` group — followed by an identifier). AST `SumVariant` gains `status: Option<u64>` + `ui: Vec<UiHint>`. The contract's `ContractType.variants` is now `Vec<ContractVariant>` (name + payload fields + status + ui map), so a frontend gets exhaustive per-variant handling: the tag to match, the HTTP status, presentation defaults, and the payload shape. OpenAPI projection: a `Result<T, E>` route whose `E` is a status-bearing error enum emits one response per `@status` (variants sharing a code collapse into one response listing both), referencing the error schema — standard client generators produce typed error branches instead of one opaque failure. Live-probed `RefundError` (404 + ui message, 410 with payload, plain payload variant). Parser test (variant status/ui/payload + plain-payload variant) + emitter test (status/ui/payload surfaced) + OpenAPI test (per-status responses, shared-code grouping).
- [x] 51f-uploads-and-pagination   **[closed 2026-07-16 — uploads and pagination are first-class HTTP-boundary types in source + contract + OpenAPI]**   Two new compiler-known generic heads: `Upload<Format>` (a file upload; the `Format` tag — `Pdf`/`Image`/`Csv`/`Json`/`Text`/`Audio`/`Video`/any — supplies default accepted MIME, resolver treats it as a free-form tag so any format is accepted) and `Page<Item>` (the cursor-pagination envelope). Both are `Type` variants that native codegen backends refuse to lower (interpreter/serve-tier, mirroring `DbHandle`); the application contract + OpenAPI describe them. `@upload(max_mb:, max_bytes:, retention_days:, mime:)` field attribute carries the semantic upload constraints (parsed in the field-attribute loop alongside `@ui`). Contract: `ContractField.upload` (format + accepted_mime + max_bytes + retention_days); `Capabilities.pagination` / `ContractRoute.pagination` (`cursor` for `Page`, `stream` for `Stream`, with item type + cursor param). OpenAPI: an upload field → binary string with `contentMediaType`/`maxLength`; a body containing an upload → `multipart/form-data`; a `Page<Item>` response → the `{items, next_cursor, has_more}` envelope + an optional `cursor` query parameter. Live-probed a `DocSubmission` (two upload fields with distinct MIME/size) and a `Page<Item>` route across all three artifacts. Tests: parser (`@upload` keys + both generic heads), contract emitter (upload surface, MIME override, cursor + stream pagination), OpenAPI (binary+multipart, page envelope + cursor param).
- [x] 51g-identity-surface   **[closed 2026-07-16 — the `identity` declaration parses, checks with mandatory safe-defaults, and flows into the application contract]**   New `identity <name>:` declaration (`KwIdentity`, a reserved word — breaking): `provider google|github|microsoft|apple|discord|slack` (the full named set, no narrowing) plus `provider oidc "<https discovery-url>" as <alias>`; a `session:` sub-block with `lifetime: <s|m|h|d>`, `same_site: strict|lax|none`, `secure`/`http_only`/`rotate_on_privilege_change` bools. AST (`IdentityDecl`/`ProviderKind`/`SessionConfig`/`CookieConfig`/`SameSite` with safe-default `Default` impls) + parser (new `decl/identity.rs`) + resolver (`DeclKind::Identity`, name registration, no-op resolve) + checker (`check_identity`). **Safe-defaults are mandatory**: at least one provider, `https://` OIDC URLs, and secure+http_only cookies with a non-`none` SameSite plus session rotation are the defaults; weakening any of them is a hard `IdentityConfigInvalid` error UNLESS the block carries an explicit `insecure_opt_out: true`, which downgrades to a loud `W0300` warning (never silent). Wired to the contract-driven architecture: the compiler surfaces the identity block (providers + session posture) in the application contract for the SDK / dev console / `corvid serve` to consume. The login identity stays SEPARATE from connector workspace tokens (51j). Live-probed the contract surface + both enforcement paths (`corvid check` error without opt-out, warning with it). Tests: parser (providers + OIDC + session + default safe flags), contract emitter (provider list + session posture), checker (reject-without-opt-out, warn-with-opt-out). NOTE: the auto-exposed `/auth/{provider}/*` routes, `Actor`, and PKCE/state/nonce flows are slice 51h.
- [x] 51h-auth-routes-and-actors   **[closed 2026-07-16 — route auth policies + typed actor + auto-exposed auth routes, all safe-defaults named in the contract]**   A server route takes a `requires authenticated | role("...") | permission("...")` clause (chained with `and`); AST `RoutePolicy` + parser (`parse_route_policy`). The checker requires an `identity` block for any policy (`RoutePolicyWithoutIdentity` error otherwise) and binds a typed `actor` local in the route body — modeled with the same `RouteParams` machinery `path`/`query` use (`id`/`tenant`/`display_name`/`roles`/`permissions`), so field access is fully typed with NO new `Type` variant. The actor deliberately carries no provider tokens (the login identity stays separate from connector workspace tokens, 51j). The `identity` block auto-exposes `/auth/{provider}/login` + `/callback` per provider plus `/auth/logout` and `/auth/session`, surfaced in the application contract (`ContractIdentity.auth_routes`) and projected into OpenAPI (tagged `auth`, with the redirect/session responses). Every OAuth safe-default is named in the contract as a machine-readable `safeguards` list (authorization_code_with_pkce, signed_expiring_state, oidc_nonce, exact_redirect_uri_allowlist, jwks_signature_verification, iss_aud_exp_nbf_validation, secure_http_only_cookies, session_rotation_on_privilege_change, csrf_double_submit, refresh_token_rotation, encrypted_provider_tokens, token_revocation, redacted_auth_logs, minimal_scopes). A policy route projects OpenAPI `security` scoped to its roles/permissions (`role:admin`, `permission:refund:write`). Live-probed contract + OpenAPI + `return actor.display_name` typing + the no-identity rejection. Tests: parser (chained requires), contract emitter (auth routes + safeguards + policy + typed-actor field access + no-identity reject), OpenAPI (auth paths + scoped security). NOTE: the runtime route-mounting in `corvid serve` (the auth routes already have their storage-layer crypto in `corvid-runtime/src/auth`) is a serve-integration follow-up; 51h ships the source + type + contract surface.
- [x] 51i-account-linking   **[closed 2026-07-16 — explicit-confirmation linking is structural; silent email-match merge is impossible]**   A `linking:` sub-block on the `identity` block (AST `LinkingConfig` + `EmailMatchPolicy` + parser). The ONLY knob is `email_match: never | verified_domain` (default `never`), with `verified_domains: "a.com, b.com"` required for the verified-domain policy; the checker rejects verified_domain without domains, rejects domains with a scheme/path/`@`, and rejects `verified_domains` under `never`. The explicit-confirmation flow (sign in → start link → authenticate the new provider → confirm ownership → approve → audit) is STRUCTURAL — there is no source syntax to turn it off, so `confirmation_required` is always true in the contract. The compiler auto-exposes `/auth/link/{provider}/start` (per provider) + `/auth/link/confirm` (both session-gated), projected into OpenAPI with 200/401/409 responses (409 = ownership unproven, no merge). Four linking guarantees join the machine-readable safeguards list: `explicit_link_confirmation`, `no_silent_email_merge`, `link_ownership_proof`, `link_audit_trail`. The round-trip renderer emits the `linking:` block. Live-probed the contract linking surface + link routes + both rejection paths. Tests: parser (linking block), contract emitter (defaults + link routes + verified-domain domains), checker (verified_domain-without-domains + malformed-domain rejection).
- [x] 51j-connector-user-auth   **[closed 2026-07-16 — per-user connector authorization + a runtime-enforced credential separation from the login session]**   The connector manifest (`corvid.connector.v1`) gains `authorize: workspace | per_user` (default workspace); manifest validation rejects a `per_user` connector with no scopes (`PerUserWithoutScopes`). The load-bearing guarantee: a `ConnectorAuthState` carries a `CredentialKind` (always `ConnectorAccess` for a usable connector token), and `authorize()` REFUSES a `LoginSession` credential (`NotAConnectorCredential`) — the login identity token and the connector workspace/per-user token are distinct credentials that never interchange, so a stolen/replayed login cookie cannot act as a connector token. A `per_user` connector additionally requires the end-user actor (`PerUserRequiresEndUser`), keeping each user's grant scoped to that user; the refresh path propagates the mode. Registered as guarantee `connector.per_user_token_separate_from_session` (RuntimeChecked, with the id literal wired at the enforcement site for the inverse-coverage sentinel), core-semantics.md regenerated. Tests: manifest (per_user parse + validate + no-scopes reject), auth (login-session refusal [adversarial] + per-user end-user requirement [positive]). Composes the Phase 41 connector runtime; the per-user OAuth authorize UI flow rides on the existing `connectors_cmd/oauth.rs` surface.
- [x] 51k-mock-idp-and-fuzz   **[closed 2026-07-16 — a mock IdP whose only verifiable token is the correct one, proven by mutators + byte-fuzz]**   New `corvid-runtime/src/jwt_verify/mock_idp.rs`: a `MockIdp` that mints Ed25519-signed ID tokens (deterministic fixed-seed key, no system RNG) and serves a matching JWKS through a `MockIdpFetcher`, so the full verification path (kid resolution → signature → iss/aud/exp) runs end-to-end without a network. Source-bypass mutators (`mint_alg_none`, `mint_tampered_signature`, `mint_forged_kid`, `mint_with` for issuer/audience/exp edits) each break exactly one safe-default; the adversarial test asserts ALL six are refused while the honest token verifies. A deterministic xorshift byte-fuzz (2000 malformed inputs, no `Math.random`) proves the JWT front door never panics and never forges a valid result. Registered as guarantee `auth.jwt_tamper_and_fuzz_resistant` (RuntimeChecked; id wired at the enforcement site for the inverse-coverage sentinel), core-semantics.md regenerated. The identity block (51g-51k) is now complete: the surface (51g), its routes + typed actor (51h), account-linking (51i), connector separation (51j), and this adversarial proof harness (51k).
- [x] 51l-ts-client   **[closed 2026-07-16 — the contract generates a typed TS client that type-checks against a shipped reused transport]**   Two pieces. (1) `sdk/typescript/client/` — the generic `@corvid/client` package (static, shipped once): `CorvidClient` transport (fetch with `credentials: include`, `invoke`/`request`/`stream`/`paginate`), the typed agent event union over SSE (`events.ts`), `CorvidError` carrying `@status` + parsed body (`errors.ts`), and cursor pagination (`pagination.ts`). Type-checks clean under `tsc` strict/NodeNext. (2) `corvid-abi/src/ts_client.rs` — `emit_ts_client(contract)` producing `types.ts` (one `interface` per record, a discriminated union + `…Meta` status/ui map per sum/error type, uploads → `File`, generics unwrapped) and `api.ts` (a thin typed `Api` method per agent/prompt delegating to `client.invoke`/`client.stream`, plus `loginWith_*`/`session`/`logout` from the identity block). The generated code imports the transport — it is NEVER regenerated, so the moat behavior lives in one audited place. CLI `corvid contract ts-client [file] --out <dir>`. Live-probed end-to-end: generated a client for a mixed app (record/error-enum/Page/Stream types + streaming + identity) and type-checked the generated files PLUS realistic usage (typed result, `event.kind === "chunk"` narrowing, exhaustive error-union narrowing, auth helper) against the package under `tsc` — zero errors. Rust tests: record→interface, error-enum→union+meta, agent→typed method (invoke vs stream), identity→login helpers.
- [x] 51m-dev-console   **[closed 2026-07-16 — one self-contained console, driven entirely by the contract, that any Corvid app gets for free]**   `corvid-abi/src/dev_console.rs` — `emit_dev_console(contract, ai)` renders a single self-contained HTML page (inline CSS+JS, theme-aware, no external requests) whose behavior is driven ENTIRELY by the embedded Application Contract + `corvid-ai.json`. The same static JS renderer works for every app: sign-in buttons per identity provider + session panel + the guaranteed-safeguards badges, a form per public agent/prompt with typed inputs and capability badges (streaming / grounded / approval / tainted / cost / latency / pagination), a Run button that POSTs to `/agents/{name}` (or consumes the SSE event stream for streaming agents, rendering a typed event log), a typed-error surface, and a type browser. The contract JSON is embedded as an inert `<script type="application/json">` so app strings can never break into script context. `corvid dev [file] [--listen addr] [--out file]` — a tiny blocking static server (no async, nothing to break in a demo) serving `/` (console), `/_corvid/contract.json`, `/_corvid/ai.json`; `--out` writes the HTML. Execution targets a backend base URL set in the console (point it at a running `corvid serve`). Live-probed: emitted the console for a mixed app and served it — `GET /` → 200 text/html (11.7 KB, self-contained), `/_corvid/contract.json` → the contract, unknown paths → 404. Tests: self-contained + embeds contract/ai; renders the identity + streaming + `/agents/` + SSE surface. NOTE: traces+replay and model-comparison panels compose the existing `corvid observe`/replay + model-diff surfaces and are a console-enrichment follow-up; 51m ships the contract-driven console + `corvid dev` command.
- [x] 51n-react-hooks   **[closed 2026-07-16 — generic React hooks that specialize with the generated types, type-checked end-to-end]**   `sdk/typescript/react/` — the `@corvid/react` package layering idiomatic hooks over `@corvid/client` + the generated `Api`. `useCorvidAgent` (invoke, tracking `data`/`error`/`loading`, `CorvidError`-typed) and `useCorvidStream` (consume a streaming agent's typed event log, accumulating `chunks` + a terminal `result`), `useCorvidApprovals` (surface `approval_required` events from a stream's event log + resolve them approve/deny through the client), and `useCorvidPaginated` (cursor pagination with `items`/`loadMore`/`hasMore`, first page on mount). Every hook is GENERIC — the generated method signatures specialize it at the call site. The package type-checks clean under `tsc` strict/NodeNext. Live-probed end-to-end: generated an `Api`, wrote a React component using all four hooks, and type-checked it — `classify.data?.text` is `string`, `chat.chunks[0]` is `string`, `useCorvidPaginated<Item>(...).items[0]?.id` is `string`, all inferred from the contract with zero annotations. Depends only on `@corvid/client` + React (peer deps); nothing per-app.
- [x] 51o-sdk-generators   **[closed 2026-07-16 — one `corvid generate sdk` over four languages from the same contract]**   `corvid-abi/src/sdk_gen.rs` — `SdkLanguage` (ts/swift/kotlin/python, case-insensitive parse) + `emit_sdk(contract, lang)`. TypeScript is the fully-realized target (reuses the 51l `emit_ts_client`: typed client + methods over `@corvid/client`); Swift/Kotlin/Python emit typed MODELS that track the contract exactly — records → `struct`(Codable) / `data class` / `@dataclass`, sum types → `enum` with associated values / `sealed interface` with data object+data class / tagged `@dataclass` + `Union` alias — with a per-language type map (Int→Int/Int/int, List<T>→[T]/List<T>/list[T], Option<T>→T?/T?/Optional[T], Upload→Data/ByteArray/bytes, etc.). Deliberate scaffolds: the model layer (which must track the contract) is generated; transport is a stub extended as demand proves. Every language reads the SAME contract, so types never drift across platforms. CLI `corvid generate sdk [file] --language <lang> [--framework react] --out <dir>`; `--framework react` (TS) also drops a `hooks.example.tsx` over `@corvid/react`. Live-probed all four: Swift Codable struct+enum, Kotlin data class+sealed interface, Python dataclass+Union, TS types.ts+api.ts+hooks.example.tsx. Tests: language parse, and each generator's record+sum mapping, plus `emit_sdk` dispatch.
- [x] 51p-components   **[closed 2026-07-16 — six prototype components over the hooks, specialized by the generated types, type-checked with real JSX]**   `sdk/typescript/react/src/components/` — `CorvidAgentForm` (typed fields → `useCorvidAgent`, with `renderResult`/`renderError` slots), `CorvidStream` (chunks + typed event log via `useCorvidStream`), `CorvidApprovalQueue` (pending `approval_required` events → approve/deny via `useCorvidApprovals`), `CorvidGroundedAnswer` (a `Grounded<T>` value + its source/citation list), `CorvidReviewQueue` (generic human-review list with accept/reject + load-more), `CorvidSignIn` (a button per identity provider → `client.login`). Explicitly SCAFFOLDS — headless-ish, `className`-restylable, for admin panels/demos, NOT polished product UI; the docs and every file say "use the hooks directly for product UI." Type-checks clean under `tsc` strict + `react-jsx` (verified genuine by an injected error the checker caught). Live-probed: wrote a component using `CorvidSignIn` + `CorvidAgentForm` + `CorvidStream` against a generated `Api` and type-checked it — `renderResult`'s arg is `Answer`, `renderChunk`'s is `string`, both inferred from the contract. Depends only on `@corvid/client` + React.
- [x] 51q-frontend-scaffolding   **[closed 2026-07-16 — `corvid generate frontend` scaffolds a runnable React project that type-checks out of the box]**   `corvid-abi/src/frontend_gen.rs` — `emit_react_frontend(contract)` produces a complete Vite + React + TypeScript starter: the generated typed client under `src/corvid/` (reusing 51l), `src/client.ts` (a configured `CorvidClient` + `Api` reading `VITE_CORVID_BACKEND`), `src/App.tsx` (a sign-in row from the identity providers + a `CorvidAgentForm` per non-streaming agent — with numeric fields coerced via `Number(...)` — and a `CorvidStream` per streaming agent), plus `main.tsx`, `index.html`, `vite.config.ts`, `vite-env.d.ts`, `tsconfig.json`, `package.json`, and a README. Unlike the shipped client/hooks/components, these files are a STARTING POINT the developer OWNS and edits. CLI `corvid generate frontend [file] --framework react --out <dir>`. Live-probed end-to-end: generated a project for a mixed app (record/identity + non-streaming + numeric + streaming agents) and ran `tsc` over the WHOLE project (App.tsx + client.ts + main.tsx + generated corvid/*) against real React + Vite types and the source packages — zero type errors. Tests: the project tree is complete + runnable, `App.tsx` wires the right form/stream per agent from the providers, numeric fields are coerced.
- [x] 51r-contract-sweep   **[closed 2026-07-16 — Phase 51's inventions are now discoverable, runnable, and test-backed; the surface advertises itself]**   The invention-shipping-contract closeout for the application surface: a `corvid tour --topic application-surface` demo whose source compiles through the driver (the "Define Once, Get Everything" topic, added to `corvid-tour-catalog` — verified by `all_tour_sources_compile`); a "6. The Application Surface" section in `docs/reference/inventions.md` (contract→OpenAPI/console/SDKs, typed errors, identity-safe-by-construction) plus ten Proof Matrix rows across 51a-51q with runnable commands + test refs + non-scopes; a README "The Application Surface" catalog entry; a new guarantee row `contract.matches_compiled_surface` (Static/AbiEmit, kind AbiDescriptor — the emitted contract describes exactly the checked PUBLIC surface, private decls never leak; id wired at the enforcement site for the inverse-coverage sentinel, `core-semantics.md` regenerated). And the live backend advertises itself: `corvid serve` now serves the Application Contract at `/.well-known/corvid` and OpenAPI 3.1 at `/openapi.json` (built best-effort from the same source; `{}` fallback). Live-probed both endpoints (contract with agents/identities/routes; OpenAPI with all paths incl. the auto-exposed auth routes; `application/json`). **Phase 51 (the full-stack application surface) is complete.**

## Phase 52 — The Complete Application Runtime

Phase 51 shipped the definition + generation layer: a Corvid backend describes its whole public interface as a machine-readable Application Contract, and OpenAPI / a console / typed SDKs fall out of it. But the runtime deferred execution at almost every seam (routes → `501`, uploads/pagination contract-only, `/auth/*` unmounted, no source-level connectors, no data/migration surface, `dev`/`deploy`/`prove` incomplete). Phase 52 closes that gap without shortcuts: it makes Corvid a **proof-carrying AI application language** — the runtime, data, integrations, and deployment all carry machine-checkable evidence about their behavior. Pre-phase chat: 2026-07-16 (this file's git history + dev-log).

**Locked phase invariant.** The running backend proves it implements its own contract, or it refuses to start. Contract/runtime disagreement is a compiler or startup error — never a `501` surprise.

**Locked completion criterion.** Any feature emitted by the Corvid Application Contract is executable by the Corvid runtime, consumable by generated clients, covered by deployment evidence, and requires no mandatory host-language glue. Completion is certified by the continuously-grown reference app (52q), which must exercise the entire surface with no exceptions or special cases — not by a curated demo.

**Locked design decisions (pre-phase chat).**
- **Execution tier = the Rust interpreter runtime** as the *production* backend (a deliberate architecture, not a fallback): no mandatory Python/host-language glue; every contract feature executes through it; it carries the full effect/approval/provenance/taint/replay/evidence semantics; Contract Closure prevents unsupported apps from starting; OCI deployment is self-contained + reproducible; performance is validated against realistic I/O-bound AI-backend workloads with explicit SLO targets. Native compilation stays supported for compute-hot functions + library embedding, but full native backend parity is NOT a completion gate. Any interpreter/native semantic difference is reported explicitly — silent tier drift is forbidden.
- **Query surface = a typed query builder + compile-time tenant-scope proof.** Any query against a `partition_key` table must constrain the partition or it is a compile error; the only escape is `.across_tenants(approve: …)` (dangerous + approval-gated). This makes tenant isolation a language guarantee, not middleware.
- **Persistence = SQLite (dev) + Postgres (prod)**, both in scope.
- **Connectors v1 = the full surface**: reliability (auth refresh / retry / rate-limit / circuit-breaker / idempotency) + response status→typed-error mapping + async provider state machines + provider-drift quarantine + webhooks.
- **Deploy target = an OCI image** — built, proven, smoke-tested, and deployed *as-tested* (not merely manifests).
- **`corvid prove` taxonomy** = `PROVEN` / `RUNTIME TESTED` / `CONFIGURATION DEPENDENT` / `NOT PROVEN`. It gates what a developer may even assert: unprovable `require`s (provider honesty, model factuality, infra security, legal compliance) are rejected, not just labeled. Reuses + generalizes the guarantee registry (whose `Static/RuntimeChecked/OutOfScope` classes + mandatory non-scopes already model this) and the Phase 43 deploy/attestation/SBOM/signing chain.

**Honesty boundaries baked in from day one.** Provenance is *preserved, not authenticated* — a stored `Grounded<T>` is trustworthy because Corvid's write path preserved it, not because the row is inherently trusted (explicit non-scope: out-of-band DB writes). A branch that has passed a non-reversible effect boundary is not cancellable (cancellation only interrupts pending/reversible work). Real provider behavior (webhooks, OAuth refresh, async) is `RUNTIME TESTED`, never `PROVEN`.

**52q is a continuous phase fixture, not deferred work.** The one-file reference application is created as a stub in 52a and extended in EVERY slice; Contract Closure fails the moment a runtime path lags the contract. The final 52q milestone certifies the continuously-developed fixture exercises the whole surface without special cases.

### A · Contract-Closed Runtime (the spine)

- [x] 52a-route-execution   Every declared route shape executes: path params, query structs, typed request bodies — the interpreter dispatches each to its handler agent. Kills the `501 not_implemented` fallback for supported shapes. Create the one-file reference app stub (`examples/reference_app/` or similar) exercising a first route, and make it the acceptance target that grows every slice. — DONE 2026-07-22. Each route lowers to a synthetic handler agent (`__route__<METHOD>__<path>`) whose params reuse the resolver's `path`/`query`/`body`/`actor` `LocalId`s, so `corvid serve` executes any route body through ordinary agent machinery (effects/approval/replay). Serve registers real axum paths (`{id}`→`:id`), coerces path params + query struct fields from their string form, decodes typed JSON bodies, and returns structured `400`s on malformed input. Reference app `examples/reference_app/src/main.cor` proven end-to-end (GET path-param, GET query, POST body all 200; bad/missing query → 400). The old `dispatch_for`/`RoutePlan` shape-classifier + its "not served yet" 501 branch are deleted.
- [x] 52b-contract-closure   The phase invariant, mechanically enforced: at startup, `corvid serve`/`corvid dev` construct an execution graph over the Application Contract and assert a runtime path exists for every public element (route shape, streaming return, upload, page, auth route, policy). A missing path is a startup error (e.g. `E5204 Contract not executable`, naming the element), never a runtime `501`. + a `contract.runtime_closure` guarantee row (RuntimeChecked). The developer's SOURCE is the forcing function: `public agent chat() -> Stream<Y>` forces the runtime to have SSE or refuse to start. — DONE 2026-07-22. `corvid_driver::check_contract_closure(ir, RuntimeCapabilities)` walks the route surface; serve refuses to start (exit 1, `E5204 Contract not executable`, before binding) when a route advertises a `Stream<T>`/`Page<Item>`/`Upload<Format>` boundary type or a `requires` policy the interpreter tier can't yet serve. `RuntimeCapabilities` snapshot grows one flag per slice (streaming/uploads/pagination→52c, auth→52h), so the backend can never advertise more than it delivers. `contract.runtime_closure` RuntimeChecked row (positive: reference app + capability-present; adversarial: stream/upload/page/policy gaps) + serve_smoke `serve_refuses_to_start_when_a_route_is_not_contract_closed`. Invention proof: README + inventions.md §7 + `corvid tour --topic contract-closure`. **Also fixed two latent 52a regressions the serve_smoke integration suite caught: the banner dropped shape labels (test updated to the preserved approve/non-approve intent) and approval-gated detection broke through the synthetic-handler indirection (now a transitive agent-call walker).**
- [x] 52c-1-streaming-runtime   SSE endpoint for `Stream<T>` route responses (the typed event protocol): serve consumes the interpreter's stream channel and flushes each chunk as a `data: <json>` event with an `event: done` terminator. Flip the `streaming` RuntimeCapability so Contract Closure passes stream routes; grow the reference app with a live SSE route. — DONE 2026-07-22. The SSE `finish` arm was built speculatively in the 51 era but never reachable (routes 501'd pre-52a, closure blocked pre-flip); 52c-1 verified it end-to-end (3 yielded events stream as `data:` + `done`) and unblocked it. Split out of the original 52c after streaming turned out already-built (pre-phase chat 2026-07-22).
- [x] 52c-2-upload-page-runtime   Execute the two HTTP-boundary types with no runtime yet: `Upload<Format>` request bodies (multipart parsing + accepted-MIME + max-size enforcement; read via METHODS — `body.text()`/`body.bytes()`/`body.filename()`/`body.content_type()`/`body.size()`) and `Page<Item>` responses (cursor parsing from the route's `query` struct + `{items, next_cursor, has_more}` envelope; CONSTRUCTED via `Page(items, next_cursor)`, mirroring `Ok`/`Some`, `has_more` derived). New interpreter values for Upload/Page; flip the `uploads`/`pagination` RuntimeCapabilities. Surface shape decided in the 2026-07-22 pre-phase chat. — DONE 2026-07-22. `Page(items, next_cursor)` is a builtin constructor (checker `check_page_call` → `IrExprKind::PageNew` → a `{items, next_cursor, has_more}` struct value; cursor unwrapped from the `Option`, `has_more` derived). Upload methods ride the shared builtin-method table (5 `BuiltinMethodKind` variants); serve parses multipart via `multer`, enforces the format's accepted MIME (`text/csv` etc.) + an 8 MiB max size (structured 400 on violation), and materialises an Upload struct value the methods read. Format tag threaded via new `IrRoute.upload_format` (the resolved `Type::Upload` loses it). Both flipped in `RuntimeCapabilities::interpreter_tier()`; reference app grows `/orders/page` + `/orders/import`; proven live. Native/py/wasm tiers degrade `PageNew` loudly (interpreter-only). serve_smoke `serve_parses_a_multipart_upload_and_enforces_mime` + `serve_returns_a_page_cursor_envelope`; closure-refusal test moved to a `requires`-policy route (the only remaining gap → 52h).
- [x] 52d-effect-aware-scheduling   `parallel` scheduling that is effect-aware: the runtime knows the branches' combined cost, rate-limit domains, dangerous/reversibility surface, and whether replay may contact providers. The cancellation×reversibility rule: a branch past a non-reversible effect boundary is not cancellable. — DONE 2026-07-23 across 52d-1 (effect-profile awareness), 52d-2 (reversibility-guarded live cancellation), 52d-3 (replay reproduces cancellation). Full-fidelity model per the CTO's design choice.

  **Design locked (pre-phase design consult 2026-07-22): the FULL model — cancellation is real AND replay reproduces it deterministically** (CTO chose the most faithful option over "live-only cancel, replay stays sequential" and "awareness/admission only"). Today `parallel` runs ALL arms via `join_all`, buffers per-arm traces, flushes in arm order, and replay re-runs arms sequentially. 52d changes the LIVE semantics to fail-fast reversibility-guarded cancellation and makes the TRACE + replay reproduce it. Decomposed by risk/dependency (each an always-green, committable increment):

  - [x] **52d-1 effect-profile awareness** (safe groundwork, no execution-semantics change → replay-safe): before a `parallel` block runs, compute each arm's static effect profile (transitive cost, composed `reversible` dimension) from the pre-computed IR, plus the combined profile. Record a `parallel.scheduled` trace event. — DONE 2026-07-22. Per-tool `effect_cost` + `effect_reversible` pre-computed on `IrTool` at lower time (from the registry via `compose`, mirroring `produces_grounded`); `corvid_vm::parallel_profile` walks each arm's callee transitively (agent calls recurse, cost SUMs, reversibility ANDs via LeastReversible, If-branches take Max cost) → per-arm + combined `ArmEffectProfile`; the interpreter emits a `parallel.scheduled` HostEvent (arm_count / per-arm cost+reversible / combined) which replay classifies as dispatch-metadata and SKIPS, so it's observability-only and replay-safe (corpus verify still exits 1 on exactly the two fixtures). Proven live (`corvid run` on a parallel program writes the event). 4 driver integration tests (per-arm cost/reversibility, combined SUM+AND, all-reversible block, transitive agent-call propagation). **Scope calls (documented):** combined-cost ADMISSION deferred — `@budget` is charged at RUNTIME so admitting on the static worst-case ceiling would false-positive-refuse blocks whose actual cost fits; it belongs with the 52d-2 cancellation semantics where the reversibility-guarded model makes refuse-before-side-effects sound. Rate-limit-domain serialization deferred to when connectors land (rate limits are a connector-runtime concept, not a language effect dimension) — filed for 52g/52d-2.
  - [x] **52d-2 reversibility-guarded live cancellation**: restructure arm execution to support fail-fast cancellation. Track per-arm "crossed a non-reversible effect boundary" at the effect-execution site. On the first arm error, cancel in-flight siblings that have NOT crossed a non-reversible boundary; an arm past that boundary always runs to completion (**the cancellation×reversibility rule**). Record per-arm outcomes. — DONE 2026-07-22. Execution moved from `join_all` to `FuturesUnordered` + COOPERATIVE cancellation (race-free): one shared `cancel` flag + one `crossed` flag per arm; each arm checks `cancel` at every tool dispatch and stops (`ParallelArmCancelled` sentinel) UNLESS it has set `crossed` by dispatching an irreversible tool (`IrTool.effect_reversible == false`, checked at the dispatch site BEFORE the effect fires). Flags propagate into sub-agent calls so tracking is transitive. On the first REAL error the scheduler sets `cancel`; a `parallel.outcomes` HostEvent records each arm's `completed`/`errored`/`cancelled` + `crossed_irreversible` (replay-skipped). Lowest-index real error is the block's result; cancelled arms never surface. Two DETERMINISTIC tests prove it: `arm_past_irreversible_boundary_is_not_cancelled` (a `Notify`-ordered arm that commits an irreversible write before its sibling fails STILL completes — the rule) and `reversible_arm_is_cancelled_after_a_sibling_fails` (a reversible looping arm cancels once a sibling fails). Corpus verify still exits 1 on the two fixtures (non-erroring parallel blocks behave identically; the new HostEvent is replay-skipped).
  - [x] **52d-3 replay reproduces cancellation**: replay reads the recorded per-arm outcomes and reproduces them deterministically — a `Cancelled` arm replays up to its recorded cancellation point and stops; `Completed`/`Errored` (shielded) arms replay to their recorded terminal. — DONE 2026-07-23. On replay the `parallel` block runs arms SEQUENTIALLY in arm order (matches the arm-ordered buffer flush → substitution cursor stays aligned) and caps each recorded-`cancelled` arm at its recorded tool-dispatch count. Recording additions: per-arm `dispatch_count` + terminal outcome in `parallel.outcomes`, and — the load-bearing prerequisite — FAILING tools now record their error verbatim (reserved `__corvid_tool_error__` key in a substituted `ToolResult`; `call_tool` restructured to emit-then-propagate), so a failing arm (the cancellation trigger) is replayable at all; `replayed_json_result` reproduces it as `RuntimeError::Other(<msg>)` matching the recorded run error exactly. Peek API: `ReplaySource` bracket-matches `parallel.scheduled`↔`parallel.outcomes` into per-block records in entry order (handles nesting), exposed replay module → `Runtime::replay_next_parallel_outcomes` → interpreter. `parallel.cancellation_reversibility` RuntimeChecked guarantee (id anchored in `parallel_profile.rs`, referenced in the `parallel.outcomes` event) + core-semantics regen; invention proof = README + inventions.md §7 + `corvid tour --topic parallel-cancellation`. 9 parallel tests incl. 4 adversarial (multiple cancels, shielded-arm terminal, nested parallel, missing-record honest divergence); corpus verify still exits 1 on the two fixtures (non-cancelling replay byte-identical). **The full-fidelity 52d model — awareness → live cancellation → replay reproduction — is complete.**

    **Investigated 2026-07-22 (acceptance test `replay_reproduces_a_recorded_cancellation`, `#[ignore]`d until this lands).** EMPIRICAL: replaying a recorded cancelling run currently DIVERGES — `ReplayDivergence { step: 0, expected: RunCompleted{error}, got: tool_result boom }`. Root cause: the live cancelling run is timing-dependent (a reversible arm runs SOME number of tool dispatches before the scheduler's `cancel` reaches it), so re-running the block concurrently under Substitute mode consumes the recorded cursor in a different order / for a different arm-event count than was recorded. Non-cancelling parallel replay is unaffected (46e `parallel_trace_is_arm_ordered_and_replays_identically` still passes). Approach for the focused implementation: (1) `Runtime::is_replay_mode()` already exists to branch on; (2) record each arm's tool-dispatch COUNT in the `parallel.outcomes` event (52d-2 records the outcome tag but not the count); (3) on replay, the `parallel` block reads the recorded per-arm outcomes+counts and runs arms so a `cancelled` arm stops after exactly its recorded count of dispatches (returning the `ParallelArmCancelled` sentinel) rather than running live — likely running arms SEQUENTIALLY in arm order on replay to match the recorded buffer-flush order and keep the cursor aligned. The plumbing to READ a recorded `parallel.outcomes` event from the interpreter mid-run (the replay source exposes tool/llm substitution but not arbitrary event peek) is the main new surface. Deliberately NOT rushed at the tail of the 52a-52d-2 session — replay/trace determinism is a core moat invariant.

  Key runtime facts gathered during the consult: effects carry a composed `reversible` Bool dimension (`CompositionRule::LeastReversible` — one irreversible call taints the chain; `corvid-types/src/effects.rs`); `http_egress_post`/`db_egress_write` are the canonical non-reversible effects; the buffered per-arm tracer already records how far each arm got (`corvid-runtime/src/tracing.rs`), so a cancelled arm's partial buffer IS its cancellation point; the IR pre-computes per-tool `effect_cost` but NOT a composed reversibility bool, so 52d-1 computes it from `effect_names` → registry.

### B · Identity & Authorization Runtime

- [ ] 52e-auth-routes-mounted   Auto-mount `/auth/{provider}/login|callback|logout|session` (from the identity block) wired to the existing `corvid-runtime/src/auth` storage: Authorization Code + PKCE, signed/expiring state, OIDC nonce, JWKS verification, iss/aud/exp/nbf, secure http-only cookies, session issuance/rotation.

  **Scoped 2026-07-23 (Section A milestone banked; 52e opens Section B, gets a focused session — it is the OAuth SECURITY surface, all-or-nothing, so no shortcuts).** The machinery all exists; 52e WIRES it into `corvid serve`.
  - **What serve mounts** (exactly the routes the contract already advertises — `crates/corvid-abi/src/app_contract.rs` `contract_identity`): `GET /auth/{provider}/login`, `GET /auth/{provider}/callback`, `POST /auth/logout`, `GET /auth/session` (+ 51i's `POST /auth/link/{provider}/start` if in scope). Derived from the `identity` block's providers.
  - **Existing machinery to wire** (do NOT re-implement — single-source rule): `SessionAuthRuntime` (`corvid-runtime/src/auth/`) — `create_oauth_state(OAuthStateCreate{ id, raw_state, nonce_fingerprint, expiry, pkce, … })` + `resolve_oauth_callback(raw_state, expected_tenant, trace_id, at_ms) -> OAuthCallbackResolution` (single-use, hashed, audited); `create_session`/`resolve_session`/`rotate_session`/`revoke_session` (hashed token, tenant-checked); `auth/csrf.rs` double-submit; JWT verify (`jwt_verify::VerifiedJwtClaims`, iss/aud/exp/nbf/nonce + JWKS). `auth.oauth_pkce_required` guarantee already anchored.
  - **The flow**: `login` → mint PKCE verifier+challenge + `state` + OIDC `nonce`, `create_oauth_state`, 302 to the provider authorize URL. `callback` → `resolve_oauth_callback` (consume state), exchange `code`→tokens at the provider token endpoint (PKCE verifier), verify the ID token via JWKS (iss/aud/exp/nbf/nonce), upsert the actor, `create_session`, set a `Secure; HttpOnly; SameSite` cookie, 302 to the app. `logout` → `revoke_session` + clear cookie. `session` → `resolve_session` from the cookie → the actor/session JSON.
  - **Provider config source** (decide under the decision principle): well-known presets (authorize/token/JWKS URLs) for `provider google`/`github`; OIDC discovery for `provider oidc "issuer"`; client credentials via the `secret("…")` surface (never in the trace). Where a `SessionAuthRuntime` lives in `ServeState` (open a per-serve store, likely SQLite in a serve dir or in-memory for dev).
  - **Testing** (no live provider): `jwt_verify::mock_idp::MockIdp` mints Ed25519 ID tokens + serves a matching JWKS via `MockIdpFetcher`; a mock token endpoint returns those tokens. serve_smoke-style: `login` sets state + 302; `callback` with a mock code + minted token issues a session cookie; `session` returns the actor; `logout` revokes. Adversarial: tampered/expired/replayed state, wrong nonce/aud/iss, missing PKCE (the 51k byte-fuzz corpus + `auth.jwt_tamper_and_fuzz_resistant` already exist to lean on).
  - **Contract Closure**: 52e does NOT flip `auth_enforcement` (that is 52f — authz enforcement of `requires` policies). 52e mounts the login/session ROUTES; a `requires`-policy route still refuses to start until 52f. Confirm the closure gap wording stays accurate.
  - **CRITICAL FINDING (2026-07-23, deeper investigation).** The storage-layer OAuth is the account-LINKING model, NOT first-login: `OAuthStateCreate` requires an `actor_id` that references a PRE-EXISTING actor (the oauth.rs tests `upsert_actor(...)` BEFORE `create_oauth_state`, and `resolve_oauth_callback` returns that bound `actor`). First-time OAuth LOGIN — actor unknown until the verified ID token arrives at callback — has no clean path in the current storage layer. So 52e is NOT pure serve-side wiring; it needs a small STORAGE-LAYER change (an actor-optional first-login state variant) + a first-login provisioning policy.
  - **DESIGN RESOLVED (2026-07-23, CTO directive — supersedes any earlier "default = auto-provision" note).** First-login provisioning is NEVER a silent default (see CLAUDE.md → "No hidden defaults for consequential policy"). The `identity` block MUST declare a `provisioning:` sub-block, or it is a compile error. Resolution:
    - **`provisioning: first_login: open | invited | approval_required`** — explicit, no default. 52e implements `open` (public signup: auto-provision the unknown verified subject) and `invited` (provision only against a pre-existing invitation); `approval_required` is parsed but REJECTED until 52f can execute durable approval completely (never silently degraded).
    - **`tenant: fixed("id") | from_invitation | from_claim("c") allow "a, b"`** — a tenant is NEVER derived from a bare, caller-controlled claim: it comes from fixed config, a verified invitation (`from_invitation` valid only with `first_login: invited`), or an allowlisted issuer claim mapping (non-empty allowlist). Never identify or merge accounts by email.
    - **Storage purpose is structural**: `LoginState { actor: None, purpose: Login }` (first-login / plain login) vs `LinkState { actor: Some(existing), purpose: Link }` (51i account linking) — the callback cannot confuse a login for a link.
    - **Strict callback order**: (1) validate single-use state + PKCE + nonce; (2) exchange code; (3) verify signature/iss/aud/exp/nonce; (4) key external identity by (issuer, subject); (5) if already linked → issue existing actor's session; (6) if unknown → apply the declared provisioning policy; issue+rotate the session ONLY after provisioning succeeds.
  - **Increments (each always-green, committed separately — 52d precedent):**
    - [x] **52e-1 provisioning language surface + compile gate** — DONE 2026-07-23. `provisioning:` block parses (`first_login` + `tenant`); AST `ProvisioningPolicy`/`FirstLoginPolicy`/`TenantAssignment` on `IdentityDecl`; checker raises **`E5210 First-login policy required`** (`TypeErrorKind::FirstLoginPolicyRequired`) when an OAuth block omits `provisioning`, rejects `approval_required` (not executable until 52f), rejects `from_invitation` without `first_login: invited`, rejects an empty claim allowlist, and rejects `provisioning:` with no provider. Every existing identity fixture (README/tour/inventions/serve_smoke/abi tests) updated to declare `provisioning: first_login: open / tenant: fixed("public")`. 3 parser + 7 checker tests; workspace check + corvid-types 305 + serve_smoke 14 green; corpus verify still exits 1 on the two fixtures.
    - [x] **52e-2 first-login storage model** — DONE 2026-07-23. `OAuthStatePurpose { Login, Link }` makes the flow purpose STRUCTURAL: a `Login` state carries `actor_id: None` (actor resolved at callback), a `Link` state carries `actor_id: Some(existing)` (51i). `create_oauth_state` enforces the coherence (login must not bind an actor; link must); `resolve_oauth_callback` returns `actor: Option<AuthActor>` (bound actor for link, `None` for login) while keeping single-use / expiry / tenant checks for both. New `auth_external_identities` table keyed on `(issuer, subject)` — NEVER email — with `find_actor_by_external_identity` (recognise a returning subject) and `link_external_identity` (durable bind; re-homing to a different actor refused; idempotent same-actor re-link). Ripple contained to `corvid-runtime/src/auth` (the OAuth API had no external consumers yet). 5 auth::oauth tests (login-no-actor + single-use, login/link coherence rejections, external-identity map/refuse-rehome); corvid-runtime lib 337 green; workspace check clean.
    - [x] **52e-3 provisioning executor** — DONE 2026-07-23. `SessionAuthRuntime::provision_login(ProvisioningRequest)` (new `auth/provisioning.rs`, one responsibility): (1) recognise — `(issuer, subject)` already mapped → that actor, no policy runs; (2) provision — `FirstLoginPolicy::{Open, Invited}` (approval_required not representable — the checker rejects it, so the runtime only offers what it executes completely). `Invited` gates on `find_open_invitation_by_email` (verified email → operator-created invitation, NOT an account lookup) before writing anything, then consumes it after the actor exists. Tenant from `TenantSource::{Fixed, FromInvitation, Claim{allowlist}}` — a `Claim` value outside the allowlist or a missing claim is refused; never a bare claim. New actor id is deterministic `sha256(issuer\0subject)` so a racing double-callback upserts one actor; external identity bound via `link_external_identity`. `auth_invitations` storage (new `auth/invitations.rs`, one responsibility): create / `find_open_invitation_by_email` (case-insensitive, oldest-first, unexpired+unconsumed) / single-use `consume_invitation`. 8 tests (open provision→recognise, invited match+consume, invited-without-email refused, claim allowlist enforce, missing claim refused, invitation case-insensitive single-use, expired-not-open, past-expiry rejected). corvid-runtime lib 345 green; workspace check clean; corpus verify exits 1 on the two fixtures.
  - **DECISION (2026-07-23, user): "verified userinfo for all providers."** The identity AST permits providers that do NOT issue OIDC ID tokens (github/slack/discord are OAuth2-only), yet the callback model verifies an ID token's `(issuer, subject)`. Rather than refuse those providers (they appear in the shipped README/tour surface), support EVERY declared provider with no shortcut: OIDC providers verify the ID token → `(issuer, subject)`; OAuth2-only providers do a server-to-server userinfo fetch over TLS after code exchange → `(provider_marker, authoritative_user_id)`. Identity is ALWAYS a server-side-verified value, never a client-forgeable claim. The external-identity key generalises from `(issuer, subject)` to `(identity_source, external_id)` — the existing `auth_external_identities(issuer, subject)` columns already hold it (identity_source in `issuer`, external_id in `subject`), so 52e-2/3 need no change.
    - [x] **52e-4a provider config** — DONE 2026-07-23. `serve_auth::provider::resolve_provider` (new `corvid-cli/src/serve_auth/`) maps each `ProviderKind` to an `OAuthProviderConfig` with an `IdentityVerification` method: `Oidc { issuer, jwks_url }` (google/microsoft/apple) or `Userinfo { source_marker, userinfo_url }` (github/slack/discord); generic `oidc` returns `NeedsDiscovery` (endpoint resolution deferred to the discovery HTTP step in 52e-4b). Client id/secret read from `CORVID_OAUTH_<PROVIDER>_CLIENT_ID`/`_CLIENT_SECRET` (injected reader, so pure/testable) — a missing or empty credential is a named error, never a silent empty string. 5 tests (google=OIDC, github=userinfo, oidc=needs-discovery, missing-secret named, empty-treated-as-missing). Workspace check clean.
    - [x] **52e-4b identity-resolution seam** — DONE 2026-07-23. `serve_auth::identity::resolve_identity` (new `identity.rs`) does callback steps 2–4: exchange code → establish a `VerifiedExternalIdentity { source, external_id, email?, display_name?, tenant_claim? }`. OIDC path verifies the ID token via `JwtVerifier` (sig/iss/aud/exp) + a `nonce` fingerprint match against the login nonce → `(issuer, subject)`; userinfo path fetches the provider user endpoint → `(source_marker, authoritative_user_id)` tolerant of `sub`/`id`/numeric. All network I/O behind a `ProviderGateway` trait (JWKS via the runtime's pluggable `JwksFetcher`), so the flow is exercised with a MockIdp + mock gateway, no live HTTP. Prereq runtime change: the JWT verifier's tenant claim is now OPT-IN — an empty `required_tenant_claim` skips the requirement (a plain OIDC login token carries no tenant; the tenant comes from the provisioning policy), still MissingClaim when a claim is named. 6 identity tests (verified issuer/subject, nonce-mismatch refused, tampered-token refused, mapped tenant claim, github numeric id, no-stable-id refused) + 1 verifier test. corvid-runtime lib 346 green; serve_auth 11 green; workspace clean.
    - [x] **52e-4c route handlers + serve wiring** — DONE 2026-07-23. `serve_auth::routes::auth_router()` mounts `/auth/{provider}/login|callback|logout|session`; `corvid serve` parses the identity block from the AST (not lowered into IR), builds an `AuthContext` (in-memory `SessionAuthRuntime` + reqwest gateway/JWKS + policy/cookie translation), and merges the auth router via `with_state`. A missing credential (or OIDC discovery failure) REFUSES TO START naming it. login → mint PKCE(`pkce.rs`)+state+nonce, store the single-use Login state, 302 to authorize URL (PKCE+nonce for OIDC only). callback → strict order in `run_callback` (resolve state → `resolve_identity` → `provision_login` → `create_session`), all in `spawn_blocking` (reqwest blocking can't run in a Tokio ctx); failures are an opaque 401, audited. logout → `revoke_session` + clear cookie; session → `resolve_session_cookie` → actor JSON. Cookie carries the declared Secure/HttpOnly/SameSite. New runtime helper `resolve_session_cookie` (tenant from the stored session — a cookie has no tenant context). New files `pkce.rs` / `net.rs` (reqwest `HttpProviderGateway`) / `context.rs` / `routes.rs`. Live serve_smoke test: `/auth/google/login` 302s to Google with client_id + state + PKCE S256 + nonce; `/auth/session` no-cookie → 401 `authenticated:false`; unknown provider → 404. 22 serve_auth units + serve_smoke green.
    - [x] **52e-5 mock-IdP + adversarial e2e + invention proof** — DONE 2026-07-23. `run_callback` driven end-to-end against a MockIdp (OIDC ID token + JWKS) and a mock userinfo gateway, deterministic (no live network): 6 `callback_tests` — open provisions a new actor then RECOGNISES the same subject on the next login; invited refuses without an invitation and admits into the invitation's tenant with one; a reused single-use state / a nonce mismatch / a tampered token are each refused; a userinfo (github) login provisions from the numeric user id. Invention proof: `corvid tour --topic oauth-login` (source compiles), README catalog entry, `docs/reference/inventions.md` §7 subsection (`#first-login-is-an-explicit-compile-time-decision`) + Proof-Matrix row. **Phase 52e COMPLETE** — the OAuth login surface is mounted, provisioning is a compile-time decision, and every provider resolves with no shortcut. 52e does NOT flip `auth_enforcement` (that is 52f — the last closure gap).
- [ ] 52f-authz-enforcement   Route-level enforcement of `requires authenticated|role|permission`: session + CSRF double-submit + role/permission checks run before the handler; the typed `actor` is populated from the verified session. Authenticated + DURABLE approval endpoints (replace the in-memory `/__approvals` with the durable approval queue).

  **DESIGN (2026-07-23, user chose "full role model now"):** the runtime's permission model was fingerprint-equality only, with no role→permission mapping or role-assignment surface. 52f builds the full model under LEAST PRIVILEGE — an open signup gets NO role unless the app declares a `default_role` (granting authority is never silent). Decomposed into always-green increments:
  - [x] **52f-1 roles declaration surface** — DONE 2026-07-23. `roles:` sub-block on the identity block (`name: "perm, perm"` → `RoleDecl { name, permissions }`) + `provisioning: default_role: <role>` (the role an open signup receives; `None` = least privilege). Checker: roles uniquely named + non-empty; `default_role` must reference a declared role; **`requires role("x")` / `requires permission("p")` must reference a name declared in `roles:`** (new `TypeErrorKind::RoutePolicyUndeclaredAuthority`) — an unsatisfiable requirement is a compile error, not a silent always-deny. Differential-verify renderer round-trips roles + default_role. AST/parser/checker + 1 parser + 5 checker tests; corvid-types 305 + corvid-abi 77 + differential-verify 23 green; corpus verify exits 1 on the two fixtures. NO behavior change yet (policy routes still refuse to start — enforcement is 52f-2/3).
  - [x] **52f-2 role storage + assignment + resolution** — DONE 2026-07-23. `auth_actor_roles` table + `grant_actor_role` (idempotent) / `actor_roles`; the invitation's `role_fingerprint` field became a role NAME (`role`), granted at provisioning; an open signup gets `provisioning.default_role` or NO role (least privilege — `provision_login` gained `default_role`, threaded through `AuthContext`); pure decision helper `authorize_route(actor_roles, role→perm map, RoutePolicyRequirement) -> RouteAuthzOutcome` — membership (holds every required role) + permission union (every required permission ∈ union of held roles' permissions), NOT fingerprint equality. New `auth/roles.rs`. 5 role tests (grant idempotent+queryable, held-role authorized, missing-role denied, ungranted-permission denied, union-across-roles) + open-default-role provisioning test. corvid-runtime lib 352 + serve_auth 28 green; workspace clean; corpus verify exits 1 on the two fixtures. Enforcement is still not wired into request handling (52f-3).
  - [x] **52f-3 serve enforcement + capability flip** — DONE 2026-07-23. **The LAST Contract Closure gap is closed.** IrRoute carries the lowered `IrRoutePolicy`; the ONE `AuthContext` is shared into `ServeState` (app routes + the login router resolve sessions against the same store); `AuthContext` gained `role_permissions` + a per-serve `csrf_secret`; the login callback sets a readable `corvid_csrf` cookie. `enforce_route_policy` runs BEFORE the handler / any effect: resolve session cookie (401 missing/invalid/expired/revoked; tenant↔actor consistency is part of resolution, so tenant is enforced before roles) → CSRF double-submit on state-changing methods (403 on mismatch) → `authorize_route` roles+permissions read FRESH (403 insufficient) → bind the verified typed `actor` (id/tenant/display_name/roles/permissions), NEVER a request-supplied actor. Every decision emits a redacted `route.authz` audit event (route, actor ref, tenant, requirements, outcome, trace = evidence). `revoke_actor_role` invalidates the actor's sessions (privilege change → re-auth); roles read fresh means revocation is immediate. `auth_enforcement: true` flipped in `interpreter_tier()`. Adversarial: `crates/corvid-runtime/tests/route_authz.rs` (8 — forged cookie, expired, revoked-via-role-revocation, cross-tenant, authenticated-but-insufficient, CSRF mismatch, permission-union, happy path) + serve_smoke `serve_enforces_a_requires_authenticated_route_instead_of_refusing_to_start` (401 + no body leak) + the 5 `roles.rs` unit tests. **Examples/docs updated in the same increment** (acceptance): tour `contract-closure`, README, inventions.md §7 + Proof Matrix (new authz row) + core-semantics + guarantee registry — no material still claims authenticated routes refuse to start. Corvid can now honestly claim: **if a route declares authentication, roles or permissions, the runtime identifies the actor and enforces that authority before any handler or effect executes.**
  - [x] **52f-4 durable approval + session persistence** — DONE 2026-07-23. Persistence is OPT-IN via an explicit `CORVID_SERVE_DATA_DIR` (where sensitive auth/approval data lives is the operator's choice, never a silent hardcoded path). When set, the approval queue (`approvals.sqlite`) + the session/external-identity store (`auth.sqlite`) persist there and survive a restart; unset stays in-memory (a dev restart fails closed). `ApprovalQueueRuntime::open(path)` + `AuthContext::from_identity(identity, data_dir)`; startup banner names the mode. serve_smoke `a_queued_approval_survives_a_restart_with_a_durable_data_dir` proves a queued approval is still listed after stop→start against the same dir. **Deferred to 52f-4b: authenticated approval endpoints** — the `/__approvals/:id/{approve,deny}` transitions still run as the anonymous `serve-reviewer` actor; requiring a verified reviewer holding the `operator` role is a consequential access decision (who may approve?) that gets its own increment reusing the 52f-3 enforcement.
  - [ ] **52f-4b authenticated approval endpoints** — the approve/deny transitions require an authenticated actor holding the `operator` role (reuses 52f-3 `enforce_route_policy` + the `SERVE_REVIEWER_ROLE` constant); replaces the hardcoded anonymous reviewer.
  - [ ] **52f-5 invention proof + adversarial e2e** — mock-IdP login → role-gated route allowed/denied; tour topic / inventions row update; adversarial (no session, wrong role, missing permission, CSRF mismatch).

### C · Protocol-Typed Connectors

- [ ] 52g-connector-operation-surface   The `connector`/`operation` source surface (zero-glue integrations): `connector` config (base_url, auth via `secret(…)`, reliability: retry/rate_limit/circuit_breaker, defaults: idempotency/redact/replay) + `operation` (HTTP verb + path + `body`/`form`, an effect row so budgets/approval/replay/taint compose, response status→typed-error mapping). Lowers onto the Phase 41 connector runtime; an `operation` is a tool with a declarative body.
- [ ] 52h-async-state-machines   Async provider operations (`submit … await: poll … every … timeout … when status:`). Compile-time proofs: the state machine terminates or has a timeout, every provider status is handled, retry cost contributes to the max budget, irreversible submission has an approval boundary, replay never polls the real provider, secrets never enter traces, idempotency is present on retried writes.
- [ ] 52i-provider-drift-quarantine   Validate each real response against the declared protocol shape; on mismatch, QUARANTINE (no value coerced, no write retried), emit a trace + generate a fixture under `tests/provider-drift/`. Composes the Phase 41 `detect_contract_drift` surface.

### D · Proof-Carrying Data

- [ ] 52j-data-and-query   `database` (dev/prod backends) + `table` declarations (columns with types, `primary`, `partition_key`, `sensitive(class)`, `tenant_scope`, `retention`, defaults) + the typed query builder + the compile-time tenant-scope proof (unscoped query on a `partition_key` table = compile error; `.across_tenants(approve: …)` escape).
- [ ] 52k-proof-carrying-data   Persistence that preserves properties: a stored `Grounded<T>` writes its provenance sidecar (source ids, content hashes, retrieval timestamp, model/prompt versions, derivation graph) and reads back as `Grounded<T>`; `sensitive(…)` composes with redaction; `retention` drives a reaper. Postgres adapter + connection pooling for prod. Honesty non-scope: provenance is preserved by Corvid's write path, not authenticated against out-of-band writes.
- [ ] 52l-proof-carrying-migrations   `migration N "desc":` (add/remove/alter + `compatibility: reads/writes versions` + `rollback:`) and the verification engine that (before deploy) builds the old schema, populates adversarial data, applies the migration, verifies constraints + tenant isolation + Grounded provenance survival, exercises old-reader/new-writer compat, tests declared rollback, estimates lock/rewrite risk, classifies destructive ops, and requires approval for dangerous migrations. Output in the four-level taxonomy.

### E · Lifecycle & Tooling

- [ ] 52m-jobs-and-lifecycle   Durable jobs + `schedule`s composed INTO `corvid serve` (not a separate runner); server lifecycle: `/healthz`/`/readyz` reflecting real readiness (migrations current, providers reachable-or-mocked), graceful shutdown draining in-flight requests + jobs.
- [ ] 52n-dev-unified   `corvid dev` becomes the complete local system: backend + authentication + database (auto-provisioned dev SQLite + migrations) + jobs + the generated console (driving the LIVE backend, not a configurable URL) + mock providers + traces/approvals + hot reload on source change.
- [ ] 52o-prove-pipeline   `corvid prove`: an EXECUTED gate pipeline (compiler/effect verification, route/contract equivalence, authz reachability, approval-bypass adversarial, schema fuzzing, replay determinism, eval thresholds, provider mock/failure simulation, migration verification, config/secret checks, dependency/SBOM, deploy smoke test) producing a signed **Evidence Graph** (per-node producer/inputs/timestamp/result/classification/limitations/signature) + evidence **freshness** (content-addressed: changing source/deps/model/prompt/schema/connector/fixture/env invalidates only the affected nodes and re-runs them) + the four-level bundle.
- [ ] 52p-deploy-and-self-verify   `corvid deploy`: build → prove → build the OCI image → smoke-test the actual image → sign the evidence graph → deploy exactly what was tested. A `deploy:` policy block (`require contract_closed | migrations_verified | eval(…) >= … | no_unapproved_irreversible_effects`; unprovable requires rejected at parse). Runtime self-verification: `/.well-known/corvid/evidence` (source/contract/image/migration/bundle/runtime hashes); readiness FAILS if the deployed contract or migration differs from the signed evidence — the `source → proof → image → deployed runtime` chain.

### F · Proof

- [ ] 52q-reference-application   The one-file reference application — grown continuously from the 52a stub through every slice — CERTIFIED to exercise the entire Application Contract surface (identity, data, connectors, agents, routes, jobs, approvals, streaming, uploads, pagination, errors, deploy policy) with no exceptions or special cases. The anti-demo: if a feature can only be shown by special-casing, it is not done.
- [ ] 52r-clean-machine-and-external   Clean-machine reproducibility (fresh OS image, no maintainer environment) + an external developer building a non-trivial app with zero maintainer assistance + a final adversarial/completeness sweep. Certifies the completion criterion.
...
- **Agentic authoring helpers (re-filed by slice 48c, 2026-07-14).** `corvid connectors mock-fixture-gen` (LLM-generated mock fixtures), `corvid connectors fail-sim` (LLM-driven fault synthesis), `corvid deploy tailor`, and `corvid upgrade assist`. Each accelerates authoring on top of a deterministic surface that already shipped complete (connector test-kit fixtures + adversarial corpus; deploy manifests; upgrade migrations + release notes). They need live-LLM authoring loops that cannot be honestly validated offline in CI, and none closes a guarantee gap. (`corvid beta synthesize-feedback` is NOT here — it rides the launch phase with the beta program it operates on.)
...
- **User-declared generics (45p resolution, 2026-07-09).** `fn first<T>(xs: List<T>)` and generic user types per book ch 05. Needs a type-variable representation the `Type` enum lacks (`crates/corvid-types/src/types.rs:20-145`) plus monomorphization or dictionary-passing through IR + all three codegen backends — the largest single item the 2026-07-09 language gap audit surveyed. v1.0 covers the workload with the special-cased `List`/`Option`/`Result` heads, builtin methods (45c), and plain non-generic `fn` (45r). The book's generic examples carry a Planned marker naming this entry until it ships.
