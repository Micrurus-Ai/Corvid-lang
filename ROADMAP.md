...
- [ ] 45i-match-expression   **[closes audit B1 — the largest single slice]** `match` expression + statement with literal / variant / struct-destructure / binding (`x @ pat`) / wildcard / guard (`if` clause) patterns and compiler-checked exhaustiveness over 45h sum types + `Option`/`Result`/`Bool`. Also the destructuring forms book ch 13 teaches: `Decision { refund, amount, .. } = expr` destructuring bindings (keyword-free, matching the 45a annotated-assignment decision) and destructuring in `fn`/agent params. Parser (`KwMatch` token), AST pattern nodes, checker (exhaustiveness + binding types), IR decision-tree lowering, interpreter, codegen or explicit degradation. Book ch 13 moves from Planned back to shipped, block-by-block compile-tested. This also closes audit B6's worst half: `Ok`/`Err`/`Some`/`None` become inspectable.
