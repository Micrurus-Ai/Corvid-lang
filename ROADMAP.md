...
- [ ] 45m-datetime-and-math-builtins   **[subsumes 33R5e-datetime + 33R5f-math]** `std/time.cor` executing surface (`time_now_utc`, monotonic elapsed, ISO-8601 parse/format, duration arithmetic — effect-tagged non-deterministic, replay-substituted like other executing tools) + math builtins (`abs`, `min`, `max`, `pow`, `sqrt`, `floor`, `ceil`, `round`, plus `random` as an effect-tagged tool). Full invention contract.
