...
- [ ] 50b-behavioral-diff   `corvid behavior diff`: replay a recorded scenario corpus against changed prompts/agents and report exactly which scenarios' outputs changed (count + per-scenario diff). Makes agent changes REVIEWABLE in PRs — no team can do this today. Builds on differential replay.
- [ ] 50c-model-upgrade-diff   The same machinery pointed at a model swap: `corvid behavior diff --model <new>` answers every provider deprecation email with evidence instead of fear.
