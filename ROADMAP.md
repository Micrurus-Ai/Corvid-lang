...
- [ ] 46g-rag-stdlib-dispatch   **[closes audit M8]** The runtime already has real embedders (`rag/embedders.rs:9-68`) and a SQLite cosine index (`rag/index.rs:191,258`); `std/rag.cor` is envelope-only. Add `is_stdlib_rag_tool` dispatch (the io/http/db/json pattern, now 5th surface): `rag_ingest`, `rag_search`, embedding-model config. Full invention contract + replay semantics (embedder calls are recorded/substituted).
