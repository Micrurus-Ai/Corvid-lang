...
- [ ] 45k-while-loop   `while cond:` (book 04:102 already claims it) + interaction with `break`/`continue`; also promote `break`/`continue`/`pass` from sentinel-`Ident` encoding to real AST variants (`parser/stmt.rs:181-207` "A future cleanup" note comes due).
