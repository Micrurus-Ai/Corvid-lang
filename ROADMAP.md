...
- [ ] 33R-market-readiness-track   **[launch-blocker — OSS adoption gap remediation]** 14-slice track derived from the 2026-06-08 market-readiness audit (`docs/market-readiness-audit.md`). The audit's verdict: Corvid is engineered far past `0.0.1`, but adoption is blocked by last-mile surfacing — no LICENSE on disk, no package registry, README is invention-catalog-shaped instead of an adoption funnel, stdlib is contracts-only, VS Code extension and crates.io unpublished. None of these are missing-capability problems; they're packaging + hosting + docs. Track runs in tiered order P0 → P3; lower tiers do not start until higher tiers close. Canonical decisions captured at filing time: repo URL = `github.com/Micrurus-Ai/Corvid-lang`; canonical domain = `corvid-lang.org` (to be served from the existing `web/` Cloudflare Worker); copyright holder = `Disan Ssebowa Basalidde`; license = MIT (narrowed from the prior `MIT OR Apache-2.0` claim, which never had on-disk text to back it); registry shape = GitHub Releases for artifacts + static `index.json` from the same Worker.
  - [ ] 33R1-license-files   **[P0]** Add `LICENSE` (MIT, `Copyright (c) 2026 Disan Ssebowa Basalidde`) at repo root. Update `README.md`'s License section to link the file and state inbound = outbound contribution convention. Narrow `Cargo.toml` workspace license from `MIT OR Apache-2.0` to `MIT`; align every member `Cargo.toml` (`license.workspace = true` where it isn't already). Acceptance: `grep -rn "MIT OR Apache-2.0\|Apache-2.0" .` returns no live hits; GitHub's license detector resolves to MIT; README links resolve.
  - [ ] 33R2-repo-identity-unify   **[P0]** Find/replace every reference to `github.com/corvid-lang/corvid` and `Micrurus-Ai` (if any other org spelling exists) → `github.com/Micrurus-Ai/Corvid-lang`. Update `Cargo.toml` (`repository`, `homepage`, `documentation`), `install/install.sh`, `install/install.ps1`, `web/worker.js`, `web/wrangler.toml`, and any docs/** that hardcodes a URL. Switch domain references from `corvid.dev` (no longer in use) to `corvid-lang.org`. Acceptance: `grep -rn "corvid-lang/corvid\|corvid.dev"` returns no live hits.
  - [ ] 33R3-readme-adoption-funnel   **[P0]** Restructure top of `README.md` as a funnel: (1) one-sentence pitch, (2) 3-line differentiator, (3) install one-liner, (4) `corvid new hello && cd hello && corvid run`, (5) links to `docs/book/02-quickstart.md`, `corvid tour --list`, the book index. Move invention catalog below the fold. Add Status badge line linking to ROADMAP + the stability statement (33R8). Doc-only — does not fix the E0301/E0101 quickstart drift (33R12).
  - [ ] 33R4-package-registry   **[P1 — invention; gated sub-slices]** Stand up a minimal hosted package registry so `corvid add <pkg>` resolves. Hosting decision locked: GitHub Releases (artifacts, content-addressed by sha256) + static `index.json` served from the existing `web/` Cloudflare Worker.
    - [ ] 33R4a-registry-shape-decision   Pre-phase chat — agree exact `index.json` schema, publish flow (`gh release create` + index regen), and signature-verification path on the client. No code in this slice.
    - [ ] 33R4b-client-default-registry-pointer   Point the `corvid` client at the chosen Worker URL by default; keep `--registry` / `CORVID_PACKAGE_REGISTRY` overrides. Tests for resolve against the live index shape.
