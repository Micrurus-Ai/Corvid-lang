...
- [ ] 33Q-trial-round-3-code-findings  **[33M-blocker — ten findings from the maintainer-as-reviewer-2026-06-05 trial report (`0921ec5`)]** Self-administered trial by the maintainer playing reviewer-#2 to find what reviewer-#1's two rounds didn't. Built a small production-shape cyber-threat-intel app (5 of 6 surfaces) and documented every friction point. Triage at [`docs/external-trials/33m-trial-maintainer-as-reviewer-2026-06-05.md`](docs/external-trials/33m-trial-maintainer-as-reviewer-2026-06-05.md). Three P1 launch-blockers (33Q6, 33Q7, 33Q8) plus six P2/P3/Minor follow-ups (33Q9-33Q12).
  ...
  - [ ] 33Q7-spec-reference-apps-drift-gate   The spec doc at `docs/internals/effect-spec/04-builtin-dimensions.md:65-66` documents the trust lattice as `autonomous < supervisor_required < human_required`, but `examples/backend/customer_support_agent/src/main.cor` uses `bounded`, `workspace`, `grounded` without any `corvid.toml` declaring them as custom dimensions. A reviewer copy-pasting from reference apps hits typecheck rejections; a reviewer trusting the spec is contradicted by the reference apps. CI step needed: grep every `examples/backend/*/src/main.cor` for `trust:` / other dimension declarations and cross-check against the spec's authoritative lists. Either the spec is incomplete (then update it) or the reference apps are broken (then fix them OR document the dimension-discovery mechanism that lets them work). Filed by maintainer-as-reviewer-2026-06-05 P1.2.
  - [ ] 33Q8-pub-extern-c-struct-boundary   `pub extern "c"` agents reject struct parameters and returns ("extern \"c\" agent uses unsupported ABI type `struct` in parameter"). Phase 20n-C lifted struct codegen for internal sites (prompt bridges, entry-agent boundaries) but the `pub extern "c"` boundary still rejects them, killing the signed-cdylib path for any production-shape HTTP backend with structured request/response bodies. The diagnostic's Help text honestly names this as a Phase 22 follow-up; this slice lifts it (re-using 20n-C's struct decoder/encoder + `corvid-prompt-format`'s JSON schema generator for the boundary). Acceptance: an agent with `pub extern "c" agent foo(req: StructReq) -> StructResp` builds with `--target=cdylib --sign` and the resulting `claim --explain` lists `foo` with its struct boundary types. Filed by maintainer-as-reviewer-2026-06-05 P1.3.
  - [ ] 33Q9-serve-approval-label-accuracy   `corvid serve` route table labels routes as `approval-gated -> 202 + queued` even when the agent has no syntactic `approve` boundary (the label fires for any agent whose `@trust(autonomous_if_confident(t))` constraint declares a possible runtime escalation). Misleading for reviewers planning client-side polling. Fix: only emit the label for routes whose IR contains a reachable `IrStmt::Approve`; emit a different label (e.g. `may queue if confidence < t`) for confidence-gated routes. Acceptance test pins the label-per-route-shape map. Filed by maintainer-as-reviewer-2026-06-05 P2.1.
  - [ ] 33Q10-serve-error-detail-clean   500 response bodies from `corvid serve` leak internal IR byte-span ranges (`[1227..1269]`) to HTTP clients. Strip the span prefix from the `detail` field on the 500 path (or replace with a source-line reference like `src/main.cor:24` that's at least human-meaningful). The span is already in the server's trace events; the client doesn't need it. Fix lives in the error-formatting branch of `crates/corvid-cli/src/serve_cmd.rs` (the `Err(err) => err.to_string()` path on `/approve` and the dispatch handlers). Filed by maintainer-as-reviewer-2026-06-05 P2.2.
  - [ ] 33Q11-deploy-package-atomic   `corvid deploy package` leaves partial artifacts in `deploy/` when `CORVID_DEPLOY_SIGNING_KEY` is missing (6 of 9 expected files land before the attestation step that needs the key, then errors). All-or-nothing contract: either `deploy/` is fully populated OR no artifacts are written. Fix shape: fail-fast on missing env before the first file write, OR write to a tempdir and atomic-rename on success. Also surface `CORVID_DEPLOY_SIGNING_KEY` in `corvid deploy package --help` (the prior `--help` lists only `--out` and `--cdylib`). Filed by maintainer-as-reviewer-2026-06-05 P2.3 + P3.1.
  - [ ] 33Q12-misc-polish   Three lower-severity findings: (a) `std.db` is metadata-envelopes-only — persistence requires writing tool wrappers around your own SQL, not the first-class typed query primitives the README implies. Docs slice updates the `std.db` README to be honest about the v1.0 scope vs the post-v1.0 source-syntax sugar (`35V2-P39-I`-style slice). (b) Windows path-separator mixing in OCI labels (`C:/Users/.../Temp/threat_intel_agent\\src\\main.cor`) — small `Path::display()` normalization in `deploy_cmd.rs`. (c) Build error for missing `pub extern "c"` agent has `[0..0]` zero-width span + awkward "native codegen does not yet support:" phrasing; tighten to a single-line error that names a doc page on the exported-ABI surface. Filed by maintainer-as-reviewer-2026-06-05 P3.2, P3.3, and Minor.
