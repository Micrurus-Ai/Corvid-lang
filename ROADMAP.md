...
- [ ] `35V2-P42-H-LR-per-app-ai-helpers`        Three per-app AI helpers (assistive boot summary, adversarial-test refresh, generative PR description). Each is a deterministic typed classifier over the app's ABI descriptor with Grounded<T>-shaped sources — mirroring the drift-narrator posture (`connector.drift_narration_grounded`). Each promotes a new RuntimeChecked guarantee row under a freshly added `GuaranteeKind::App` (introduced 2026-05-30 alongside H-LR-1 to keep the registry's prefix-invariant honest as the per-app helper cluster grows).
  ...
  - [ ] `35V2-P42-H-LR-2-app-adversarial-refresh` `corvid app adversarial-refresh <source.cor>` — deterministic typed walker over the app's surface that suggests adversarial fixtures (cross-tenant variants, missing-budget edges, approval-bypass attempts) with sources back-referencing each surface element. Promotes `app.adversarial_refresh_grounded`.
  - [ ] `35V2-P42-H-LR-3-app-pr-describe`         `corvid app pr-describe <base-claim> <head-claim>` — diffs two `CLAIM.md` / ABI descriptors and renders a typed `PrDescription` with `sources` back-referencing each diff (added agents, removed approvals, surface_count deltas, attestation changes). Promotes `app.pr_describe_grounded`.
