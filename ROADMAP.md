...
- [ ] 52d-effect-aware-scheduling   `parallel` scheduling that is effect-aware: the runtime knows the branches' combined cost, rate-limit domains, dangerous/reversibility surface, and whether replay may contact providers. The cancellation×reversibility rule: a branch past a non-reversible effect boundary is not cancellable.
  ...
  **Design locked (pre-phase design consult 2026-07-22): the FULL model — cancellation is real AND replay reproduces it deterministically** (CTO chose the most faithful option over "live-only cancel, replay stays sequential" and "awareness/admission only"). Today `parallel` runs ALL arms via `join_all`, buffers per-arm traces, flushes in arm order, and replay re-runs arms sequentially. 52d changes the LIVE semantics to fail-fast reversibility-guarded cancellation and makes the TRACE + replay reproduce it. Decomposed by risk/dependency (each an always-green, committable increment):
  ...
  - [ ] **52d-1 effect-profile awareness** (safe groundwork, no execution-semantics change → replay-safe): before a `parallel` block runs, compute each arm's static effect profile (transitive cost, composed `reversible` dimension, rate-limit domain, whether replay may contact providers) from the effect registry, plus the combined profile. Record a `parallel_scheduled` trace event. Combined-cost admission: refuse to start the block (fail-fast, structured error) when the arms' summed cost ceiling would exceed the enclosing `@budget`, instead of running all arms then charging after. Rate-limit-domain serialization: arms sharing a rate-limit domain run serially, not concurrently.
