...
- [ ] 51a-application-contract-core   The contract data model + emitter (mirrors `emit_abi`): public types with field refinements → JSON Schema constraints; routes (params/body/response/status/auth); public agents + prompts; effects/permissions/cost/latency/confidence/grounding capabilities. Foundation everything else consumes. `corvid check` validates it can be generated; `corvid build` writes `target/contracts/app.corvid.json`.
