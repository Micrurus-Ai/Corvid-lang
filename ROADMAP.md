...
- [ ] 48b-connector-grounded-returns-disposition   **[= the open Phase 41 provenance item, `35V2-P41-G-LR-connector-grounded-returns`]**   Connector returns today are plain JSON; the `Grounded<T>` wrapping follows from the `connector ... grounded` source syntax that is post-v1.0 sugar (`35V2-P41-I`). Decide honestly: pull the sugar forward, wrap at the runtime boundary without new syntax, or re-scope the launch-readiness item to what ships — no silent middle.
