...
- [ ] 52e-auth-routes-mounted   Auto-mount `/auth/{provider}/login|callback|logout|session` (from the identity block) wired to the existing `corvid-runtime/src/auth` storage: Authorization Code + PKCE, signed/expiring state, OIDC nonce, JWKS verification, iss/aud/exp/nbf, secure http-only cookies, session issuance/rotation.
  ...
  **Scoped 2026-07-23 (Section A milestone banked; 52e opens Section B, gets a focused session — it is the OAuth SECURITY surface, all-or-nothing, so no shortcuts).** The machinery all exists; 52e WIRES it into `corvid serve`.
  ...
  - **CRITICAL FINDING (2026-07-23, deeper investigation).** The storage-layer OAuth is the account-LINKING model, NOT first-login: `OAuthStateCreate` requires an `actor_id` that references a PRE-EXISTING actor (the oauth.rs tests `upsert_actor(...)` BEFORE `create_oauth_state`, and `resolve_oauth_callback` returns that bound `actor`). First-time OAuth LOGIN — actor unknown until the verified ID token arrives at callback — has no clean path in the current storage layer. So 52e is NOT pure serve-side wiring; it needs a small STORAGE-LAYER design decision + change: either (a) an actor-optional first-login state variant, or (b) a per-tenant "pending" placeholder actor bound at login and replaced by the real ID-token actor at callback. **Design decision to settle first (product + security policy):** how does a first-time OAuth login provision its actor? Default under the decision principle = auto-provision from the VERIFIED ID token at callback (`sub`→actor id, email/name from claims, tenant from provider config or a tenant claim) — standard SSO — with invite-gating/pre-provisioning as a later knob. This decision + the first-login state model MUST be settled before writing route handlers (no shortcuts on the auth surface). This is why 52e is a genuine focused unit, not a wiring pass.
