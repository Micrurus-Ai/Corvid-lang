...
- [ ] 49b-signed-skills-and-sources   DSSE signature verification on skills (reuse the cdylib attestation crates; `corvid skill sign` for authors); git/github sources with shallow fetch; content-hash pinning in the lockfile; `corvid skill update` (hash-diffed, re-audited on label change). Tests: signature verify/reject, hash pin round-trip, update-with-label-change requires re-consent.
