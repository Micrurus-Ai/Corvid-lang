...
- [ ] 44e-readme-streaming-claim-alignment   README markets "Streaming" as a headline construct; all four provider adapters defer token streaming (`llm/anthropic.rs:13`, `openai.rs:11`, `gemini.rs:16`, `ollama.rs:89-91`) and a `-> Stream<T>` prompt wraps one blocking response in a singleton chunk (`interp/prompt/mod.rs:35-86`). Align the headline with the shipped boundary (the README body already discloses it at line ~456); cross-link 46d as the implementation slice. Acceptance: no README claim implies live token flow before 46d ships.
