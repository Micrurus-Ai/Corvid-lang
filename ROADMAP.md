...
### Phase 45 — Core expression completeness (~10–14 weeks) — **the language-completeness phase**
...
- [ ] 45r-fn-pure-function-declarations   **[direction-locked 2026-07-09 — implements book 04:36-44 / 05:81-107 (non-generic forms)]** Plain `fn name(params) -> Ty:` declarations as a fourth callable kind alongside agent/tool/prompt. Semantics: statically effect-free (empty effect row enforced by the checker — a `fn` body may call other `fn`s and pure builtins but no tools/prompts/agents), synchronous, callable from `@deterministic` contexts and from everywhere agents can call. Parser (`KwFn` token + decl production), AST `FnDecl`, resolver, checker (effect-freedom enforcement + return-type checking), IR lowering (no async machinery — direct call), interpreter, codegen on all three backends (pure functions are the EASY codegen case — no dispatch boundary), LSP symbol support. Book ch 04/05 `fn` examples become real (generic forms stay Planned per 45p). Depends on 45a (let) for the book examples' bodies; sequence after 45c so `fn` bodies can use builtin methods. Full invention-contract treatment not required (it's a standard language feature, not a Corvid-specific invention) but tour + book + tests are.
