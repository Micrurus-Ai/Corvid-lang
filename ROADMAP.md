...
- [ ] 49z-verify-temp-leak   **[filed 2026-07-14 — found live: 11.1 GB of `%TEMP%/corvid-verify` artifacts]** `corvid verify --corpus` leaves its per-run native build artifacts in the temp dir and never cleans them; on this dev machine they accumulated to 11 GB and filled the disk mid-session. Make verify clean up per run (or reuse one content-keyed dir); a bounded cache is fine, unbounded growth is not.
