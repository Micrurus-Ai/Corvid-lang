...
- [ ] 52e-auth-routes-mounted   Auto-mount `/auth/{provider}/login|callback|logout|session` (from the identity block) wired to the existing `corvid-runtime/src/auth` storage: Authorization Code + PKCE, signed/expiring state, OIDC nonce, JWKS verification, iss/aud/exp/nbf, secure http-only cookies, session issuance/rotation.
  ...
  **Scoped 2026-07-23 (Section A milestone banked; 52e opens Section B, gets a focused session — it is the OAuth SECURITY surface, all-or-nothing, so no shortcuts).** The machinery all exists; 52e WIRES it into `corvid serve`.
  ...
  - **DECISION (2026-07-23, user): "verified userinfo for all providers."** The identity AST permits providers that do NOT issue OIDC ID tokens (github/slack/discord are OAuth2-only), yet the callback model verifies an ID token's `(issuer, subject)`. Rather than refuse those providers (they appear in the shipped README/tour surface), support EVERY declared provider with no shortcut: OIDC providers verify the ID token → `(issuer, subject)`; OAuth2-only providers do a server-to-server userinfo fetch over TLS after code exchange → `(provider_marker, authoritative_user_id)`. Identity is ALWAYS a server-side-verified value, never a client-forgeable claim. The external-identity key generalises from `(issuer, subject)` to `(identity_source, external_id)` — the existing `auth_external_identities(issuer, subject)` columns already hold it (identity_source in `issuer`, external_id in `subject`), so 52e-2/3 need no change.
    ...
    - [ ] **52e-5 mock-IdP + adversarial e2e + invention proof** — MockIdp (OIDC) + mock userinfo (OAuth2) login→callback→session→logout; tampered/expired/replayed state, wrong nonce/aud/iss, missing PKCE; the E5210 compile-error test; tour topic + inventions.md row.
