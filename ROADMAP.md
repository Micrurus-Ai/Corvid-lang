...
- [ ] 51j-connector-user-auth   `connector ... authorize per_user` with approval-gated scopes and per-user token flow, kept SEPARATE from the login session (identity token vs. workspace access token). Composes Phase 41 connectors.
