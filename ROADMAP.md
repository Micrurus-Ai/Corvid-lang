...
- [ ] 51h-auth-routes-and-actors   Auto-exposed `/auth/{provider}/login`, `/callback`, `/logout`, `/session`; typed `Actor`, `requires authenticated`, role/permission route policies compile-checked. EVERY safe default MANDATORY (Authorization Code + PKCE, signed/expiring `state`, OIDC `nonce`, exact redirect-URI allowlists, JWKS signature verification, `iss`/`aud`/`exp`/`nbf` validation, secure http-only cookies, session rotation on privilege change, CSRF, refresh-token rotation, encrypted provider tokens, revocation, redacted logs/traces, minimal scopes). Unsafe variants require a loud explicit opt-out.
