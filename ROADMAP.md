...
- [ ] 45j-lambdas   `fn (x) -> expr` / block form, capture-by-value semantics decision (pre-phase chat: interaction with the ownership/RC model — Phase 17b context), checked function types (currently `(Int) -> Int` silently resolves `Unknown` — `checker/types.rs:47`), then `List.map`/`filter`/`fold`/`any`/`all` land on the 45c table.
