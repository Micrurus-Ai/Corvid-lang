...
Native cross-file struct codegen (`G0`) — **inserted 2026-05-28 before `G-LR`**. Reason: `G-LR` commits each app's `corvid claim --explain` output, which requires building the app as a signed cdylib. A cdylib needs ≥1 `pub extern "c"` agent (a real, scalar-signature entrypoint) — but the native codegen lowers *every* agent and rejects `Type::ImportedStruct` params/returns (the apps return std types like `Actor` / `ApprovalContractRef` / `SessionRef`). The DefId-panic fix (`ec19e31`) cleared ABI emission; this track clears native lowering. Per the no-shortcuts mandate we add real `ImportedStruct` native support rather than scope codegen to the exported closure (which would silently drop the catalog ABI for the unreachable agents). Verified root cause: imported-struct *construction* already uses the cross-module-remapped DefId (`remap_imported_target`), but the two root-file type resolvers (`resolve_root_lifted_type_ref` / `resolve_root_imported_type_ref`) emit the *type* with the original per-module DefId, so it misses the `ir.types` layout table (keyed by the remapped id). No non-codegen consumer reads `ImportedStruct.def_id` (abi / vm / codegen-py use `.name`), so the remap is safe.
...
- [x] `35V2-P42-G0-imported-struct-2`  Codegen: `cl_type_for` / `reject_unsupported_types` accept `Type::ImportedStruct` (width I64, identical to `Struct`). Advances the build past the agent-signature gate to the next blocker (field access). Unit test `cl_type_for_treats_imported_struct_like_local_struct`. Closed `(this commit)`.
- [ ] `35V2-P42-G0-imported-struct-3`  Codegen: `FieldAccess` resolves an `ImportedStruct` target to its def_id (mirror the `Struct` arm).
- [ ] `35V2-P42-G0-imported-struct-4`  Codegen: entry-agent + native-prompt boundaries accept `ImportedStruct` (route through the existing struct JSON encoder/decoder). Then add the scalar `pub extern "c"` entrypoint per app + build all 5 cdylibs.
