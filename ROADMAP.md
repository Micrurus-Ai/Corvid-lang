...
- [ ] 45b-assignment-targets   `x.field = v`, `xs[i] = v`, compound `+= -= *= /=` (`parser/stmt.rs:211-233` currently parses bare `IDENT = expr` only). Checker + IR + interpreter + all three codegen backends or explicit `not_supported`.
