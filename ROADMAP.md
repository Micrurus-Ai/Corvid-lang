...
- [ ] 48a-executing-secrets-and-cache   **[= slice 47i, promoted; design decided at the 2026-07-14 pre-phase chat per the standing delegation]**   (1) SECRETS: `secret_read(name) -> Result<SecretReadEnvelope, String>` over `secrets::SecretRuntime`, with the envelope gaining a value channel the PROGRAM sees but the TRACE never does. Replay semantics decided: **redacted recording + re-read-on-replay** — the trace records the envelope with the value redacted, and Substitute-mode replay re-executes the env read instead of substituting, exactly the `db_query` read-passthrough precedent (env reads are process-internal inputs; they cannot escape). If the env differs at replay time the run diverges HONESTLY. Residual channel stated as explicit non-scope: a secret the program forwards into another tool's ARGUMENTS is recorded by that tool's own trace events — the structural fix (an opaque `SecretHandle` Value that never serializes, the DbHandle pattern, accepted by consuming surfaces) is the post-v1.0 deepening, filed below. (2) CACHE: `cache_put` / `cache_get` / `cache_invalidate` over `cache::CacheRuntime` — String values v1 (JSON-encode richer shapes via std/json), provenance-keyed + invalidation-keyed, recorded/substituted as ordinary tool events so replay stays deterministic. Both surfaces follow the 47h Result-envelope semantics and the 46g invention contract (README, tour topics, inventions rows, reference docs, e2e tests).
