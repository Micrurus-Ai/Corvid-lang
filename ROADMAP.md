...
- [ ] 47a-no-python-default-scaffold   **[closes audit M11]** `corvid new`'s hello-world calls a Python `echo` tool via PyO3 (`corvid-driver/src/scaffold.rs:64-85`), contradicting the no-Python positioning in the first minute. New default scaffold uses an executing surface (io or the 33S4 pipeline shape); `tools.py` becomes an opt-in template (`corvid new --with-python-tools`); scaffold output text updated.
