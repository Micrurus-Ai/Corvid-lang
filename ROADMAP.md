...
- [ ] 50i-injection-taint-v1   **[INVENTION — design doc at slice time]** Untrusted-content taint: retrieved documents, user messages, and untrusted-MCP outputs carry a taint that cannot parameterize a `dangerous` tool without an explicit sanitization/confirmation boundary, checker-enforced. "Prompt injection is a compile error" — the industry's loudest fear (OWASP LLM #1) answered structurally. Pulls the filed confused-deputy row forward as v1. Sequenced LAST so the invention cannot hold the mechanical slices hostage.
