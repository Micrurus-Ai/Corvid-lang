...
### Phase 46 — AI-native expressiveness (~8–12 weeks) — **pre-phase chat held 2026-07-11**
...
- [ ] 46f-mcp-client   **[closes audit B9]** Consume MCP tool servers as Corvid tools: `import mcp "server-name"` or manifest-declared servers producing effect-tagged tool declarations; runtime MCP client (stdio + HTTP transports); approve/trust dimensions on MCP-sourced tools; replay quarantine for MCP calls. Currently explicitly deferred to v0.4 (`FEATURES.md:161`, `corvid-ast/src/decl.rs:256`) — this slice re-scopes that decision. Pre-phase chat on scope (client only for v1.0; server is post-v1.0).
