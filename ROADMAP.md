...
- [ ] 47g-trust-dangerous-coupling-decision   **[filed by 44d — semantics decision, pre-phase chat]** The compile-time approve gate keys ONLY on the `dangerous` keyword; the effect `trust:` dimension does NOT trigger it. Discovered live: the quickstart's original example (a tool with `trust: supervisor_required` and no `dangerous`) compiled clean — the book's central claim was false as written (44d fixed the docs to the shipped `dangerous` model). The open question: should the checker ALSO require approve when a tool's composed effect row carries `trust: supervisor_required` or `human_required` (i.e. derive dangerousness from the trust tier rather than relying on the author remembering the marker)? Arguments for: defense in depth — an author who declares supervisor-required trust but forgets `dangerous` today gets NO compile-time protection, which is exactly the silent-footgun shape the language exists to prevent. Arguments against: `dangerous` is explicit and greppable; deriving it couples two systems and could surprise-break existing programs (any tool with a high-trust effect row suddenly needs approve at every call site). Options: (a) derive approve-requirement from trust tier (breaking, safer), (b) warning when trust >= supervisor_required without `dangerous` (non-breaking nudge, W-code like the 33Q14 W0280 precedent), (c) status quo + doc clarity (44d already shipped the doc half). Recommendation: (b) for v1.0, revisit (a) post-v1.0. Acceptance: decision recorded here + implemented (if a/b) with pinning tests + book ch 07/08 updated to state the rule precisely.
